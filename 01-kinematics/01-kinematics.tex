...
\begin{frame}
  ...
  \item Difficult to perform vector arithmetics
  \end{itemize}
...
\begin{frame}{IJK Vector Notation}
  ...
  \vspace{-.2in}
  \begin{itemize}
  %\item $A_x$, $A_y$ and $A_z$ are the $x$, $y$ and $z$ components of the
  %  vector $\mb{A}$.
...
\begin{frame}{Dot Product}
  ...
  \eq{-.22in}{
    C=\mb{A}\cdot\mb{B}=\mb{A}\cdot\mb{B}=AB\cos\theta
  }
...
%  \frametitle{Average Velocity}
%
%  Instantaneous velocity is the average velocity at the limit
%  $\Delta t\rightarrow 0$
%  
%  \eq{-.2in}{
%    \boxed{\mb{v}=\lim_{t\rightarrow 0} \frac{\Delta\mb{x}}{\Delta t}}
%  }
%sstill\end{frame}
...
\begin{frame}{Converting Motion Graphs}
  ...
  \item The $d-t$ graph for motion with constant acceleration is part of a
    \emph{parabola}
    \begin{itemize}
    \item If the parabola is \emph{convex}, then velocity is positive
    \item If the parabola is \emph{concave}, then velocity is negative
    \end{itemize}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \item Only applicaable for constant acceleration
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Typical Problems}
  ...
  \item A lot of problems are \emph{smybolic}, which means that they deal with
