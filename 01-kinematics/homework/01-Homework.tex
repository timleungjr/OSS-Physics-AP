...
\begin{multicols}{2}
  ...
  \item A golf ball is hit from level ground and has a horizontal range of
    ...
    \end{enumerate}
  \end{enumerate}
%  \item A small cart is moving with an initial positive velocity of 4.0 m/s on a
%    track of negligible friction when it rolls up the ramp, just makes it over
%    the top, and rolls back down the ramp. The cart then rolls along the
%    level track. Which of the following graphs best represents the velocity
%    vs. time graph for the entire trip?
%    \begin{enumerate}[noitemsep,topsep=0pt,leftmargin=18pt,label=(\Alph*)]
%      (A)
%      (B)(C)
%      (D)
...
%  from $t = 0$ until the filters reach terminal velocity?
  ...
  \item Two velocity vectors $v_1$ and $v_2$ each have a magnitude of 10 m/s.
    Graph 1 shows the velocity $v_1$ at $t =\SI{0}{s}$, and then the same object
    has a velocity $v_2$ at $t=\SI{2}{s}$, shown in Graph 2. Which of the
    following vectors best represents the average acceleration vector that
...
%second time interval. The elevator starts from rest at time t = 0.
...
%time graph for the elevator for the 20-second time interval.Questions 49–50. A particle follows a parabolic path with the equation y =
...
%of time.
%50. On the diagram below, sketch arrows to represent the horizontal and
