...
\begin{frame}{Mass on a Spring}
  Applying Newton's second law of motion in the $x$-direction:
...
\begin{frame}{Mass on a Spring}
  ...
  \item To solve the above equation, we look for a function $x(t)$ where the
    second derivative w.r.t.\ time $\ddot{x}$ looks like $x$ but with a
    negative sign
  \item The obvious choice are the two trigonometric functions: $\sin(t)$ and
    $\cos(t)$. We start with this general form
    
    \eq{-.2in}{
      x(t)=A\cos(\omega t-\phi)
    }

    \vspace{-.15in}We usually prefer using $\cos$ over $\sin$ because
    $\cos(0)=1$, and oscillations usually begin at maximum amplitude $A$.
...
\begin{frame}{Damped Oscillator}
  ...
  \eq{-.2in}{
    \boxed{x(t)=A_0e^{-\frac{b}{2m}t}\cos(\omega't+\phi)}
  }
