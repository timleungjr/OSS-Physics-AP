...
\title{Classes 9 \& 10: Faraday's Law and Magnetic Induction}
...
\begin{frame}{Changing Magnetic Flux}
  ...
  \begin{enumerate}
  \item The magnitude of magnetic field is time-dependent:
    \begin{displaymath}
      |\vec B|=|\vec B|(t)
    \end{displaymath}
  \item The surface for integration is time-dependent:
    \begin{displaymath}
...
\begin{frame}{Example}
  ...
  \begin{tikzpicture}[scale=.35]
    ...
    \uncover<2->{
      ...
      \node[thick,draw=red,fill=red!5,text width=5.8cm]
      (flux) at (-14,2.8){\color{red}Magnetic flux through the wire loop into
        the page is increasing, and it is a function of time:
      ...
      \draw[axes,red] (flux) to[out=0,in=110] (-.75,1.5)
      node[below=-2]{$\mathcal S$};
    }
    
    \uncover<3->{
      \node[thick,draw=green!60!black,fill=green!5,text width=5.6cm]
      ...
      \foreach \theta in {0,45,90,...,360}
      \draw[rotate=\theta,vectors,green!60!black] (3,0)--(3,2.5)
      node[pos=1.2]{$\vec E$};
    }
    \uncover<4->{
      \node[thick,draw=blue!70!black,fill=blue!5,text width=7.4cm]
      (curr) at (14,0){\color{blue!70!black}An induced current runs in the wire
        in direction of the electric field (Ohm's law):
        
        \vspace{-.05in}\begin{displaymath}
          I(t)=\frac{\mathcal E(t)}R
        \end{displaymath}\par
        $R$ is the internal resistance of the wire loop, or it is
    ...
    \uncover<5>{
      \node[text width=14cm,font=\large] at (8,-7){
        One remaining (and very important) question: how do we know the
        direction of the electric field $\vec E$ and therefore the current $I$?
      };
    }
  \end{tikzpicture}
\end{frame}
  ...
  \begin{center}
    \fcolorbox{black}{yellow!10}{
      \begin{minipage}{.7\textwidth}
        ...
        such a direction as to oppose the change that produces them
      \end{minipage}
    }
  \end{center}
...
%\begin{frame}{Applying Lenz's Law}
%  \begin{columns}[t]
%    \column{.32\textwidth}
%    \begin{center}
...
%        \draw[vectors,cyan] ( .6,-.15)--+(0,1);
%        \draw[vectors,cyan] (-.6,-.15)--+(0,1);
%        \draw[vectors,cyan] (0,-.15)--+(0,1.1) node[above]{$\vec B$};
%      \end{tikzpicture}
%    \end{center}
%    \begin{itemize}
%    \item Constant magnetic flux
%    \item No induced \emph{emf}
...
\begin{frame}{AC Generator}
  \begin{columns}
    ...
    \eq{-.3in}{
      \mathcal E(t) =\left|\diff{\Phi_m}t\right| =
      \underbracket[1pt]{NAB\omega}_{\mathcal E_\text{max}}\sin(\omega t+\theta_0)
    }
  \end{columns}
\end{frame}
...
\begin{frame}{Motional EMF}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node {\pic{.6}{motional-emf-1}};
      \uncover<2->{
        \fill[magenta!60,opacity=.6] (-2.75,-1.37) rectangle (.97,1.17);

        \node[thick,draw=red,fill=pink!30,text width=4.7cm]
        (phi) at (-4,-3.3){\color{red}The magnetic flux through the circuit is
          given by
          
          \vspace{-.3in}
          \begin{displaymath}
            \Phi_m(t)=BA=B\ell x(t)
          \end{displaymath}\par
        };
      ...
      \uncover<3->{
        \node[thick,draw=green!60!black,fill=green!10,text width=7.6cm]
        (phi) at (2.5,-3.3){\color{green!60!black}When sliding the rod to the
          right at speed $v$, the rate of change in magnetic flux is

          \vspace{-.25in}
          \begin{displaymath}
            \mathcal E(t)=\left|\diff{\Phi_m(t)}t\right|
            =B\ell\diff xt=B\ell v(t)
          \end{displaymath}\par
        };
      }

      \uncover<4->{
        \node[thick,draw=blue!70!black,fill=blue!10,text width=3cm]
        (i) at (5,0){\color{blue!70!black}This emf induces a current of:
...
\begin{frame}{Solving Problems with Faraday's Law \& Lenz's Law}
  \centering
  \vspace{.2in}
  \begin{tikzpicture}[auto,node distance=145,thick]
    \node[fill=pink!40,font=\Large] (a) {
      $\displaystyle\Phi_m(t)=\int_{\mathcal S}\vec B\cdot\dl\vec A$};
    \node[fill=green!20,font=\Large] (b) [right of=a] {
      $\displaystyle\left|\diff{\Phi_m}t\right|=\mathcal E(t)$};
    \node[fill=blue!20,font=\Large] (c) [right of=b] {
      $I(t)=\dfrac{\mathcal E(t)}R$};
    \node[fill=orange!20,font=\Large] (d) [right of=c]{
      $P(t)=I^2R=\dfrac{\mathcal E^2}R$};
    \draw[->] (a)--(b);
    \draw[->] (b)--(c);
    \draw[->] (c)--(d);
    \draw[<-,red] (a)--+(0,-1.3)
    node[draw=red,fill=pink!30,text width=4.1cm,below]{
      Find the loop of circuit and calculate magnetic flux $\Phi_m$ through it
      as a function of time 
    };

    \draw[<-,green!60!black] (b)--+(0,-1.3) node[
      draw=green!60!black,fill=green!10,text width=3.55cm,below]{
    ...
    \draw[<-,blue!80!black] (c)--+(0,-1.3) node (cc) [
      draw=blue!80!black,fill=blue!10,text width=5cm,below]{
      Apply Ohm's law to find the induced current. $R$ can be the
      internal resistance, or from a resistor in the circuit
    };

    \draw[<-,orange] (d)--+(0,-1.3) node[
      draw=orange,fill=orange!10,text width=3.6cm,below]{
      Calculate power dissipated in the circuit, and compare to the
      power input
    };

    \node[fill=gray!20,draw=black,text width=10cm]
    (I) at (1.75,-3.9) {Direction of induced current $I$ must produce a magnetic
...
\begin{frame}{Self Inductance and Induced EMF}
  ...
  an electromotive force in the circuit according to Faraday's law:
