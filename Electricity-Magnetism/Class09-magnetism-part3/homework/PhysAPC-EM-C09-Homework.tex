...
\gentitle{9}{MAGNETISM, PART 3 (FARADAY'S LAW)}
...
\begin{questions}
  ...
  \begin{parts}
    \part\textbf{Determine} the magnetic flux $\Phi_m$ through the loop as a
    function of time.
    \vspace{\stretch1}
    
    \part\textbf{Graph} the magnetic flux $\Phi_m$ as a function of time on the
    axes below.
    \uplevel{
      \centering
      \begin{tikzpicture}[yscale=1.2]
        \draw[axes] (-.1,0)--(11,0) node[pos=0,left]{0} node[right]{$t$ [s]};
        \draw[axes] (0,-2.3)--(0,3) node[above]{
          $\Phi_m$ [\si{\tesla.\metre\squared}]};
        \foreach \x in {1,...,10}{
<<<<<<<< HEAD:Electricity-Magnetism/Class09-magnetism-part3/homework/PhysAPC-EM-C08-Homework.tex
          \draw[thick,dashed] (\x,-2.3)--(\x,2.7);
          \node[fill=white,below] at (\x,-.03) {$\x$};
========
          \draw[gray] (\x,-2.3)--(\x,2.7);
          \node[below] at (\x,-.03) {$\x$};
>>>>>>>> dba11cc (Fixed Class 9 material):Electricity-Magnetism/Class09-magnetism-part3/homework/PhysAPC-EM-C09-Homework.tex
        }
        \foreach \y in {-0.1,-0.05,0.05,0.1}
        \draw[gray] (0,\y*20)--(10.7,\y*20) node[pos=0,left,black]{$\y$};
        \foreach \y in {-2,-1.8,...,2.2} \draw(-.1,\y)--(.1,\y);
      \end{tikzpicture}
    }
    
    \part\textbf{Calculate} the magnitude of the induced emf in the loop.
    \vspace{\stretch1}
    \newpage
    
    \part\textbf{Calculate} the magnitude of the induced current in the loop.
    \vspace{\stretch1}
    
    \part\textbf{Indicate} the direction of the induced current on the diagram
    below.
    \cpic{.65}{flux-through-loop}
    \vspace{\stretch4}
  \end{parts}
...
>>>>>>>> dba11cc (Fixed Class 9 material):Electricity-Magnetism/Class09-magnetism-part3/homework/PhysAPC-EM-C09-Homework.tex
      ...
      \draw[vectors] (-2,-1)--(2,-1) node[below]{$I$};
      \draw[very thick] (1.9,-1)--(6,-1);
    \end{tikzpicture}
  }
  ...
  \begin{parts}
<<<<<<<< HEAD:Electricity-Magnetism/Class09-magnetism-part3/homework/PhysAPC-EM-C08-Homework.tex
    \part\textbf{Calculate} the magnetic flux through the rectangular loop in
    terms of $I$, $\ell$, and fundamental constants.
========
    \part\textbf{Derive} an algebraic expression for the magnetic flux through
    the rectangular loop in terms of $I$, $\ell$, and fundamental constants.
>>>>>>>> dba11cc (Fixed Class 9 material):Electricity-Magnetism/Class09-magnetism-part3/homework/PhysAPC-EM-C09-Homework.tex
    \vspace{\stretch1}
    ...
    \part\textbf{Indicate} the direction of the current induced in the loop.
    ...
    \part\textbf{Determine} the current in the loop as a function of time $t$
    \vspace{\stretch1}
    
    \part\textbf{Determine} the total energy dissipated in the loop from $t=0$
    to $t=\infty$
    \vspace{\stretch1}
  ...
  % TAKEN FROM 2024 AP PHYSICS C EXAM FREE-RESPONSE SET 2 QUESTION 3.
  \uplevel{
    \centering
    \begin{tikzpicture}[scale=.75]
      \draw[axes] (0,0)--(10,0) node[right]{$x$};
      \foreach \x in {0,...,9} \draw (\x,.15)--(\x,-.15);
      \node[below] at (0,-.1) {$0$};
      \node[below] at (2,-.1) {$D$};
      \node[below] at (4,-.1) {$2D$};
      \node[below] at (6,-.1) {$3D$};
      \node[below] at (8,-.1) {$4D$};
      \draw[thick,dashed] (2,.5) rectangle (7,7.5);
      \draw[thick,dashed] (4,.5)--(4,7.5);

      \draw[axes] (10,5)--+(1.25,0) node[right]{$+x$};
      \draw[axes] (10,5)--+(0,1.25) node[above]{$+y$};
      \fill (10,5) circle (.08);
      \draw[thick] (10,5) circle (.25) node[below left]{$+z$};
      \draw[|<->|] (2,8)--(4,8) node[midway,above]{Region 1};
      \draw[<->|] (4,8)--(7,8) node[midway,above]{Region 2};
      \foreach \x in {2.5,3.3}{
        \foreach \y in {1,...,7} \fill (\x,\y) circle (.1);
      }
      \foreach \x in {4.3,4.9,...,7}{
        \foreach \y in {1,...,7} \fill (\x,\y) circle (.1);
      }
      \draw[thick] (0,2.8) rectangle +(-2,2);
      \fill (0,3.8) circle (.1) node[right]{$S$};
      \draw[thick,fill=gray!70] (-1.8,4.7) rectangle (-.2,4.9)
      node[midway,above]{$R$};
      \draw[|<->|] (-2,2.4)--+(2,0) node[midway,fill=white]{$D$};

      \draw[vectors] (-1.8,6)--+(1.6,0) node[midway,above]{$\vec v$};
  ...
  \question A wire is connected to a resistor of resistance $R$ to form a rigid
  square loop of side length $D$. An external force is exerted on the loop so
  that the loop always moves with constant speed $v$ in the $+x$ direction, as
  shown above in Figure 1. The loop then enters Region 1 of external uniform
  magnetic field of magnitude B that is directed in the $+z$ direction. Region
  1 has boundaries $x=D$ and $x=2D$. The loop later enters Region 2 of external
  uniform magnetic field of magnitude $2B$ that is directed in the $+z$
  direction. Region 2 has boundaries $x=2D$ and $x=3.5D$. Point S is the
  midpoint of the leading edge of the loop.

  \begin{parts}
    \part On the following axes, \textbf{sketch} a graph of the magnetic flux
    $\Phi$ through the square loop as a function of the position $x$ of Point S
    from $x=0$ to $x=4.5D$. The $+z$ direction indicated in Figure 1
    corresponds to $+\Phi$.
    \uplevel{
      \centering
      \begin{tikzpicture}[scale=.8]
        \draw[axes] (0,0)--(10,0) node[pos=0,left]{$O$} node[right]{$x$};
        \draw[axes] (0,-5.3)--(0,6) node[above]{$\Phi$};
        \draw[gray,thin] (0,-5) grid (9,5);
        \node[below] at (2,0) {$D$};
        \node[below] at (4,0) {$2D$};
        \node[below] at (6,0) {$3D$};
        \node[below] at (8,0) {$4D$};
    ...
    \part\textbf{Indicate} whether the current $I_S$ that is induced in the
    square loop when Point S reaches $x=1.5D$ is clockwise, counterclockwise,
    or zero.

    \vspace{.15in}
    \underline{\hspace{.3in}} Clockwise\hspace{.3in}
    \underline{\hspace{.3in}} Counterclockwise\hspace{.3in}
    \underline{\hspace{.3in}} Zero

    \vspace{.15in}Briefly \textbf{justify} your answer.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for $I_S$ when Point S reaches $x=1.5D$.
    If $I_S=0$, indicate how the derived expression shows that $I_S =0$.
    Express your answer in terms of $R$, $D$, $v$, $B$, and physical constants,
    as appropriate.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the power $P$ dissipated by the
    resistor when Point S reaches $x=1.5D$. Express your answer in terms of
    $R$, $D$, $v$, $B$, and physical constants, as appropriate.
    \vspace{\stretch1}
    \newpage
    
    \uplevel{
      The total energy dissipated by the resistor in the square loop as Point S
      moves from $x=0$ to $x=4.5D$ is $E_\text{original}$.

      \vspace{.15in}The vertical boundary between regions 1 and 2 is now
      shifted to $x=2.5D$. After the boundary is shifted, the square loop again
      moves with speed $v$ in the $+x$ direction, as shown in Figure 2. The
      total energy dissipated by the resistor as Point S moves from $x=0$ to
      $x=4.5D$ is $E_\text{new}$.
      \begin{center}
        \begin{tikzpicture}[scale=.75]
          \draw[axes] (0,0)--(10,0) node[right]{$x$};
          \foreach \x in {0,...,9} \draw (\x,.15)--(\x,-.15);
          ...
          \node[below] at (8,-.1) {$4D$};
          \draw[thick,dashed] (2,.5) rectangle (7,7.5);
          \draw[thick,dashed] (5,.5)--+(0,7);
          
          \draw[axes] (10,5)--+(1.25,0) node[right]{$+x$};
          \draw[axes] (10,5)--+(0,1.25) node[above]{$+y$};
          \fill (10,5) circle (.08);
          \draw[thick] (10,5) circle (.25) node[below left]{$+z$};
          \draw[|<->|] (2,8)--(5,8) node[midway,above]{Region 1};
          \draw[<->|] (5,8)--(7,8) node[midway,above]{Region 2};
          \foreach \x in {2.3,3.1,...,5}{
            \foreach \y in {1,...,7} \fill (\x,\y) circle (.1);
          }
          \foreach \x in {5.3,6,6.7}{
            \foreach \y in {1,...,7} \fill (\x,\y) circle (.1);
          }
          \draw[thick] (0,2.8) rectangle +(-2,2);
          \fill (0,3.8) circle (.1) node[right]{$S$};
    ...
    \part\textbf{Indicate} whether $E_\text{new}$ is greater than, less than,
    or equal to $E_\text{original}$.

    \vspace{.15in}
    \underline{\hspace{.3in}} $E_\text{new} > E_\text{original}$\hspace{.3in}
    \underline{\hspace{.3in}} $E_\text{new} < E_\text{original}$\hspace{.3in}
    \underline{\hspace{.3in}} $E_\text{new} = E_\text{original}$
    
    \vspace{.15in}Briefly \textbf{justify} your answer.
    \newpage

    \uplevel{
      The original magnetic fields are modified so that the region $D<x3.5D$
      contains an external uniform magnetic field of magnitude $B$ that is
      directed in the $+z$ direction.

      \vspace{.15in}A new wire is connected to a resistor of resistance $R$ to
      form a rigid triangular loop with base length $D$ and height $D$. An
      external force is exerted on the loop so that the loop always moves with
      speed $v$ in the $+x$ direction, as shown in Figure 3. Point S represents
      the upper-leading corner of the loop.
      ...
      \begin{center}
        \begin{tikzpicture}[scale=.75]
          ...
          \foreach \x in {2.5,3.3,...,7}{
            \foreach \y in {1,...,7} \fill (\x,\y) circle (.1);
          }

          \draw[thick] (.3,2.8)--(.3,4.8)--(-1.7,2.8)--cycle;
          \fill (.3,4.8) circle (.1) node[above]{$S$};
          \draw[thick,fill=gray!70,rotate around={45:(-1.7,2.8)}]
          (-1,2.65) rectangle +(1.5,.3) node[midway,above left]{$R$};
          \draw[|<->|] (-1.7,2.4)--+(2,0) node[midway,fill=white]{$D$};
          \draw[|<->|] (.7,2.8)--+(0,2) node[midway,fill=white]{$D$};
    ...
    \part On the following axes, \textbf{sketch} a graph of the induced current
    $I_T$ in the loop as Point S moves from $x=D$ to $x=3D$.

    \uplevel{
      \centering
      \begin{tikzpicture}[scale=.9]
        \fill[gray!30] rectangle (2,5.3);
        \fill[gray!30] (6,0) rectangle (9,5.3);
        \draw[axes] (0,0)--(10,0) node[right]{$x$};
        \draw[axes] (0,0)--(0,6) node[above]{$I_T$};
        \draw[dashed] grid (9,5.3);
        \foreach \x in {1,...,9} \draw (\x,.15)--(\x,-.15);
        \node[below] at (0,-.1) {$0$};
        \node[below] at (2,-.1) {$D$};
        \node[below] at (4,-.1) {$2D$};
        \node[below] at (6,-.1) {$3D$};
