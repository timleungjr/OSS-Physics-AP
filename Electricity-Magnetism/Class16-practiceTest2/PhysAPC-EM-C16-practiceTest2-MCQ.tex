...
\begin{center}
  {
    ...
    \textbf{PHYSICS C: ELECTRICITY AND MAGNETISM\\SECTION I\\
      TIME -- 50 MINUTES}
  }
\end{center}
...
\begin{questions}
  ...
  \question A spherical charge distribution varies with the radius $r$ by the
  equation $\rho=ar$, where $\rho$ is the volume charge density and $a$ is a
  positive constant. The distribution goes out to a radius $R$. Which of the
  following is true of the electric field strength due to this charge
  distribution at a distance $r$ from the centre?
  \begin{choices}
    \choice It increases as $r$ approaches infinity.
    \choice It decreases linearly for $r>R$.
    \choice It increases linearly for $r>R$.
    \choice It increases linearly for $r<R$.
    \choice It increases non-linearly for $r<R$.
  \end{choices}
  \newpage
  
  \question The electric potential along an $x$-axis is given by the equation
  $V(x)=ax-bx^2$, where $a$ and $b$ are constants. At what point on the
  $x$-axis is the electric field zero?
  \begin{choices}
    \choice $x=0$
    \choice $x=\dfrac a{2b}$
    \choice $x=\dfrac ab$
    \choice $x=\dfrac{3a}{2b}$
    \choice At no point
  \end{choices}
  \newpage
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[thick]
      ...
      \foreach \x in {0,2}{
        \foreach \y in {0,2} \draw (\x,\y)--+(1.1,1.1);
      }
      \draw[|<->|] (0,-.3)--+(2,0) node[midway,fill=white]{$d$};
      \draw[|<->|] (-.3,0)--+(0,2) node[midway,fill=white]{$d$};
      \draw[|<->|,rotate around={-45:(2,0)}]
      (2.25,0)--+(0,1.55) node[midway,fill=white,rotate=45]{$d$};
    \end{tikzpicture}
    \hspace{.3in}
    \begin{tikzpicture}[thick,scale=1.2]
      \draw circle (1);
      \draw[|<->|] (-1,-1.3)--+(2,0) node[midway,fill=white]{$d$};
      \draw (-1,0) arc (180:360:1 and .35);
    \end{tikzpicture}
  }
  \question The figure above shows two Gaussian surfaces: a cube with side
  length $d$ and a sphere with diameter $d$. The net electric charge enclosed
  within each surface is the same, $+Q$. If $\Phi_C$ denotes the total electric
  flux through the cubical surface, and $\Phi_S$  denotes the total electric
  flux through the spherical surface, then which of the following is true?
  \begin{choices}
    \choice $\Phi_C=\dfrac\pi6\Phi_S$
    \choice $\Phi_C=\dfrac\pi3\Phi_S$
    \choice $\Phi_C=\Phi_S$
    \choice $\Phi_C=\dfrac 3\pi\Phi_S$
    \choice $\Phi_C=\dfrac 6\pi\Phi_S$
  \end{choices}
  \newpage
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      ...
      \draw[axes,rotate=20] (0,0)--(1,0) node[midway,below]{$R$};
      \draw[axes,rotate=70] (0,0)--(2,0) node[pos=.75,left]{$2R$};
      \draw[thick,fill=gray!10] (-.05,-1) rectangle (.05,-2);
      \draw[axes] (-1,-2.5) to[out=0,in=180] (-.1,-1.5);
      \node[left,text width=1.5cm] at (-1,-2.5) {insulating support};
    \end{tikzpicture}
  }
  \question The figure above shows a cross section of two concentric spherical
  metal shells of radii $R$ and $2R$, respectively. Find the capacitance.
  \begin{choices}
    \choice $\dfrac1{8\pi\varepsilon_0R}$
    \choice $\dfrac1{4\pi\varepsilon_0R}$
    \choice $2\pi\varepsilon_0R$
    \choice $4\pi\varepsilon_0R$
    \choice $8\pi\varepsilon_0R$
  \end{choices}
  \newpage
...
%  \newpage
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      ...
      \draw[very thick,fill=white] (2,0) circle (.3) node[below=9]{Wire R};
      \fill (2,0) circle (.2);
      \draw (2,.3)--(2,.7) node[above]{$x=0$};

      \draw[very thick,fill=white] (4,0) circle (.3) node[below=9]{Wire S};
      \fill (4,0) circle (.2);
      \draw (4,.3)--(4,.7) node[above]{$x=d$};
      
      \fill (6,0) circle (.1) node[below right]{P};

      \draw[very thick,fill=white] (8,0) circle (.3) node[below=9]{Wire T};
  ...
  \uplevel{
    \centering
    \begin{tikzpicture}
      \draw[thick,fill=lightgray] circle (1.5);
      \draw[axes,rotate=-65] (0,0)--(1.5,0) node[midway,left]{$a$}
      node[right=10]{$(R>a)$};
      \begin{scope}[rotate=10]
        \draw[thick] (0,0)--(3,0);
        \draw[axes]  (0,0)--(2.3,0) node[below]{$R$};
        \draw[<->] (1.6,0) arc (0:30:1.6) node[midway,right=-1]{$\theta$};
        \draw[thick] (3,0) arc (0:30:3)--(0,0);
        \draw[axes]  (3,0) arc (0:18:3);
      \end{scope}
      \draw[axes,rotate=40] (3,0)--(2.3,0) node[above=3]{$R$};
      \node at (-.4,.2) {$I$};
    \end{tikzpicture}
  }
  
  \question A long, straight wire of radius $a$ carries a current $I$ out of
  the page, which is uniformly distributed over the cross section of the wire.
  The value of $\oint \vec B\cdot\dl{\vec\ell}$, the line integral of the
  magnetic field $\vec B$ around the wedge-shaped path, equals which of the
  following?
  \begin{choices}
  \item $\dfrac{\mu_0\theta I}{2\pi}$
  \item $\dfrac{\mu_0\theta I}{2\pi^2a^2}$
  \item $\dfrac{\mu_0\theta I}{2\pi^2R^2}$
  \item $\dfrac{\mu_0I}{\pi a^2}$
  \item $\dfrac{\mu_0I}{\pi R^2}$
  \end{choices}
  \newpage
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=.8]
      \foreach \y in {0,...,3}\draw[vectors] (0,\y)--+(4,0);
      \node at (4.3,1.5) {$\vec B$};
      \node at (2,1.5) {\Large$\times$};
      \node[right=4] at (2,1.5) {$\vec v$};
    \end{tikzpicture}
  }
  
  \question A dipole molecule is travelling into the plane of the page through a
  uniform magnetic field directed to the right. Which of the following
  arrangements of the dipole would result in zero net force, but a non-zero net
  torque?
  
  \begin{oneparchoices}
    \choice
    \begin{tikzpicture}[thick]
      \draw circle (.3) node{\LARGE$+$};
      \draw (0,-2) circle (.3) node{\LARGE$-$};
      \draw (0,-.3)--(0,-1.7);
    \end{tikzpicture}
    \hspace{.3in}
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=.7]
      ...
      \draw[very thick] (2.5,2.5) circle (2.1);
      \node at (4.5,4.5){$\vec B$};
    \end{tikzpicture}
  }
  \question The metal loop of wire shown in the above diagram is situated in a
  magnetic field $\vec B$ pointing out of the plane of the page. If $\vec B$
  decreases uniformly in strength, the induced electric current within the loop
  is
  \begin{choices}
    \choice clockwise and decreasing
    \choice clockwise and increasing
    \choice counterclockwise and decreasing
    \choice counterclockwise and constant
    \choice counterclockwise and increasing
  \end{choices}
  \newpage
  ...
  \uplevel{
    \textbf{Questions \ref{rcirc1}--\ref{rcirc2}}
    \begin{center}
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[thick,scale=1.25]
      \draw (0,0) to[battery,l=$\mathcal E$] (0,2)--(4,2)--(4,0)--(0,0);
      \draw[fill=white] (2,1.6) rectangle +(1.5,.8) node[midway]{metal};
      \foreach \x in {1.7,2.1,...,3.8}{
        \foreach \y in {1.4,1.8,...,2.7}
        \fill (\x,\y) circle (.03);
      }
      \node at (2.7,2.8){$\vec B$};
      \fill (3.1,1.6) circle (.045) node[below]{$Y$};
      \fill (3.1,2.4) circle (.045) node[above]{$X$};
      %(1.75,.35)--(1.2,0) node[midway,above left]{$S$}--(0,0)
      % (0,2) to[R,l=$R$] (4,2) to[C,l=$C$]
      %(4,0)--(1.8,0);
    \end{tikzpicture}
  }
  \question A piece of metal in the plane of the page is connected in a circuit
  as shown above, causing electrons to move through the metal to the left. The
  piece of metal is in a magnetic field $\vec B$ directed out of the page. X
  and Y are points on the edge of the metal. Which of the following statements
  is true?
  \begin{choices}
    \choice The current will decrease to zero due to the magnetic field.
    \choice The potentials at X and Y are equal.
    \choice X is at a higher potential than Y.
    \choice Y is at a higher potential than X.
    \choice The current will increase exponentially due to the magnetic field.
  \end{choices}
