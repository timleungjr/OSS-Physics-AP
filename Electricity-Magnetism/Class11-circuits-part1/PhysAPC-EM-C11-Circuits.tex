...
\begin{frame}{Electric Current}
  The \textbf{electric current} $I(t)$ is the rate at which \textbf{charges}
  $Q$ pass through a point in a circuit.\footnote{To be clear, as discussed in
  Classes \#6 and \#7, \emph{current}, measured in \emph{amp\`{e}re}, is the
  base SI unit, so the formal definition should be written as:
  \begin{displaymath}
    Q(t)=\int I(t)\dl t
  \end{displaymath}
  } In differential form, the instantaneous electric current $I$:
...
\begin{frame}{Electric Current}
  ...
  We can expand the equation for electric current
  
  \eq{-.1in}{
    I(t)=\diff Qt = \left[\frac QV\right]\diff Vt = (ne)(Av_d)
  }
  \begin{itemize}
  \item $Q/V$ is the amount of free charges \emph{per volume}
    \begin{itemize}
    \item The \textbf{charge carrier density} (number of charge carriers per
      volume) $n$ times the \textbf{elementary charge} $e$
    \item Here, we assume that the electron is the charge carrier
    \end{itemize}
  \item $\diff V/t$ is the rate the volume of charges flows through the
    conductor
    \begin{itemize}
    \item i.e. the the volume flow rate,
    \item i.e.\ the velocity flux (discussed earlier in Class \#3 and \#4 as an
      introduction to Gauss's law)
    \end{itemize}
    given by the wire's cross-section area $A$ times the
...
\begin{frame}{Charge Carrier Density}
  Collecting all the terms from the last slide, we have a charge carrier
  density of:
...
\begin{frame}{Direct Current}
  ...
  \vspace{-.15in}\begin{columns}[T]
    ...
    \begin{center}
      ...
      \end{tikzpicture}
    \end{center}
    \vspace{-.1in}{\footnotesize A circuit with only resistors usually has a
      constant, time-independent current\par
    }
    ...
    \vspace{-.1in}{\footnotesize A circuit with capacitors and inductors has a
      current with a sinusoidal component\par}
  \end{columns}
\end{frame}
...
\begin{frame}{Current}
  ...
  per meters squared} (\si{\ampere\per\meter\squared}). We had encountered this
  quantity in Classes \#6 and \#7 when studying Amp\`{e}re's law:
  ...
  \begin{columns}[T]
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[scale=.4,very thick]
        ...
        \draw[dashed] (8,-3) arc (270:90:1 and 3);
        \draw[dashed,lightgray] (4,4)--(4,-4);
        \node at (2.5,0){\huge$I$};
        \node at (5.3,0){\pic{.45}{arrow-east}};
      \end{tikzpicture}
    \end{center}
    \vspace{-.07in}{\scriptsize\textbf{Conventional current} flows from the
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[scale=.4,very thick]
        ...
        \draw[thick,fill=pink!20] (5,1) circle (.3) node{\tiny$e^-$};

        \draw[axes] (8,-1)--(5,0)--(4.5,-.5)--(0,-.8); %--(0,0);
        \draw[thick,fill=pink!20] (8,-1) circle (.3) node{\tiny$e^-$};

        \draw[axes] (6.5,-1.8)--(4.2,-2)--(4,-1)--(2.5,-1.8)--(1,-1.5);
        \draw[thick,fill=pink!20] (6.5,-1.8) circle (.3) node{\tiny$e^-$};

        \draw[axes] (2.5,1.8)--(0,1.8);
        \draw[thick,fill=pink!20] (2.5,1.8) circle (.3) node{\tiny$e^-$};
      \end{tikzpicture}
    \end{center}
...
\begin{frame}{Electric Field Inside the Wire}
  ...
  \begin{itemize}
  \item As the charges move through a conductor,
    \begin{itemize}
    \item Electric field does positive work on the charges, therefore
    \item The charges lose potential energy (because electric force is
      conservative)
    \end{itemize}
...
\begin{frame}{Ohm's Law}
  Georg Ohm discovered that when an electric current passes through a conductor,
  the voltage (electric potential difference) $V$ at the two ends of the
  conductor is \emph{linearly proportional} to the amount of current $I$
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The proportionality constant between voltage and current is called
  \textbf{resistance}. \textbf{Note that Ohm's law is \emph{not} a fundamental
    law in physics.}
\end{frame}
...
\begin{frame}{Ohm's Law}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  Resistance is due to collisions between the electrons and the atoms in the
  conductor
  \begin{itemize}
...
\begin{frame}{Resistance of a Conductor}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \textbf{Don't confuse resistivity with the \emph{density} of the material;
    both use the same Greek letter $\rho$}
\end{frame}
...
\begin{frame}{Resistance of a Conductor}
  ...
  Resistivity $\rho$ is determined by the material.
  For an ``ohmic'' device, resistivity should be independent of
...
\begin{frame}{Resistivity and Electric Field}
  The resistivity of a material is proportional to the electric field inside
  the conductor, and current density:
...
\begin{frame}{Ohmic Devices}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[scale=.7]
      ...
      node[right]{$\text{slope}=\frac1R$};
      \draw[axes] (0,0)--(3,0) node[right]{$V$};
      \draw[axes] (0,0)--(0,3) node[right]{$I$};
    \end{tikzpicture}
  \end{center}
  Ohmic devices are loads that emit EM radiation (e.g.\ heat and light). For
  example:
  \begin{center}
    \begin{circuitikz}[american voltages,thick]
      \draw (0,0) to[R,l=Resistor,o-o] (2,0);
      \draw (3,0) to[bulb,l=Light Bulb,o-o] (5,0);
      \draw (6,0) to[lamp,l=Lamp,o-o] (8,0);
    \end{circuitikz}
  \end{center}

  {\footnotesize(Strictly speaking, light bulbs and lamps are \emph{not} ohmic,
    because their resistance increases with temperature. However, in AP exams,
    we will assume that their behaviour is approximately ohmic)\par}
\end{frame}
...
\begin{frame}{Non-Ohmic Devices}
  ...
  current flows through it, but the motor shaft/axle does not turn. All of the
  ...
  changes, as discussed in the slides for Classes 9 and 10.}
  \end{itemize}
  \vspace{.25in}
\end{frame}
...
\begin{frame}{Power Dissipated by a Resistor}
  Power ($P$) is the rate at which work ($W$) is done, and from electrostatics,
  the change in electric potential energy $\Delta E_q$ is proportional to the
...
\begin{frame}{Energy Sources}
  ...
  \emph{emf} from a solar panel is generally controlled by a device called the
  \emph{maximum power point tracker} (MPPT); a generic DC voltage may not have
  a constant \emph{emf}.
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{center}
      \begin{tikzpicture}[scale=1.5,thick]
        ...
        \draw[very thick,fill=black!2] (1.5,-.8) circle (.2) node{V};
      \end{tikzpicture}
    \end{center}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=1.5]
        ...
        \draw[very thick,fill=black!2] (.7,0) circle (.2) node{A};
      \end{tikzpicture}
    \end{center}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=1.5,thick]
        ...
        \draw (.7,0) to[short,*-] (.7,-.8)--(2.3,-.8) to[short,-*](2.3,0);
        \draw[very thick,fill=black!2] (1.5,-.8) circle (.2) node{$\Omega$};
      \end{tikzpicture}    
    \end{center}
...
\begin{frame}{Kirchhoff's Junction Law}
  ...
  \begin{columns}[T]
    ...
    the junction.
  \end{columns}
  \vspace{.1in}It means that there must be \emph{no} accumulation of charges
...
\begin{frame}{Kirchhoff's Voltage Law}
  ...
  \begin{columns}
    ...
    \vspace{-.3in}{\large
      \begin{align*}
        +\mathcal E-V_{R_1}-V_{R_2} &= 0\quad\text{(ABCHA)}\\
        +\mathcal E-V_{R_1}-V_C &= 0\quad\text{(ABCDGHA)}\\
        +\mathcal E-V_{R_1}-V_L &=0\quad\text{(ABCDEFGHA)}
      \end{align*}
    }
  \end{columns}
