...
\title{Class 13: Circuit Analysis, Part 2}
...
\section{Review: Capacitors in Circuit}
...
\begin{frame}{Discharging a Capacitor}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.2,thick]
      \draw (.7,0) to[short,*-] (0,0) to[R=$R$] (0,2)--(2,2) to[C=$C$]
      (2,0)--(1.2,0) to[short,-*] (.78,-.3);
    \end{tikzpicture}
  \end{center}
  Initially ($t<0$), the switch is open. At this time, let's assume that,
  before the switch is closed, there is already an initial charge of $\pm Q_0$
  on either side of the capacitor, and therefore an initial voltage of
  $V_0=Q_0/C$.
\end{frame}
...
\begin{frame}{Discharging a Capacitor}
  \begin{columns}
    ...
    \item As current starts to flow, the charge on the capacitor decreases
      ...
      \end{itemize}
    \item The voltage across a capacitor decreases as a function of time:
      $V_c(t)=Q(t)/C$
...
\begin{frame}{Discharging a Capacitor}
  ...
  \eq{-.1in}{
    V_c-\underbracket[1pt]{V_R}_{IR}=0\quad\to\quad
    \frac QC+R\diff Qt=0
  }
\end{frame}
...
\begin{frame}{Charging a Capacitor}
  \begin{columns}[T]
    ...
    \item Current flow $I$ \emph{increases} the charge in the capacitor, i.e.\
      $I=+\diff Q/t$
    \item Voltage across the capacitor $V_c(t)=Q(t)/C$ also increases
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Charging a Capacitor}
  ...
  \eq{-.1in}{
    \mathcal E-\underbracket[1pt]{V_R}_{IR}-\underbracket[1pt]{V_c}_{Q/C}=0
    \quad\to\quad
...
\begin{frame}{Charging a Capacitor}
  ...
  \eq{-.1in}{
    ...
    \quad\to\quad-\ln(C\mathcal E-Q)=\frac t{RC}+K
  }
...
\begin{frame}{Practical RC Circuits}
  ...
  \begin{enumerate}[(a)]
  \item Immediately after the switch is closed (the ``initial'' behaviour
    at $t=0$)
  \item A long time after the switch is closed (the ``steady-state''
...
\begin{frame}{Typical RC Circuit Problem}
  \begin{columns}
    \column{.35\textwidth}
    Immediately after the switch is closed (i.e.\ at $t=0$), the circuit behaves
    like this:

    \vspace{.2in}
    \begin{tikzpicture}[scale=1.1,american voltages]
      \draw[thick] (4,0)--(0,0)
      to[battery,l=12<\volt>] (0,2) to[R=4<\ohm>] (2,2)--(4,2)--(4,0);
      \draw[thick] (3,0) to[R=8<\ohm>] (3,2);
      \draw[line width=6,yellow,opacity=.3] (0,0)--(0,2)--(4,2)--(4,0)--cycle;
    \end{tikzpicture}

    \column{.65\textwidth}
    \begin{itemize}
    \item No charges and therefore voltage across the capacitor; it acts like a
      short circuit

      \eq{-.2in}{
        V_c(0)=0\quad Q_c(0)=0
      }
    \item No voltage across the \SI8{\ohm} resistor (in parallel with the 
      capacitor), therefore no current through it:

      \eq{-.2in}{
        I_{\SI8\ohm}=0\quad V_{\SI8\ohm}=0
      }
    \item Current out of battery:

      \eq{-.33in}{
        I_\text{batt}(0)=I_{\SI4\ohm}(0)=I_c(0)=\frac{\mathcal E}{R_{\SI4\ohm}}
        =\SI3\ampere
      }
    \end{itemize}
  \end{columns}
...
\begin{frame}{Typical RC Circuit Problem}
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=1.1,american voltages]
      \draw[thick] (4,0)--(0,0)
      to[battery,l=12<\volt>] (0,2) to[R=4<\ohm>] (2,2)--(4,2);
      \draw[very thick,red] (4,2) to[short,-o] (4,1.2);
      \draw[very thick,red] (4,.8) to[short,o-] (4,0);
      \draw[thick] (3,0) to[R=8<\ohm>] (3,2);
      \draw[line width=6,yellow,opacity=.3] (0,0)--(0,2)--(3,2)--(3,0)--cycle;
    \end{tikzpicture}

    \column{.65\textwidth}
    A long time after the switch is closed (i.e.\ as $t\to\infty$);
    \begin{itemize}
    \item The capacitor is fully charged. There is no current through it; it
    ...
    \item Current out of battery:
      ...
      \eq{-.3in}{
        I_\text{batt}(\infty)=I_{\SI4\ohm}(\infty)=I_{\SI8\ohm}(\infty)=
        \frac{\mathcal E}{R_s}=\SI1\ampere
      }
   
    \item\vspace{-.2in}The voltage across capacitor is the same as across the
      \SI8{\ohm} resistor (they are parallel):

      \eq{-.35in}{
        V_c(\infty)=V_{\SI8\ohm}(\infty)=I(\infty)R=\SI8\volt
      }
...
\begin{frame}{Typical RC Circuit Problem}
  Once we know the initial ($t=0$) and ``steady-state'' ($t\to\infty$)
  solutions, we can sketch what is in between.  We cannot (easily) find the
  time constant for such a circuit, but the overall behaviour is still
  an exponential decay
  \begin{center}
    \begin{tikzpicture}[scale=1.1,american voltages,thick]
      ...
      \begin{scope}[red]
        ...
        \draw[thick] (-2.5,3.5)--+(-.1,0) node[left=-3]{\SI3\ampere};
        \draw[thick] (-2.5,2.5)--+(-.1,0) node[left=-3]{\SI1\ampere};
      ...
      \begin{scope}[violet]
        ...
        \draw[axes] (0,3)--+(0,1.8);
        \draw[thick] (0,4.5)--+(-.1,0) node[left=-3]{\SI{12}\volt};
        \draw[thick] (0,3.5)--+(-.1,0) node[left=-3]{\SI4\volt};
      ...
      \begin{scope}[orange]
        ...
        \draw[thick] (2.5,3.5)--+(-.1,0) node[left=-3]{\SI1\ampere};
        ...
        \draw[thick] (5,3.5)--+(-.1,0) node[left=-3]{\SI8\volt};
      ...
      \begin{scope}[blue]
        ...
        \draw[thick] (5.5,1.5)--+(-.1,0) node[left=-3]{\SI3\ampere};
...
\begin{frame}{Analyzing LR Circuits}
  \begin{columns}
    ...
    \eq{-.3in}{
      \mathcal E-V_R-V_L=0\quad\to\quad
...
\begin{frame}{Analyzing LR Circuits}
  ...
  \item As $t\to\infty$, magnetic flux no longer changes with time, and
...
\begin{frame}{Practical RC Circuits: Example}
  \begin{columns}[T]
    ...
    \column{.5\textwidth}{\footnotesize
      As $t\to\infty$, magnetic flux through the inductor is no longer
      changing, and no back \emph{emf} is generated. The inductor acts like a
...
\begin{frame}{LC Circuit}
  Since both sides are continuously differentiable, we can differentiate the
  equation with respect to time, which gives us a second-order ordinary
