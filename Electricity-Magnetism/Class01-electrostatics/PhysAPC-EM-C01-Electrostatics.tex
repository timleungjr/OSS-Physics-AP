...
\title{Classes 1 \& 2: Electrostatics, Part 1 (Point Charges)}
...
\begin{frame}{Coulomb's Law for Electrostatic Force}
  ...
  course.} $\vec F_q$ is a mutually repulsive or attractive force between all
...
\begin{frame}{Coulomb's Constant}
  ...
  \eq{-.1in}{
    \vec F_{12}=\frac{{\color{red}k}q_1q_2}{|\vec r_{12}|^2}\hat r_{12}
  }

  The constant $\color{red}{k}$ in the Coulomb's law is called the
  ...
  where $\varepsilon_0$ is a fundamental constant called the
...
\begin{frame}{More Than One Charge}
  A charge $Q$ subjected to the influence of $N$ multiple discrete point
  charges $q_i$ will experience a total force that is the vector sum of all the
  forces $\vec F_i$:
  \begin{columns}
    \column{.27\textwidth}
    \centering
    \begin{tikzpicture}[scale=.4]
      \begin{scope}[rotate=45]
        \draw[vectors,blue] (0,0)--(2,0) node[right]{$\vec F_1$};
        \shade[ball color=blue] (7,0) circle (.2) node[above=3,blue]{$q_1$};
      \end{scope}

      \begin{scope}[rotate=135]
        \draw[vectors,orange] (0,0)--(-3,0) node[right]{$\vec F_2$};
        \shade[ball color=orange] (4,0) circle (.2) node[above=3,orange]{$q_2$};
      \end{scope}

      \begin{scope}[rotate=270]
        \draw[vectors,magenta] (0,0)--(1.5,0) node[below]{$\vec F_3$};
        \shade[ball color=magenta] (6,0) circle (.2) node[below=3,magenta]{
          $q_3$};
      \end{scope}
...
%      \end{scope}
      ...
      \shade[ball color=red] circle (.4) node[white]{$Q$};
    \end{tikzpicture}
    ...
    If charge $Q$ is free to move, it will have an acceleration based on the
    second law of motion:

    \eq{-.2in}{
      \vec a=\frac{\vec F_q}m
    }

    \vspace{-.1in}where $m$ is the mass of charge $Q$. As it accelerates, both
    magnitude and direction of $\vec a$ changes. Predicting the motion of $Q$
    is generally difficult, or at least tedious
  \end{columns}
\end{frame}
...
\begin{frame}{Continuous Distribution of Charges}
  ...
  used to describe the electroststatic force on charges with
  \emph{spatial extend} i.e.\ charges that take up physical space (e.g.\ a
  ...
  The definition of the infinitesimal charge $\dl q$ is similar to that of the
  infinitesimal mass $\dl m$ from AP Physics C Mechanics.
\end{frame}



\begin{frame}{Infinitesimal Charge $\dl q$}
  %The infinitesimal charge $\dl q$ is defined similar to the infinitesimal
  %mass $\dl m$ introduced in AP Physics C Mechanics.
  %\footnote{At Meritus, this
  %is discussed in Class 6: Centre of Mass, and Class 9: Rotational Motion}
  Like mass densities, charge densities also do not have to be uniform
  (constant):
  \begin{itemize}
  ...
  \end{itemize}
  You may be asked to do simple integration in an AP Physics C exam.
\end{frame}
...
\begin{frame}{Review of Gravitational Field}
  To understand the concept of electric field, we first review the topic of
  \textbf{gravitational field} which was studied in AP Physics 1, Grade 12
  Physics, and AP Physics C (Mechanics).
\end{frame}
...
\begin{frame}{Review of Gravitational Field}
  \begin{center}
    \begin{tikzpicture}[scale=.65]
      \draw[vectors,red] (0,0)--(2,0) node[right]{$\vec F_g$};
      \draw[vectors,blue] (8,0)--(6,0) node[left]{$\vec F_g$};
      \shade[ball color=red] circle (.35) node[white]{$m_1$};
      \shade[ball color=blue] (8,0) circle (.55) node[white]{$m_2$};
      \foreach \x in {0,8} \draw[dashed] (\x,0)--+(0,-1.2);
      \draw[thick,<->] (0,-1)--+(8,0) node[midway,fill=black!2]{$r$};
    \end{tikzpicture}
  \end{center}
  In classical mechanics, \textbf{gravity} is the mutually attractive force
  between massive objects governed by the \textbf{law of universal gravitation}:

  \eq{-.1in}{
    F_g=\frac{Gm_1m_2}{r^2}
  }

  which has a very similar form to Coulomb's law that we have just studied
\end{frame}


\begin{frame}{Gravitational Field}
  When solving for force problems where a mass is under the influence of
  gravity, we generally describe the gravitational force (weight) as:
  
  \eq{-.1in}{
    \vec F_g=m\vec g
  }

  To find ``$\vec g$'', we group the variables in the law of universal
  gravitation:
  ...
  The vector field function $\vec g$ is known as the \textbf{acceleration due
    to gravity} in kinematics, and \textbf{gravitational field}.
\end{frame}
...
\begin{frame}{Gravitational Field}
  ...
  \eq{-.1in}{
    g(m,\vec r)=-\frac{Gm}{r^2}\hat r
  }

  The SI unit for gravitational field is
  \textbf{newton per kilogram} (\si{\newton\per\kilo\gram}).
  %\begin{center}
  %  \begin{tabular}{l|c|c}
  %    \rowcolor{pink}
  %    \textbf{Quantity} & \textbf{Symbol} & \textbf{SI Unit} \\ \hline
...
\begin{frame}{More Than One Mass}
  When $N$ multiple point masses are present, the total gravitational field at
  any position $\vec r$ is the vector sum of all the fields $\vec g_i$:
...
\begin{frame}{Relating Gravitational Field \& Gravitational Force}
  ...
  Note: A mass is not affected by its own gravitational field
\end{frame}
...
\begin{frame}{Electric Field}
  ...
  procedure as with gravitational field for masses, by grouping the variables
  in Coulomb's law:
  ...
  \textbf{vector field}) that describes how $q_1$ influences other charged
  particles around it.
\end{frame}
...
\begin{frame}{Electric Field Near a Point Charge}
  The electric field $\vec E$ a distance $r$ away from a source (point) charge
  $q_s$ is given by:

  \eq{-.1in}{
    \boxed{\vec E(q_s,\vec r)=\frac{kq_s}{r^2}\hat r}
  }
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
      ...
      Source point charge         & $q_s$   & \si\coulomb \\
...
\begin{frame}{More Than One Charge}
  When $N$ multiple discrete point charges are present, the total electric
  field at any position $\vec r$ is the vector sum of all the fields $\vec E_i$
  at that point:
  ...
  In general, this integral is difficult to evaluate for complex or irregular
  geometries, but in AP Physics C, we can usually exploit symmetry.
\end{frame}
...
\begin{frame}{Think Electric Field}
  ...
  the electric field is created\footnote{This is, of course, analogous to the
  relationship for gravity: $\vec F_g=m\vec g$}:

  \eq{-.1in}{
    \boxed{\vec F_q=q\vec E}
  }

  A positive charge in an electric field experiences an electrostatic force
...
\begin{frame}{Electric Field from Multiple Charges}
  ...
  \item Electric field lines to not tell us anything about the magnitude of the
    fields, although electric field lines that are closer together indicate
    stronger fields
  \end{itemize}
\end{frame}
...
\begin{frame}{Lord of the Ring Charge}
  Suppose you have been given \emph{The One Ring To Rule Them All}\footnote{This
  is a \emph{Lord of the Rings} reference, if you don't know already.}, and you
  found out that it is charged! What is the magnitude of its electric field at
  point $P$ along its axis, i.e.\ $E(x)$?
  \begin{center}
    \pic{.4}{physicsbook_emism_graphik_35}
  \end{center}
  Note that calculating the electric field away from the axis is \emph{very}
  difficult, and well beyond the scope of this course.
  \vspace{.2in}
\end{frame}
...
\begin{frame}{Sanity Check}
  Before we move on to the next example, we need to do a quick sanity check:
  from very far away, this ring will look like a point charge. Does the electric
  field approach $\dfrac{kQ}{x^2}$ as $x\to\infty$?
\end{frame}



\begin{frame}{Electric Field Along Axis of a Uniformly Charged Disk}
  Let's extend what we know to a disk of radius $a$ and a uniform surface
...
\begin{frame}{Eclectic Field Along Axis of a Uniformly Charged Disk}
  ...
  \end{columns}
  (Problems at this level of difficulty are commonplace for a university-level
  second course, but you are very unlikely to see in an AP Physics C exam.)
\end{frame}
...
\begin{frame}{Electric Potential Energy}
  Using the definition of mechanical work, the work done by $\vec F_q$ as it
  moves point charges $q_1$ and $q_2$ is expressed by:
  %is related to the change in  $U_q$:
  ...
  The work done by $\vec F_q$ is equal to the negative change of a quantity
  $U_q$ called the \textbf{electric potential energy}. For a system of two
  charges, it is defined as:
...
\begin{frame}{Electric Potential Energy}
  The existence of the electric potential energy means that the electrostatic
  force $\vec F_q$ is a conservative force, with properties that are common to
  all conservative forces

  \begin{enumerate}
  \item Work done by the electrostatic force is equal to the negative change in
    ...
    %\begin{minipage}{.9\linewidth}
    \begin{itemize}
    \item\vspace{-.1in}Positive work (i.e.\ $W_q>0$)
      decreases potential energy (i.e.\ $\Delta U_q<0$), while
    \item Negative work (i.e.\ $W_q<0$) increases potential
      energy (i.e.\ $\Delta U_q>0$)
    \end{itemize}
  \item Work done is \textbf{path independent}: $W_q$ depends on initial and
    final positions ($r_0$ and $r_1$), but not \emph{how} the charge
    moves from $r_0\rightarrow r_1$
  \item Only work done by electric force $\vec F_q$ can change the electric
    potential energy $U_q$
  \end{enumerate}
\end{frame}
...
\begin{frame}{How it Differs from Gravitational Potential Energy}
  ...
  \end{columns}
  \begin{itemize}
  \item $U_q>0$ (both charges have the same sign): positive external work is
    done to bring two charges together from $r=\infty$ to $r$
  \item $U_q<0$ (the charges are opposite signs): positive external work is
    done to separate the two charges from $r$ to $r=\infty$
...
\begin{frame}{Example Problem}
  \textbf{Example:} Four identical charges $q$\footnote{They are either all
  positive, or all negative. But once we have solved the answer, we should be
  able to tell that the sign does not matter} are located at the four corners
  of a square of length $L$. What is the total potential energy of the system?
  \begin{center}
    \begin{tikzpicture}[scale=1.3]
      \draw rectangle (2,2) node[midway,above=50]{$L$};
      \shade[ball color=blue] (0,2) circle (.15) node[white]{$q$};
      \shade[ball color=blue] (0,0) circle (.15) node[white]{$q$};
      \shade[ball color=blue] (2,0) circle (.15) node[white]{$q$};
      \shade[ball color=blue] (2,2) circle (.15) node[white]{$q$};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Electric Potential: Using Gravity as Example}
  ...
  This ``constant''\footnote{This ``constant'' depends on where the object is,
  i.e.\ this is really a \emph{function} of position} $V_g$ is called the
  \textbf{gravitational potential}, defined as
  \emph{gravitational potential energy per unit mass}. In the simple case with
...
\begin{frame}{Electric Potential: Point Charges}
  \begin{columns}
    ...
    \column{.6\textwidth}
    For a source point charge $q$, every point at a distance $r$ has the same
...
\begin{frame}{Electric Potential: Example}
  \begin{columns}
    ...
    \column{.65\textwidth}
    \textbf{Example:} A uniform charge $Q$ is distributed along a semi-circle
    of radius $R$, with the centre at the origin.
    \begin{enumerate}[(a)]
...
\begin{frame}{Relating $V$ to $\vec E$}
  ...
  \eq{-.1in}{
    V(r)=-\int\vec E(r)\cdot\dl\vec r\quad\rightarrow\quad
    \vec E(r)=-\nabla V(r)=-\diff Vr\hat r
  }
  \begin{itemize}
...
\begin{frame}{Relating $V$ and $\Delta V$}
  ...
  \eq{-.1in}{
    V{\color{red}(r)}=-\int\vec E(\vec r)\cdot\dl\vec r
  }
  ...
  \eq{-.1in}{
    {\color{blue}\Delta} V
    =-\int_{\vec r_0}^{\vec r_1}
    \vec E(\vec r)\cdot\dl\vec r
  }
\end{frame}
