...
\begin{questions}
  ...
  \begin{center}
    \begin{tikzpicture}
      \draw[thick,dashed,fill=gray!20] circle (1);
      \node at (1.1,-.6) {X};
      \draw[thick] (0,2.5) circle (.1) node[above=3]{$e^-$};
  ...
  \begin{choices}
    ...
    \choice The direction cannot be determined
  \end{choices}
  
  \question Two long parallel wires carry currents ($I_A$ and $I_B$), as shown
  in the figure. Current $I_A$ in the left wire is twice that of $I_B$ in the
  right wire. The magnetic force on the right wire is $F$. What is the
  magnetic force on the left wire in terms of $F$?
  \begin{center}
    \begin{tikzpicture}[scale=1.1]
      \foreach \x in {0,1} \draw[thick] (\x,0) rectangle +(.15,3);
      \draw[vectors] (.075,2)--(.075,3) node[above]{\small$I_A$};
      \draw[vectors] (1.075,2)--(1.075,3) node[above]{\small$I_B$};
    \end{tikzpicture}
  \end{center}
  \begin{choices}
    ...
    \choice $2F$ in the opposite direction
  \end{choices}
  \newpage
...
%    objects are placed in a strong, external magnetic field directed towards the
%    top of the page?
%    \begin{center}
%      \pic{.15}{copper-iron}
%
...
%    \end{center}
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=.85,thick]
      \foreach \x in {0,2} \draw (\x,0) rectangle +(.1,4);
      \foreach \x in {.3,.6,...,2}{
        \foreach \y in {.2,.5,...,4} \node at (\x,\y) {$\times$};
      }
      \draw[vectors] (1.05,-.5)--(1.05,4.5);
      \draw[very thick] (1.05,-.7) circle (.2) node{\small$p$};
    \end{tikzpicture}
  }
  \question A magnetic field, directed into the page, is placed between two
  charged capacitor plates, as shown in the figure above. The magnetic and
  electric fields are adjusted so a proton moving at a velocity of $v$ will pass
  ...
  acting on the proton when travelling at $2v$?
  ...
  \question Which of the following is true concerning the direction of the
  force on the current-carrying wire due to the electron?
  \begin{center}
    \begin{tikzpicture}
      \draw[vectors] (0,4)--(0,2.5);
      \draw[very thick] (0,3)--(0,0);
      \fill[thick] (-.5,2) circle (.1) node[left=3]{$e^-$};
      \draw[vectors] (-.5,2)--(-.5,1) node[below]{$v$};
    \end{tikzpicture}
  \end{center}
  \begin{choices}
    ...
    \choice There is not enough information to determine the direction of the
    force.
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      ...
      \draw[vectors,rotate=-35] (-.75,0) arc (180:-190:.75) node[above]{$I$};
      \node at (1.3,1) {$B$};
    \end{tikzpicture}
  }
  ...
  \begin{choices}
    ...
    \choice The wire loop will flip on a horizontal axis through its centre.
...
%    to each other. The two currents flow in the same direction.
%  }
%
  ...
  \uplevel{
    \begin{center}
      \begin{tikzpicture}[scale=1.3]
        ...
        \draw[vectors] (0,0)--(1.5,0) node[below]{$4A$};
        \draw[very thick] (0,0)--(0,2.5);
        \draw[vectors] (0,0)--(0,1.75) node[left]{$2A$};
        \draw[thick,dashed] (0,2)--(1,2) node[midway,above]{$r$}
        --(1,0) node[midway,right]{$2r$};
        \fill (1,2) circle (.05) node[right]{$P$};
      \end{tikzpicture}
    \end{center}
    \textbf{Questions \ref{q:2wires1}--\ref{q:2wires2}}: Two wires carry
    currents $2A$ and $4A$ in the directions shown above. Point $P$ is a
...
\begin{questions}
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[american voltages,scale=1.2]
      ...
      \draw[line width=1mm] (-2,1)--(2,1) node[midway,above]{$\ell$};
      ...
      \begin{scope}[<-]
        \draw (4.7,0) to[in=180,out=-60] (5.5,-.3) node[right]{Cable};
        \draw (1.7,1) to[in=180,out=-80] (3,.85) node[right]{Rod};
        %Rod (mass $m$, resistance $R$)};
        \draw (1.5,2.5) to[in=270,out=90] (2.5,3) node[above]{
  ...
  Earth's gravitational pull is towards the bottom of the page. Express all
  ...
  \begin{parts}
    ...
    \part\textbf{Determine} the direction of current in the cable to exert an
    upward force on the rod. \textbf{Justify} your answer.
    %direction must there be a current in the cable to exert an
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=1.2]
      \foreach \y in {.1,1.8} \draw (0,\y) rectangle +(5,.1);
      \draw (-.75,.2) rectangle (-2.5,1.8) node[midway]{\small Radioactive};
      \foreach \x in {.5,1,...,4.5}{
        \foreach \y in {.5,1,1.5} \draw(\x,\y) circle (.06);
      }
      \draw[dashed] (0,1)--(7,1);
      \draw[thick,<->] (4.3,.2)--(4.3,1.8) node[pos=.7,left=-1]{$d$};
      \foreach \xx in {-.5,5.5}{
        \draw[vectors] (\xx,1)--+(1,0) node[midway,above]{$v$};
        \draw[thick,fill=gray!60] (\xx,1) circle (.1);
      }
      \foreach \y in {-.5,1.3} \draw (5.4,\y) rectangle +(.15,1.2);
    \end{tikzpicture}
  }
  ...
  figure above. Electrons that enter along the centre line of the capacitor
  plates travel straight through (i.e.\ undeflected) with a velocity of
  $v=\SI{1.0e7}{\metre\per\second}$ and out the hole in the centre of the
  ...
  \begin{parts}
    ...
    undeflected. \textbf{Support} your argument with an algebraic equation
    ...
    \vspace{.15in}
    \underline{\hspace{.4in}}Top plate\hspace{.4in}
    \underline{\hspace{.4in}}Bottom plate

    \vspace{.1in}\textbf{Justify} your reasoning making reference to the
    electric field.
    \vspace{\stretch1}
    \newpage
    ...
    \uplevel{
      A positron enters the apparatus along the same path as the electrons from
      part \textbf{B}.
    }
    
    \part\textbf{Explain} why the positron, travelling at the same speed as the
    electrons, will also travel straight through the device undeflected.
    \textbf{Support} your argument with an equation.
    \vspace{\stretch1}
      
    \part A second positron enters the apparatus at a speed of $2v$.
    \textbf{Sketch} the path of the positron through the capacitor plates on
    ...
    \uplevel{
      ...
      as shown in the figure below. The distance between the electron and the
      wire is \SI1{\milli\metre}.
      \begin{center}
        \begin{tikzpicture}[american voltages,scale=1.2]
          \draw[thick] (0,-.5)--(0,1)--(4,1)--(4,-.5)
          to[battery1=$\mathcal E$] (2,-.5) to[R=$R$] (0,-.5);
          \draw[vectors] (1.5,1.3)--+(1.2,0) node[midway,above]{$v$};
          \draw[thick,fill=lightgray] (1.5,1.3) circle (.1);
        \end{tikzpicture}
      \end{center}
    }
    ...
    \part\textbf{Draw} a force vector on the figure above to show the direction
...
%  \end{parts}
  ...
  \begin{parts}
    ...
    \uplevel{
      ...
      \begin{displaymath}
        B(r)=\frac{\mu_0I_0r}{2\pi b^2}
      \end{displaymath}
      An experiment is conducted using a particular solid cylinder of radius
      \SI{.010}{\metre} carrying a current of \SI{25}\ampere. The magnetic
      field inside the cylinder is measured as a function of $r$, and the data
      is tabulated below.
