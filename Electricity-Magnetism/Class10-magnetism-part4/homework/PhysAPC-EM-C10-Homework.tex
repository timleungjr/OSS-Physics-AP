...
\gentitle{10}{MAGNETISM, PART 3: FARADAY'S LAW}
...
\begin{questions}
  ...
  \begin{choices}
    ...
    \choice Moving the loop down towards the wire
  \end{choices}
  ...
  uniform magnetic field $\vec B$ directed into the page, as shown in the figure
  above. In which of the following cases, if any, will an \emph{emf} be induced
  in the loop at the moment shown in the figure?
  \begin{choices}
    \choice The loop is moving towards the right.
    \choice The loop is moving towards the top of the page.
    ...
    plane remains perpendicular to the magnetic field.
    \choice An \emph{emf} cannot be induced in the loop without changing its
  ...
  % TAKEN FROM THE 2023 AP PHYSICS C E&M EXAM SET 1, QUESTION 3
  \uplevel{
    \centering
  ...
  \begin{parts}
  \item On the following diagram of the bar, \textbf{draw} an arrow indicating
    the direction of the net force $F_\text{net}$ exerted on the bar at time
    $t_B$. If the net force is zero, write $F_\text{net}=0$.
    \begin{center}
      \vspace{.3in}
    ...
    \part\textbf{Calculate} the magnitude of the current in the bar at time
    $t_B$.
    \vspace{\stretch1}

    \part\textbf{Calculate} the magnitude of the net force $F_\text{net}$
    exerted on the bar at time $t_B$.
    \vspace{\stretch1}

    \part On the following axes, \textbf{sketch} a graph of the speed $v$ of
    the bar as a function of time $t$ between $t=0$ and $t_4$.
    \uplevel{
      \centering
      \begin{tikzpicture}
    ...
    \part\textbf{Determine} the total resistance $R_\text{total}$ of the closed
    circuit for the new scenario.
    \vspace{\stretch1}
    ...
    field is $a_\text{new}$. Indicate whether $a_\text{new}$ is greater than,
    less than, or equal to $a_\text{original}$.
