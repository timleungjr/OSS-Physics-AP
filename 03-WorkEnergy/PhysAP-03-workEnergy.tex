...
\begin{frame}{Work by Constant Force}
  ...
  Or in the scalar form that is more familiar in Grades 11/12 Physics courses
  that avoid vector notations:

  \eq{-.2in}{
    \boxed{W=F\Delta r\cos\theta}
  }

  \vspace{-.1in}($\theta$ is the angle between the force and displacement
  vectors)
\end{frame}
...
\begin{frame}
  ...
  $\mb{g}=-g\bm{\hat{\jmath}}=-10\bm{\hat{\jmath}}$ (in
  \si{\metre\per\second^2}) is a constant. The work done to move an object
...
\begin{frame}{Spring Force \& Elastic Potential Energy}
  ...
  When applied to the work equation, we can find the work done to  displace a
  spring:
