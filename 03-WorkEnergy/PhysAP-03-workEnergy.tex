...
\begin{frame}{Kinetic Energy}
  ...
  \vspace{-.2in}Since both $\mb{v}$ and $\mb{x}$ are continuous functions in
  time, we can switch the order of differentitation, i.e.:
  ...
  \vspace{-.15in}and since $\mb{v}$ and $d\mb{v}$ must be in the same
  direction, the dot product is trivial: $\mb{v}\cdot d\mb{v}=vdv$
\end{frame}
...
\begin{frame}{Kinetic Energy}
  ...
  \vspace{-.15in}where $K$ is defined as the
...
\begin{frame}
  ...
  \vspace{-.2in}For objects near the surface of Earth, we assume that
  $\mb{g}=-g\bm{\hat{\jmath}}=-10\bm{\hat{\jmath}}$
  (in \si{\metre\per\second^2}) is a constant. The work done to move an object
  ...
  \vspace{-.1in}where $U_g$ is the \textbf{gravitational potential energy}:
...
\begin{frame}{Spring Force \& Elastic Potential Energy}
  ...
  \vspace{-.2in}When applied to the work equation, we can find the work done to
  displace a spring:
  ...
  \vspace{-.2in}where $U_e=\frac{1}{2}kx^2$ is the
...
\begin{frame}{Efficiency}
  ...
  \eq{-.2in}{
    \boxed{\eta = \frac{E_o}{E_i}\times 100\%}\quad
    \boxed{\eta = \frac{W_o}{W_i}\times 100\%}
  }
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
    ...
    \end{tabular}
  \end{center}
  Efficiency is always $0 \leq\eta\leq 100\%$
\end{frame}
