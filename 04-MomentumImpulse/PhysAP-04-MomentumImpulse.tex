...
\begin{frame}{Linear Momentum}
  ...
  \textbf{momentum}) is the ``quantity of motion'' that is proportional to an
  object's \emph{mass} and its \emph{velocity}.
...
\begin{frame}{Newton's Second Law}
  Starting with the familiar form of Newton's second law of motion with constant
  $m$, we can find out how change in momentum $\mb{p}$ is related to net
  force $\mb{F}_{\textrm{net}}$:
 
  \eq{-.1in}{
    \mb{F}_{\textrm{net}}=m\mb{a}=m\frac{d\mb{v}}{dt}=\frac{d(m\mb{v})}{dt}
    =\frac{d\mb{p}}{dt}
  }

  In fact, this is the \emph{general} form of Newton's second law of motion that
  accounts for the effects of changing mass. \textbf{The net external
    force on an object is the rate of change of its momentum.}

  \eq{-.2in}{
    \boxed{\mb{F}_{\textrm{net}}=\frac{d\mb{p}}{dt}}
  }
  %$\mb{F}_{\textrm{net}}=m\mb{a}$ is just a special case
\end{frame}
...
\begin{frame}{Second Law of Motion \& Conservation of Momentum}
  ...
  \item In the absence of external forces,  we have
    ...
    \eq{-.25in}{
      \sum_i\mb{p}_i=\sum_i\mb{p}_i'
    }
    
    We have previously looked at internal forces when solving connected-bodies
    problems.
  \end{itemize} 
\end{frame}
...
\begin{frame}{Impulse}
  Rearranging the variables in the general form of Newton's second law:
  
  \eq{-.15in}{
    \mb{F}_{\textrm{net}}=\frac{d\mb{p}}{dt}\;\rightarrow\;
    \mb{F}_{\textrm{net}}dt=d\mb{p}
  }
  ...
  \eq{-.15in}{
    \boxed{
      \mb{J}_{\textrm{net}}
      =\int_{t_1}^{t_2}\mb{F}_{\textrm{net}}dt
