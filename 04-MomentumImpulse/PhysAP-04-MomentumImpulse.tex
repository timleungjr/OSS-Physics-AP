...
\begin{frame}{Linear Momentum}
  \textbf{Linear momentum} is proportional to the object's \emph{mass} and
  its \emph{velocity}. This is the ``quantity of motion'' referred to by
  Newton.
...
\begin{frame}{Newton's Second Law}
  ...
  \vspace{-.15in}In fact, this is the general form of Newtons second law of
  motion:
  \textbf{net external force on an object is the rate of change of its momentum}

  \eq{-.25in}{
    \boxed{\mb{F}_{\textrm{net}}=\frac{d\mb{p}}{dt}}
  }

  \vspace{-.15in}$\mb{F}_{\textrm{net}}=m\mb{a}$ is just a special case
\end{frame}
...
\begin{frame}{Impulse}
  ...
  \vspace{-.15in}We can integrate both sides to get the
...
\begin{frame}{Impulse}
  ...
  \vspace{-.15in}\textbf{Average force} is a ``force'' that gets the same
  impulse:
  ...
  \vspace{-.15in}Note that impulse from each individual force does not depend
  on whether the object moves. The change in momentum only depends on
  \emph{net} impulse
\end{frame}
