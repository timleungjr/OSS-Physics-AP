...
%  \end{minipage}
  ...
  \begin{choices}
    ...
    \choice $mgR\cos\theta$
    \choice $mgR\sin\theta$
    \choice $mgR(1-\cos\theta)$
    \choice $mgR(1-\sin\theta)$
  \end{choices}
  ...
  \begin{minipage}{.68\linewidth}
    \begin{choices}
      \choice$gR$
      \choice$2gR$
      \choice$\sqrt{gR\sin\ang{45}}$
      \choice$\sqrt{2gR\sin\ang{45}}$
  ...
  \question A roller coaster ride at an amusement park lifts a car of mass
  \SI{700}{\kilo\gram} to point $A$ at a height of \SI{90}{\metre} above the
  lowest point on the track, as shown above. The car starts from rest at point
  $A$, rolls with negligible friction down the incline and follows the track
  around a loop of radius \SI{20}{\metre}. Point $B$, the highest point on the
  loop, is at a height of \SI{50}{\metre} above the lowest point on the track.
  \begin{parts}
    \part
    \begin{subparts}
      \subpart Indicate on the figure the point $P$ at which the maximum speed
      of the car is attained.
      \subpart Calculate the value $\varv_\text{max}$ of this maximum speed.
    \end{subparts}
    \part Calculate the speed $\varv_B$ of the car at point $B$.
    \part
    \begin{subparts}
      \subpart On the figure of the car below, draw and label vectors to
      represent the forces acting on the car when it is upside down at point
      $B$.
      ...
      \begin{center}
        \begin{tikzpicture}[scale=.25]
          ...
          \draw[very thick](.2,1) arc(180:0:.4);
          \draw[very thick](3.8,1) arc(0:180:.4);
        \end{tikzpicture}
      \end{center}
      \vspace{.5in}
      
      \subpart Calculate the magnitude of all the forces identified in (c)i.
      \end{subparts}
    \part Now suppose that friction is not negligible. How could the loop be
    modified to maintain the same speed at the top of the loop as found in
    ...
    \begin{tikzpicture}[scale=.7]
      \begin{scope}[rotate=-30]
        ...
        \draw[thick,
          ...
          decorate] (-.1,.5)--(-1.5,.5) node[midway,above right]{$k$};
        \fill(-1.5,0) rectangle(-1.6,1);
        \draw[|<->|,thick](-1.6,1.3)--(-5.6,1.3)
        node[midway,above right,sloped]{\SI4{\metre}};
        \draw[fill=yellow!50](-5.6,0) rectangle(-6.3,.7)node[black,midway]{$m$};
      \end{scope}
      \draw[fill=pink!50](0,0)--(-6.06,0)--(-6.06,3.5)--cycle;
      \draw[thick](-2,0) arc(180:150:2) node[midway,left]{$\theta$};
    \end{tikzpicture}
  }
  \question A block of mass $m=\SI2{\kilo\gram}$ is released \SI4{\metre}
  from a massless spring with a spring constant of
  $k=\SI{100}{\newton\per\metre}$ that is fixed along a frictionless plane
  inclined at $\theta=\ang{30}$, as shown in the figure above. (Plese give
