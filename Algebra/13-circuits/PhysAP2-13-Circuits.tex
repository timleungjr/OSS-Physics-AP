...
%      $\mb{E}$ & \si{\newton/\coulomb} \\
%      Density of charge carrier & $\mb{J}$ & \si{\ampere\per\metre\squared} \\
%      Resistivity & $\rho$ & \si{\ohm/\metre}
%    \end{tabular}
%  \end{center}
%\end{frame}
...
\begin{frame}{Resistors in Series}
  ...
  \eq{-.2in}{V=V_1+V_2+V_3+\cdots=I(\underbrace{R_1+R_2+R_3+\cdots}_{R_s})}
\end{frame}
...
%  and the capacitor is already charged to $V_c=Q_{tot}/C$. What happens
...
\begin{frame}{Charging a Capacitor}
  ...
  \vspace{.1in}\begin{columns}
    ...
    resistor. At $t=\infty$, current through the capcitor is $I_c=0$
  \end{columns}
\end{frame}
...
%
%  \eq{-.15in}{\boxed{Q(t)=Q_{tot}(1-e^{-t/\tau})}}
%
