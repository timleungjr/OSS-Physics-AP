...
\begin{frame}{Angular Position and Angular Velocity}
  ...
  \begin{columns}
    ...
    describes an object's position. It is generally measured in radians (rad):
    ...
    It is measured in \si{rad\per\second}
...
\begin{frame}{Angular Acceleration}
  ...
  \textbf{average tangential acceleration} $a_\theta$ is related to average
  angular acceleratio $\overline{\alpha}$ by the radius $r$:
    
  \eq{-.3in}{
    \boxed{\overline{a}_\theta
...
\begin{frame}{Banked Curves on Highways and Racetracks}
  \begin{columns}
    ...
    Note that $v_{\textrm{max}}$ does not depend on mass.
  \end{columns}
\end{frame}
