...
\begin{frame}{Angular Position and Angular Velocity}
  ...
  \begin{columns}
    ...
    $\theta$ can fully describes an object's position. It is generally measured
    in radians (rad):
...
\begin{frame}{Period \& Frequency}
  \begin{columns}
    ...
    periodic, with its \textbf{frequency} $f$ and \textbf{period} $T$ given by:
    ...
    (\si{\hertz}). Period and frequency are reciprocals of each other.
  \end{columns}
\end{frame}
...
\begin{frame}{Centripetal Acceleration for Uniform Circular Motion}
  In uniform circular motion ($\alpha=0$, constant $\omega$) problems where the
  period or frequency are known, the speed of the object is:
...
\begin{frame}{Simple Pendulum}
  \begin{columns}
    ...
    therefore, there is a tangential acceleration along that direction, with a
    magnitude of:
