...
\begin{frame}{Files to Download}
  ...
  already done so:
  \begin{itemize}
  \item\texttt{PhysAP1-05-circMotion-print.pdf}---The ``print version'' of the
...
\begin{frame}{Angular Position and Angular Velocity}
  ...
  \begin{columns}
    ...
    \column{.68\textwidth}
    For constant radius $r$, the \textbf{angular position} $\theta$ fully
    ...
    \textbf{Average angular velocity} $\omega$ (or \textbf{angular frequency})
    is the rate of change in angular position over a finite time interval:
      
    \eq{-.2in}{
      \boxed{\omega(t)=\frac{\Delta\theta}{\Delta t}}
    }

    $\omega$ is measured in \si{rad\per\second}
  \end{columns}
\end{frame}
...
\begin{frame}{Period \& Frequency}
  \begin{columns}
    ...
    \column{.7\textwidth}
    For constant $\omega$ (uniform circular motion), the motion is periodic,
    with its \textbf{frequency} $f$ and \textbf{period} $T$ given by:
    ...
    $T$ is measured in \textbf{seconds} (\si{\s}) and $f$ in \textbf{hertz}
    (\si{\hertz})
  \end{columns}
\end{frame}
...
\begin{frame}{Angular Acceleration}
  The change in $\omega$ over a finite time $\Delta t$ is
  \textbf{average angular acceleration}, which has a unit of \si{rad/\second^2}:

  \eq{-.2in}{
    \boxed{\bar{\alpha}=\frac{\Delta\omega}{\Delta t}}
  }
  ...
  \eq{-.2in}{
    \bar{a}_\theta=
    \frac{\Delta v}{\Delta t}=\frac{r\Delta\omega}{\Delta t}=r\bar{\alpha}
  }
    
  For \emph{uniform} circular motion, $\omega$ is constant, therefore
  $\alpha_\theta=0$
\end{frame}
...
\begin{frame}{Kinematics in the Angular Direction}
  ...
  like in rectilinear motion, but with the $\theta$ replaces $x$, $\omega$
  ...
  If $\alpha$ is \emph{not} constant, calculus will be required.
\end{frame}
...
\begin{frame}{Centripetal Acceleration \& Centripetal Force}
  From Grade 12 Physics, you should know that there is also a component of
  acceleration toward the center of the rotation, called the
  \textbf{centripetal acceleration} $a_r$:
  ...
  \eq{-.15in}{
    \boxed{\mb{F}_r=ma_r=-\frac{mv^2}{r}}
  }
\end{frame}
...
\begin{frame}{Centripetal Acceleration for Uniform Circular Motion}
  ...
  \eq{-.2in}{
    a_r=-\frac{v^2}{r^2}\quad\rightarrow\quad
    \boxed{
      a_r=\frac{4\pi^2r}{T^2}=4\pi^2rf^2
    }
  }
\end{frame}
...
\begin{frame}{What About a Pendulum?}
  ...
  \vspace{.1in}\begin{columns}
    ...
    \item Speed of the pendulum at any height is found using conservation
      of energy
      \begin{itemize}
      \item Tension $\mb{T}$ is always $\perp$ to motion, therefore it does not
        do any work
...
\begin{frame}{Simple Pendulum}
  \begin{columns}
    ...
    \column{.5\textwidth}
    In the radial direction , there is still a net force of $mg\sin\theta$,
    therefore, there is a tengential acceleration along that direction, with a
    magnitude of:
