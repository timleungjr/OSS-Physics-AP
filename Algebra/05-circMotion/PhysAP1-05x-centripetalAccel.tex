  ...
  \begin{columns}
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=30]
        \draw[->,very thick,blue](2.5,0)--(2.5,1.5) node[above]{$\mb{v}_i$};
        \draw[->,very thick,red] (0,0)--(2.44,0)node[pos=.6,below]{$\mb{r}_i$};
        \fill (2.5,0) circle(.06);
      \end{scope}
      \begin{scope}[rotate=90]
        \draw[->,very thick,blue] (2.5,0)--(2.5,1.5)node[left]{$\mb{v}_f$};
        \draw[->,very thick,red] (0,0)--(2.44,0) node[midway,left]{$\mb{r}_f$};
        \fill[gray] (2.5,0) circle(.06);
      \end{scope}
      \draw(0,1)[<->] arc(90:30:1) node[pos=.6,above]{$\Delta\theta$};
    \end{tikzpicture}
    
    \column{.6\textwidth}
    Consider an object moving in circular motion in the counter clockwise
    direction with constant radius $r$ and constant speed $v$.
    \begin{itemize}
    \item At initial time $t_i=0$, the position and velocity of the object are
      given by $\mb{r}_i$ and $\mb{v}_i$
    \item At $t_f=t+\Delta t$, the object has moved by an angular displacement
      of $\Delta\theta$, and the final position and velocity are given by
      $\mb{r}_f$ and $\mb{v}_f$
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Centripetal Acceleration}
  \begin{columns}
    ...
    \eq{-.2in}{
      \mb{a}=\frac{\Delta\mb{v}}{\Delta t}=\frac{\mb{v}_f-\mb{v}_i}{\Delta t}
    }

    And the magnitude of the acceleration is
    
    \eq{-.2in}{
      |\mb{a}|=\frac{|\Delta\mb{v}|}{\Delta t}
    }

    Both are important in deriving the expression for centripetal acceleration.
  \end{columns}
\end{frame}



\begin{frame}{Displacement and Change in Velocity}
  \begin{columns}
    \column{.3\textwidth}
    ...
    \column{.7\textwidth}
    Note that the triangles formed by the displacement vector and the change in
    velocity are similar isosceles triangles:
    \begin{center}
      \begin{tikzpicture}[scale=1.2]
        \draw[->,very thick,rotate=-60,red!60](0,0)--(0,2)
        node[midway,below]{$r$};
        \draw[->,very thick,red!60](0,0)--(0,2)
        node[midway,left]{$r$};
        \draw[<-,very thick,rotate around={-30:(0,2)},red!70!black](0,2)--(2,2)
        node[pos=.6,above]{$\Delta\mb{r}$};
      \end{tikzpicture}
      \begin{tikzpicture}[scale=1.2]
        \draw[->,very thick,rotate=-60,blue!60](0,0)--(-2,0)
        node[midway,right]{$v$};
        \draw[->,very thick,blue!60](0,0)--(-2,0) node[midway,below]{$v$};
%        \draw[<-,very thick,rotate around={-30:(0,2)},red!70!black](0,2)--(2,2)
%        node[pos=.6,above]{$\Delta\mb{r}$};
      \end{tikzpicture}
    \end{center}
