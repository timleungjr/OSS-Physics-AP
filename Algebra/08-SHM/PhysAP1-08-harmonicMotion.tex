...
%  \item Since acceleration related to springs is not constant, we cannot rely
%    on kinematic equations, so we often have to use conservation of energy
%    instead
\end{frame}
...
\begin{frame}{Frequency and Period of Mass on a Spring}
  ...
  Angular frequency $\omega$, frequency $f$ and period $T$ are all independent
  of amplitude $A$
\end{frame}
...
\begin{frame}{Damped Oscillator}
  The solution this equation is a standard (but difficult) problem in calculus.
  The motion of the mass has both an exponential decay and a sinusoidal term:

  \eq{-.2in}{
   x(t)=A_0e^{-\frac{b}{2m}t}\cos(\omega't+\phi)
  }

  \vspace{-.15in}where $A_0$ is the initial amplitude, and angular frequency
  $\omega'$ is now:
  ...
  \vspace{-.1in}Angular frequency $\omega'$ of the damped oscillator differs
...
%  
  ...
  This is a difficult problem (even for calculus students), but its solution
