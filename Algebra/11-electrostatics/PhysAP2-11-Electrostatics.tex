...
\begin{frame}{Coulomb's Law for Electrostatic Force}
  \begin{center}
    \begin{tikzpicture}[scale=.65]
      \draw[->,very thick,red](0,0)--(2,0)  node[right]{$\mb{F}_{21}$};
      \draw[->,very thick,blue](8,0)--(6,0) node[left]{$\mb{F}_{12}$};
...
\begin{frame}{Coulomb's Law for Electrostatic Force}
  \begin{center}
    \begin{tikzpicture}[scale=.5]
      \draw[->,very thick,red](0,0)--(2,0)  node[right]{$\mb{F}_{12}$};
      \draw[->,very thick,blue](8,0)--(6,0) node[left]{$\mb{F}_{21}$};
      \tikzstyle{balloon1}=[ball color=red];
      \tikzstyle{balloon2}=[ball color=blue];
      \shade[balloon1] (0,0) circle(.2) node[above]{$q_1$};
      \shade[balloon2] (8,0) circle(.2) node[above]{$q_2$};
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.4]
      ...
      \begin{scope}[rotate=45]
        \draw[->,very thick,blue](.72,0)--(2.5,0) node[right]{$\mb{F}_1$};
      ...
      \uncover<5>{
        \begin{scope}[rotate=125]
          \draw[->,ultra thick](.72,0)--(3.5,0) node[left]{$\mb{F}$};
        \end{scope}
      }
    \end{tikzpicture}
...
%      node[above]{\footnotesize$\hat{\mb{n}}$};
...
\begin{frame}{Electrical Potential Energy}
  ...
  electrostatic force is given by this integral:
  ...
  And if you don't know calculus, all you need to know is the result: $Q_q$ is
...
\begin{frame}{How it Differs from Gravitational Potential Energy}
  ...
  \vspace{.2in}
  \begin{itemize}
  \item $U_q>0$: positive work is done to bring two charges together from
   $r=\infty$ to $r$ (both charges of the same sign)
  \item $U_q<0$: work is done to pull the objects from $r$ to  $r=\infty$
  \item In comparison, gravitational potential $U_g$ is always $<0$
  \end{itemize}
\end{frame}
...
\begin{frame}{Electric Potential}
  \textbf{Using gravity as an example:} An object at a specific location inside
  a gravitational field has a gravitational potential energy that is
  ...
  \vspace{-.1in}This ``constant'' $V_g$ is called the \textbf{gravitational
    ...
    mass}. In the trivial case with a uniform $\mb{g}$:

  \eq{-.2in}{
    V_g=\frac{U_g}m=g\Delta h
  }
...
\begin{frame}{Electric Potential}
  This is also true for charged particle $q$ in an electric electric field
  created by $q_s$, and the ``constant'' is called the
  ...
  The unit for electric potential is a \emph{volt}\footnote{Named after Italian
    physicist Alessandro Volta} which is \emph{one joule per coulomb}:

  \eq{-.25in}{
    \SI1{\volt}=\SI1{\joule\per\coulomb}
  }

  \vspace{-.2in}The relationship between  $V$ and $\mb{E}$ is given by:

  \eq{-.2in}{
    \boxed{
      |\mb{E}|\approx\frac{\Delta V}{\Delta r}
    }
  }
