...
\begin{questions}
  ...
  \question A diver descends from a salvage ship to the ocean floor at a depth
  of \SI{35}{\metre} below the surface. The density of ocean water is
  \SI{1.025e3}{\kilo\gram\per\metre\cubed}.
  \begin{parts}
    \part Calculate the gauge pressure on the diver on the ocean floor.
    \part Calculate the absolute pressure on the diver on the ocean floor.
    \uplevel{
      The diver finds a rectangular aluminum plate having dimensions
      $\SI{1.}\meter\times\SI{2.}\metre\times\SI{.03}{\metre}$. A hoisting
      cable is lowered from the ship and the diver connects it to the plate.
      The density of aluminum is \SI{2.7e3}{\kilo\gram\per\metre\cubed}. Ignore
      the effects of viscosity.
    }
    \part Calculate the tension in the cable if it lifts the plate upward at a
    slow, constant velocity.

    \part Will the tension in the hoisting cable increase, decrease, or remain
    the same if the plate accelerates upward at
    \SI{.05}{\metre\per\second\squared}? Explain your reasoning.
  \end{parts}
  \newpage
