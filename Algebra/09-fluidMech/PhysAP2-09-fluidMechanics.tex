...
\begin{frame}{Density of a Fluid}
  ...
  $m_\mathrm{fluid}$ per unit volume $V_\mathrm{fluid}$ that it occupies:
  
  \eq{-.2in}{
    \boxed{\rho=\frac{m_\mathrm{fluid}}{V_\mathrm{fluid}}}
  }
  \begin{itemize}
  \item Unit for density is \textbf{kilograms per meter cubed}
    (\si{\kilo\gram\per\metre^3})
...
\begin{frame}
  ...
  \eq{-.2in}{ \boxed{p=p_0+\rho_\mathrm{fluid}gz} }
...
\begin{frame}{Buoyant Force}
  Using some basic calculus (well, depends on who you ask), we can find the
  pressure over the entire surface to find the total buoyant force $\mb{B}$ the
...
\begin{frame}{Example: Multiple Inlet \& Outlets}
  \begin{columns}
    ...
    $D_1=\SI{6}{\centi\metre}$ and the volume flow is \SI{100}{\metre^3/h}. At
    section 2, $D_2=\SI{5}{\centi\metre}$ and the average velocity is
    \SI{8}{\metre\per\second}. If $D_3=\SI{4}{\centi\metre}$, what is
    \begin{enumerate}
    \item the flow rate $Q_3$ in \si{\metre^3/h}?
