...
\begin{frame}{Momentum}
  ...
  \textbf{momentum}) is a quantity of motion simply defined as the product of
  mass $m$ and velocity $\mb{v}$:
...
%  \end{center}
  \begin{itemize}
  \item  The unit of momentum is \textbf{kilogram meter per second}
    (\si{\kilo\gram\metre\per\second})
  \item Momentum is a \emph{vector} quantity; direction of $\mb{p}$ is the same
    as $\mb{v}$
  \item For rotational motion of rigid bodies, there is also
    \textbf{angular momentum} which will be studied in a later topic
  \end{itemize}
\end{frame}
...
\begin{frame}{General Form of Second Law of Motion}
  ...
  \eq{-.15in}{
    \frac{d\mb{p}}{dt}=\frac{d(m\mb{v})}{dt}
    =m\frac{d\mb{v}}{dt}+\frac{dm}{dt}\mb{v}
    =m\mb{a}+\dot{m}\mb{v}
  }

  For constant mass $m$ (i.e.\ $\dot{m}=0$), this right-hand-side reduces to the
  familiar form of the second law of motion, $m\mb{a}$. In fact, the
  ...
  \eq{-.2in}{
    \mb{F}_{\textrm{net}}=\frac{d\mb{p}}{dt}
  }
\end{frame}



\begin{frame}{Second Law of Motion \& Conservation of Momentum}

  \eq{-.2in}{
    \boxed{\mb{F}_{\textrm{net}}=\frac{d\mb{p}}{dt}}
  }
  \begin{itemize}
  \item Momentum is conserved (i.e.\ $\sum\mb{p}$ constant) when the net
  ...
  \item We have previously studied internal forces when solving 
...
\begin{frame}{Impulse}
  ...
  \eq{-.15in}{
    \bar{\mb{F}}_{\text{net}}=\frac{\Delta\mb{p}}{\Delta t}\;\rightarrow\;
    \boxed{\mb{J}_{\text{net}}=\bar{\mb{F}}_{\text{net}}\Delta t=\Delta\mb{p}}
  }
...
\begin{frame}{Impulse}
  Any force $\bar{\mb{F}}$ acting on an object over a time $\Delta t$ generates
  an impulse, regardless of whether or not there is a change in momentum:

  \eq{-.2in}{
    \boxed{\mb{J}=\bar{\mb{F}}\Delta t}
  }
  
  The change in momentum only depends on \emph{net} impulse $\mb{J}_{\text{net}}$.
  The force is an \textbf{average force} $\bar{\mb{F}}$ is the
  time-averaged vector that generates the same impulse. It is used extensively
  in introductory physics courses to avoid calculus
