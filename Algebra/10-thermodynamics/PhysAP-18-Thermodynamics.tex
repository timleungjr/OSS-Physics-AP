...
\begin{frame}{Kinetic Theory of Gases}
  ...
  \item The molecules are separated, on average, by distances that are large
    ...
    electrostatic etc) on each other except when they collide.\footnote{This
      means that the density of the gas is low, i.e.\ assume ideal gas
      behavior.}
...
\begin{frame}{Thermodynamic Temperature}
  ...
  heated/cooled at \emph{constant volume}, the relationship between pressure and
  temperatuer is \emph{linear}:
  \begin{center}
    \begin{tikzpicture}
  ...
  no temperature can exist below that value.
\end{frame}
...
\begin{frame}{Thermodynamic Temperature}
  The \textbf{thermodynamic temperature} (or \textbf{absolute temperature}) $T$
  is obtained by shifting the ``null point'' (where temperature is zero) from
  the Celsius temperature $T_C$. The unit for thermodynamic is a
  \textbf{kelvin} (\si{\kelvin}). It is one of the 6 base units of the SI
  system.
...
%  \end{center}
  ...
  \item\vspace{-.1in}A temperature of \SI{0}{\kelvin} is called
    \textbf{absolute zero}, but it is impossible to obtain because of
    quantum-mechanic effects
  \item This temperature scale is more consistent with the physical and
...
\begin{frame}{Ideal Gas Law for Low-Density Gases}
  Physicist Robert Boyle (1627-1691) discovered that, when a gas is allowed to
  expand or compress at \emph{constant temperature}, the product of pressure
  $P$ and $V$ remain constant (\textbf{Boyle's law}), i.e.:

  \eq{-.2in}{
    PV=\text{constant} %\quad\quad\text{\normalsize constant temperature}
  }

  \vspace{-.1in}From the previous discussion on temperature scale, we also know
  that at \emph{constant volume}, (thermdynamic) temperature is proportional to
...
\begin{frame}{Ideal Gas Law}
  The ideal gas law is often expressed in a slightly different form in
  chemistry courses:  
...
\begin{frame}{First Law of Thermodynamics}
  ...
  molecules. It is a linear function of temperature.

  \eq{-.2in}{
    \boxed{\Delta\textcolor{red}{U}=Q-W}
  }

  \vspace{-.1in}For monatomic gases (including an ideal gas): there are 3
  degrees of translational freedom, therefore the internal energy is
...
\begin{frame}{First Law of Thermodynamics}
  \textcolor{green!80!black}{$W$} is the mechanical work\footnote{The full
    ...
    $W=\int\mb{F}\cdot d\mb{x}$, i.e.\ force times distance.} done \emph{by}
...
\begin{frame}{Work Done By a Gas}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The fact that $W$ is \emph{path dependent} means that the pressure force is
  non-conservative.
\end{frame}
