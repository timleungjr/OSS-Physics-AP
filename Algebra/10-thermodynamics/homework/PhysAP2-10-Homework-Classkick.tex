...
%  \end{enumerate}
  ...
  \question A cylinder with a movable piston contains 0.1 mole of a monatomic
  ideal gas. The gas, initially at state a, can be taken through either of two
  cycles, abca or abcda, as shown on the PV diagram above. The following
  information is known about this system.
  \begin{align*}
    Q_{c\rightarrow a} &= \SI{685}{\joule}\text{ along the curved path}\\
    W_{c\rightarrow a} &= \SI{-120}{\joule}\text{ along the curved path}\\
    U_a-U_b &= \SI{450}{\joule}\\
    W_{a\rightarrow b\rightarrow c} &= \SI{75}{\joule}
  \end{align*}
  \begin{parts}
    \part Determine the change in internal energy, $U_a - U_c$ , between states
    $a$ and $c$.
    \part
    \begin{subparts}
      \subpart Is heat added to or removed from the gas when the gas is taken
      along the path $abc$?
      \subpart Calculate the amount added or removed.
    \end{subparts}
    \part How much work is done on the gas in the process $cda$?
    \part Is heat added to or removed from the gas when the gas is taken along
    the path $cda$? Explain your reasoning.
  \end{parts}
  \newpage
  ...
  \question Two moles of a monatomic ideal gas are enclosed in a cylinder by a
  movable piston. The gas is taken through the thermodynamic cycle shown in the
  figure above. The piston has a cross-sectional area of
  \SI{5e3}{\metre\squared}.
  \begin{parts}
    \part
    \begin{subparts}
      \subpart Calculate the force that the gas exerts on the piston in state
      $A$, and explain how the collisions of the gas atoms with the piston
      allow the gas to exert a force on the piston.

      \subpart Calculate the temperature of the gas in state $B$, and indicate
      the microscopic property of the gas that is characterized by the
      temperature.
    \end{subparts}
    \part
    \begin{subparts}
      \subpart Predict qualitatively how the internal energy of the gas changes
      as it is taken from state $A$ to state $B$. Justify your prediction.

      \subpart Calculate the energy added to the gas by heating as it is taken
      from state $A$ to state $C$ along the path $ABC$.
    \end{subparts}

    \part Determine the change in the total kinetic energy of the gas atoms as
    the gas is taken directly from state $C$ to state $A$.
  \end{parts}
