    ...
    \fbox{
      \begin{minipage}{.63\textwidth}
        \textbf{Third Law: To every action there is always opposed an equal
          reaction; the fores of two bodies acting on each other are always
...
\begin{frame}{Connected Bodies}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[very thick,<-](0,.5)--(-1,.5) node[left]{$\mb{F}_a$};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{A More Typical Problem}
  ...
  \begin{columns}
    \column{.45\textwidth}
    \begin{tikzpicture}[scale=.8]
      \draw[thick](0,0)--(-4,0) node[midway,below]{$\mu$};
      \draw[thick,fill=blue!50!gray!60](-1.75,0)rectangle(-3,1)node[midway]{$m$};
      \draw(-1.75,.44)--(.1,.44);
      \begin{scope}[rotate=-30]
        \draw[thick](0,0)--(5.5,0) node[midway,below left]{$\mu$};
        \draw[thick,fill=pink!80!gray](3,0) rectangle(5,1) node[midway]{$M$};
        \draw(3,.44)--(-.05,.44);
      \end{scope}
      ...
      \draw[->](0,-1)arc(270:330:1)node[midway,below]{$\theta$};
    \end{tikzpicture}
    \column{.55\textwidth}
