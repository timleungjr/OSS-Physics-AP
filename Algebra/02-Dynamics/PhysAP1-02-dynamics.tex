...
%    Multiple objects pressed against one another:
  \begin{center}
    \begin{tikzpicture}
      \draw[thick](-2,0)--(4,0);
      \draw[thick](0,0) rectangle(1,1) node[midway]{$m_1$};
      \draw[thick](1,0) rectangle(3,1.75)  node[midway]{$m_2$};
%      \draw[thick](3,0) rectangle(4.5,2);
      \draw[very thick,->](-1,.5)--(0,.5) node[pos=0,left]{$\mb{F}_a$};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
%    \column{.5\textwidth}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[thick](0,0) rectangle(3,1) node[midway]{$m_1$};
      \draw[thick](.75,1) rectangle(2.25,1.75) node[midway]{$m_2$};
      %\draw(2,1.5) rectangle(2.5,2);
      \draw[very thick,->](3,.5)--(4.5,.5) node[pos=1,right]{$\mb{F}_a$};
    \end{tikzpicture}
  \end{center}
%  \end{columns}
\end{frame}
...
\begin{frame}{A More Typical Problem}
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.8]
      ...
      \draw[->](0,-.5)arc(270:330:.5)node[midway,below right]{\tiny\ang{60}};
    \end{tikzpicture}
    \column{.55\textwidth}
