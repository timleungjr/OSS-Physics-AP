...
\begin{wrapfigure}{r}{.35\textwidth}
  ...
  \begin{tikzpicture}[scale=.5]
    ...
    \draw[->,red,very thick] (0,0)--(4,1) node[midway,above]{$\bm x_i$};
...
\textbf{Displacement} is the change in position from $\bm x_i$ to
$\bm x(t)$ within the same coordinate system, whenever an object moves.
\begin{equation*}
  \Delta\bm x(t)=\bm x(t)-\bm x_i=
...
$\bm x_{\textrm{ref}}=\bm{0}$, any position vector $\bm x$ is also its
...
from position $\bm x_i$ to position $\bm x_f$, as shown in
