...
\begin{frame}{Coulomb's Law for Electrostatic Force}
  ...
  repulsive/attractive force between all charge objects. The force that charge
  $q_1$ exerts on $q_2$ is given by:

  \eq{-.3in}{
    \boxed{\mb{F}_{12}=\frac{kq_1q_2}{|\mb{r}_{12}|^2}\mb{\hat{r}}_{12}}
  }
\end{frame}
...
\begin{frame}{Coulomb's Law for Electrostatic Force}
  ...
  \begin{center}
    \begin{tabular}{l|c|c}
      ...
      Unit vector of direction between point charges & $\mb{\hat{r}}_{12}$ &
    \end{tabular}
  \end{center}
...
\begin{frame}{Electric Field Intensity Near a Point Charge}
  ...
  \eq{-.2in}{
    \boxed{\mb{E}(q,\mb{r})=\frac{kq}{|\mb{r}|^2}\mb{\hat{r}}}
  }
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
...
\begin{frame}{Electric Field Along Axis of a Uniformly Charged Disk}
  ...
  \begin{columns}
    ...
    \vspace{-.1in}Integrating over the entire disk:

    \eq{-.3in}{
      E_x =\pi kx\sigma\int\frac{2ada}{(x^2+a^2)^{3/2}}
    }
    
    \vspace{-.1in}This is not an easy integral!
  \end{columns}
\end{frame}
...
\begin{frame}{Electrical Potential Energy}
  ...
  is the \textbf{electrical potential energy}. $U_q$ can be ($+$) or ($-$),
  because charges can be either ($+$) or ($-$).
\end{frame}
