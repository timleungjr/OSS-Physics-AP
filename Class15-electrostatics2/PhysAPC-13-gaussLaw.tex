...
%      \begin{scope}[->,very thick]
%        \draw[red] (0,0)--(2,0) node[right]{$\vec{F}_{12}$};
%        \draw[blue](8,0)--(6,0) node[left] {$\vec{F}_{21}$};
%      \end{scope}
%      \tikzstyle{balloon1}=[ball color=red];
%      \tikzstyle{balloon2}=[ball color=blue];
%      \shade[balloon1] (0,0) circle(.2) node[above]{$q_1$};
...
%      \draw[dashed] (0,0)--(0,-1.5);
%      \draw[dashed] (8,0)--(8,-1.5);
%      \draw[->,thick](0,-1.3)--(8,-1.3) node[midway,below]{$\vec{r}_{12}$};
%    \end{tikzpicture}
%  \end{center}
%  \begin{itemize}
%  \item\vspace{-.15in}If $q_1$ exerts an electrostatic force $\vec{F}_{12}$ on
%    $q_2$, then $q_2$ likewise exerts a force of $\vec{F}_{21}=-\vec{F}_{12}$
...
%        \draw[->,very thick,blue](.72,0)--(2.5,0) node[right]{$\vec{F}_1$};
...
%    the vector sum of all the forces $\vec{F}_i$:
%    
%    \eq{-.25in}{
%      \boxed{\vec{F}
%        =\sum_i\vec{F}_i
%        =kQ\left(\sum_{i=1}^N\frac{q_i}{r_i^2}\hat{\vec{r}_i}\right)
%      }
%    }
%  \end{columns}
%\end{frame}
...
%    \boxed{\vec{F}
%      =\int\dl\vec{F}
%      =kQ\int\frac{\dl q}{r^2}\hat{\vec{r}}
%    }
%  }
%\end{frame}
%
...
%    F_q
%    =\underbrace{
%      \left[\frac{kq_1}{|\vec{r}_{12}|^2}\hat{\vec{r}}\right]
%    }_{\vec{E}}q_2
%  }
%
...
%
%  \eq{-.2in}{
%    \boxed{\vec{E}(q,\vec{r})=\frac{kq}{|\vec{r}|^2}\hat{\vec{r}}}
%  }
%  \begin{center}
%    \begin{tabular}{l|c|c}
%      \rowcolor{pink}
...
%      Outward unit vector from point source & $\hat{\vec{r}}$ &
%    \end{tabular}
%  \end{center}
...
%  position $\vec{r}$ is the vector sum of all the fields $\vec{E}_i$:
%    
%  \eq{-.2in}{
%    \boxed{\vec{E}
%      =\sum_i\vec{E}_i
%      =k\left(\sum_i\frac{q_i}{r_i^2}\hat{\vec{r}_i}\right)
%    }
%  }
%\end{frame}
%
...
%  \eq{-.2in}{
%    \boxed{
%      \vec{E}=\int\dl\vec{E}=k\int\frac{\dl q}{r^2}\hat{\vec{r}}
%    }
%  }
%  
...
%  force $\vec{F}_q$ and electric field $\vec{E}$ which are vectors
%  \begin{itemize}
...
%
%
%\begin{frame}{Relating $U_q$, $\vec{F}_q$ and $\vec{E}$}{Our Integrals In Reverse}
