...
\begin{frame}{Files to Download}
  ...
  \item\texttt{PhysAP-06-rotMotion-print.pdf}--The ``print version'' of the
    slides for this topic. If you must print, I recommend printing 4 slides per
    page.
  \item\texttt{PhysAP-07-Homework.pdf} Homework problems for Topics 6 and 7.
  \end{itemize}
\end{frame}
...
%  We can extend the coordinate system into 3D. This is called the
...
\begin{frame}{Rigid Body Motion}{Angular Position and Angular Velocity}
  \begin{columns}
    ...
    \vspace{-.15in}\textbf{Angular velocity} $\omega$ (or
    ...
    \vspace{-.1in}$\theta$ is measured in \si{radians}, and $\omega$ in
    \si{rad/\s}
  \end{columns}
\end{frame}
...
\begin{frame}{Rigid Body Motion}{Angular Velocity}
  \begin{columns}
    ...
    \vspace{-.3in}In vector form:

    \eq{-.35in}{ \boxed{\mb{v}=\bm{\omega}\times\mb{r}} }
    
    \begin{itemize}
    \item\vspace{-.25in}$\mb{v}$ is always tangent to circle (perpendicular to
      $\mb{r}$)
    \item We can also relate $\omega$ to \textbf{frequency} and \textbf{period}
      ...
      \vspace{-.15in}$T$ is in seconds (\si{\s}) and $f$ is in hertz
      (\si{\hertz})
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Rigid Body Motion}{Angular Acceleration}
  ...
  \eq{-.3in}{
    \boxed{\alpha=\dot{\omega}(t)=\ddot{\theta}}
  }

  \vspace{-.15in}Similar to the relationship between velocity and angular
  velocity, \textbf{tangential acceleration} $a_\theta$ is related to angular
  acceleration $\alpha$ by the radius $r$:
    
  \eq{-.3in}{
    \boxed{a_\theta(t)=\dot{v}(t)=r\dot{\omega}(t)=r\alpha}
  }
    
  \vspace{-.2in}For \emph{uniform} circular motion, $\omega$ is constant, and
  therefore $\alpha=0$
\end{frame}
...
\begin{frame}{Rotational Equilibrium}
  ...
  \vspace{-.2in}An object is in \textbf{rotational equilibrium} when the
  ...
  \vspace{-.2in}Note that it doesn't mean that the object isn't rotating, it
...
\begin{frame}{Angular Momentum}
  ...
  \begin{columns}
    ...
    \vspace{-.2in}Expanding the terms in the definition:
...
%    \begin{displaymath}
  ...
  \begin{itemize}
  \item\vspace{-.15in}If the net torque on a system is zero, then the rate of
...
\begin{frame}{Kinetic Energy of a Rotating System}
  ...
  \vspace{-.1in}In this case, $I_\mathrm{CM}$ is calculated at the center of
  ...
  \vspace{-.1in}In this case, the $I_\mathrm{P}$ is calculated at the pivot.
