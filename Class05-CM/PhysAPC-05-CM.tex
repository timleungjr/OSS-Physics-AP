...
\begin{frame}{Simple Example}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[fill=gray!50] (2.5,0) circle(.3) node{$m$};
      \uncover<2->{ \fill(0,0) circle(.05) node[below]{CM}; }
    \end{tikzpicture}
  \end{center}
  \vspace{.2in}
...
\begin{frame}{Slightly More Challenging}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \uncover<2->{ \fill (.833,0) circle(.05) node[below]{CM}; }
    \end{tikzpicture}
  \end{center}
...
\begin{frame}{An Equation Helps}
  ...
  \eq{-.2in}{
    \boxed{\bm x_{CM}=\frac{\sum \bm x_i m_i}{\sum m_i}}
  }
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
      \textbf{Quantity} & \textbf{Symbol} & \textbf{SI Unit} \\ \hline
      Position of center of mass (vector) & $\bm x_{CM}$ & \si\metre \\
      Position of point mass $i$ (vector) & $\bm x_i$ & \si\metre \\
...
\begin{frame}{An Example}
  ...
  \begin{align*}
    m_1&=\SI{5.}{\kg} &\bm{x}_1&=3\iii-2\kkk\\
    m_2&=\SI{10.}{\kg}&\bm{x}_2&=-4\iii+2\jjj+7\kkk\\
    m_3&=\SI{1.}{\kg}&\bm{x}_3&=10\iii-17\jjj+10\kkk
  \end{align*}
...
\begin{frame}{Continuous Mass Distribution}
  ...
  \eq{-.15in}{
    \bm x_{CM}=\lim_{n\rightarrow\infty}
    \left(\frac{\sum_{i=1}^n \bm x_i m_i}{\sum_{i=1}^n m_i}\right)
  }

  This gives us an integral form of our equation:

  \eq{-.15in}{
    \boxed{\bm x_{CM}=\frac{\int\bm x\dl m}{\int\dl m}}
  }
\end{frame}
    ...
    \begin{tikzpicture}[scale=.8]
      ...
      \uncover<2->{
        \draw[fill=red](1,0)--(1.2,0)--(1.2,2.4)--(1,2.67)
          node[midway,right]{\footnotesize $\dl m$}--cycle;
      }
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Velocity, Acceleration and Momentum}
  Take time derivative of the equation for $\bm x_\text{CM}$ to get the
  velocity at the CM:
    
  \eq{-.2in}{
    \bm v_\text{CM}=\diff{\bm{x}_\text{CM}}t
    =\frac1m\diff{}t\left(\int\bm x\dl m\right)
    =\frac1m\int\diff{\bm x}t\dl m
    =\frac{\int\bm v\dl m}m
  }
  
  The integral in the numerator is the sum of the momentum of all the masses in
  the system ($\bm p_\text{net}$) which means that we have

  \eq{-.2in}{
    \bm p_\text{net}=m\bm v_\text{CM}
    }

  Taking the derivative of $\bm p_\text{net}$ relates force and acceleration
  at the CM as well:
    
  \eq{-.2in}{
    \bm F_\text{net}=\diff{\bm p_\text{net}}t=m\diff{\bm v_\text{CM}}t
    =m\bm a_\text{CM}
  }
\end{frame}
