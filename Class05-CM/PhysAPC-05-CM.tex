...
\begin{frame}{Slightly More Challenging}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \fill(2.5/3,0) circle(.05) node[below]{CM};
    \end{tikzpicture}
  \end{center}
  ...
  larger masses. We can show using a weighted average:
  
  \eq{-.2in}{
    x_{cm}=\frac{mx_1+(2m)x_2}{m+2m}
  }  
\end{frame}
...
\begin{frame}{An Equation Helps}
  ...
  \eq{-.2in}{
    \boxed{\vec x_{cm}=\frac{\sum \vec x_i m_i}{\sum m_i}}
  }
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
      \textbf{Quantity} & \textbf{Symbol} & \textbf{SI Unit} \\ \hline
      Position of center of mass (vector) & $\vec x_{cm}$ & \si\metre \\
...
%  
%  \eq{-.15in}{
%    \vec x_{cm}=\lim_{n\rightarrow\infty}
%    \left(\frac{\sum_{i=1}^n \vec x_i m_i}{\sum_{i=1}^n m_i}\right)
%  }
  ...
  \eq{-.15in}{
    \boxed{\vec x_{cm}=\frac{\int\vec x\dl m}{\int\dl m}}
  }
...
\begin{frame}{Velocity, Acceleration and Momentum}
  Take time derivative of the equation for $\vec x_{cm}$ to get the
  velocity at the CM:
    
  \eq{-.2in}{
    \vec v_{cm}=\diff{\vec x_{cm}}t
  ...
  \eq{-.2in}{
    \vec F_\text{net}=\diff{\vec p_\text{net}}t=m\diff{\vec v_{cm}}t
    =m\vec a_{cm}
  }
\end{frame}
