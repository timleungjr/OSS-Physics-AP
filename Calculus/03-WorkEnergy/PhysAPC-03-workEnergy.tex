...
\begin{frame}{Kinetic Energy}
  ...
  resulting amount of work done on the object (net work $W_{\textrm{net}}$) is
  given by:
  ...
  Both $\mb{v}$ and $\mb{x}$ are continuous functions in time, we can switch
  the order of differentiation. Also, since $\mb{v}$ and $d\mb{v}$ must be in
  the same direction, the dot product is trivial: $\mb{v}\cdot d\mb{v}=vdv$
...
\begin{frame}{Gravitational Force \& Gravitational Potential Energy}
  ...
  $\mb{g}=-g\bm{\hat{\jmath}}\approx -10\bm{\hat{\jmath}}$ \si{\metre\per\second^2}
...
\begin{frame}{Example: Gravity}
  \begin{center}
    \begin{tikzpicture}[scale=.8]
      ...
      \draw[very thick, red!65,->] (0,4)--(0,3) node[below]{$\mb{F}_g$};
    \end{tikzpicture}
  \end{center}
