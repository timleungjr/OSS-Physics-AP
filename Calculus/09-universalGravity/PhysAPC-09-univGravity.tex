...
\begin{frame}{Law of Universal Gravitation}
  \begin{center}
    \begin{tikzpicture}[scale=.65]
      \draw[->,very thick,red](0,0)--(2,0) node[right]{$\bm{F}_{21}$};
      \draw[->,very thick,blue](8,0)--(6,0) node[left]{$\bm{F}_{12}$};
  ...
  \eq{-.2in}{
    \boxed{
      \bm{F}_{12}=-G\frac{m_1m_2}{|\bm{r}_{12}|^2}\hat{\bm{r}}_{12}}
  }

  where $G=\SI{6.674e-11}{\newton.\metre\squared\per\kilo\gram\squared}$ is the
  \textbf{universal gravitational constant}, $r=|\bm{r}_{12}|$ is the distance
  between the centers of the masses, and
  $\hat{\bm{r}}_{12}=\bm{r}_{12}/|\bm{r}_{12}|$ is the unit vector pointing in
...
\begin{frame}{Law of Universal Gravitation}
  ...
  \end{center}
  \begin{itemize}
  \item If $m_1$ exerts a gravitational force $\bm{F}_{12}$ on
    $m_2$, then $m_2$ likewise also exerts a force of $\bm{F}_{21}=-\bm{F}_{12}$
...
\begin{frame}{Gravitational Field}
  ...
  \begin{center}
    \begin{tabular}{l|c|c}
      ...
      & $G$ & \si{\newton.\metre\squared/\kilo\gram\squared} \\
...
\begin{frame}{More Than One Mass}
  ...
  at any position $\bm{r}$ is the vector sum of all the forces $\bm{F}_i$:
...
\begin{frame}{Relating Gravitational Field \& Gravitational Force}
  ...
  enters the field. Then, $m$ experiences a gravitational force $\bm{F}_g$
...
\begin{frame}{What If You Are Inside Another Mass?}{A Spherical Shell}
  \begin{columns}
    ...
    \column{.7\textwidth}
    That $\bm{g}_\text{inside}=\bm{0}$ can be calculated by:
    \begin{itemize}
    ...
    \item Using \textbf{Gauss's law} for gravity, similar to finding the
      ...
      \eq{-.2in}{
        \oint\bm{g}\cdot d\bm{A}=-4\pi GM_\text{encl}
      }
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Falling Toward the Center of Earth}
  ...
  \begin{columns}
    ...
    \eq{-.2in}{
      g(r)=\frac{GM_E r}{r_E^3}=\left[\frac{g_0}{r_E}\right]r
    }
...
\begin{frame}{Falling Toward the Center of Earth}
  \begin{columns}
    ...
    \eq{-.2in}{
      T=2\pi\sqrt{\frac{m}k}=2\pi\sqrt{\frac{r_E}{g_0}}
    }

    For Earth, $T=\SI{5068}{\second}$. The traveler would pop up on the
...
\begin{frame}{Gravitational Potential Energy}
  ...
  \vspace{-.25in}{\Large
    \begin{align*}
      W&=\int \bm{F}_g\cdot d\bm{r}=
      -\int_{r_1}^{r_2}\frac{Gm_1m_2}{r^2}\hat{\bm{r}}\cdot d\bm{r}\\
...
\begin{frame}{Relating Gravitational Potential Energy to Force}
  ...
  ($\bm{F}_g$) is the negative gradient of the gravitational potential energy
  ($U_g$):
  
  \eq{-.1in}{
    \bm{F}_g=-\nabla U_g=
    -\frac{\partial U_g}{\partial r}\hat{\bm{r}}
  }

  The direction of $\bm{F}_g$ always points from high to low potential energy
  \begin{itemize}
  \item A free-falling object is always decreasing in $U_g$
  \item ``Steepest descent'': the direction of $\bm{F}_g$ is the shortest path
    to decrease $U_g$ 
  \item Objects traveling perpendicular to $\bm{F}_g$ has constant $U_g$
  \end{itemize}
\end{frame}



\begin{frame}{Relating $U_g$, $\bm{F}_g$ and $\bm{g}$}
  Knowing that $\bm{F}_g$ and $\bm{g}$ only differ by a constant (mass $m$), we
  can also relate gravitational field to potential energy by the gradient
  operator:

  \eq{-.15in}{
    \bm{g}=-\nabla V_g=-\frac{\partial V_g}{\partial r}\hat{\bm{r}}
    \quad\text{where}\quad
    V_g=\frac{U_g}{m}
  }

  We already know that the direction of $\bm{g}$ is the same as $\bm{F}_g$,
  i.e.
  \begin{itemize}
  \item The direction of $\bm{g}$ is the shortest path to decrease $U_g$ 
  \item Objects traveling perpendicular to $\bm{g}$ has constant $U_g$
...
\begin{frame}{Properties of Gravitational Force}
  ...
  \item Gravity is a \emph{central force}, in that
    \begin{itemize}
    \item Gravitational force $\bm{F}_g$ is always in the $-\hat{\bm{r}}$
...
%      \;\rightarrow\;
%      d\bm{A}=\frac{1}{2}\bm{r}\times d\bm{r}
%    }
%
...
%    \eq{-.15in}{
%      \frac{d\bm{A}}{dt}
%      =\frac{1}{2}\bm{r}\times\frac{d\bm{r}}{dt}
%      =\frac{1}{2}\bm{r}\times\bm{v}
%    }
%  \end{columns}
%\end{frame}
%
...
%
\begin{frame}{Kepler's Second Law: Law of Equal Areas}
%  We can express $\bm{r}\times\bm{v}$ in terms of angular momentum,
%  $\bm{L}=m(\bm{r}\times\bm{v})$. But in motion under any central force (such
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  ...
  \textbf{radial velocity} $\bm{v}_r$ and \textbf{angular velocity}
  $\bm{v}_\theta$.
  \begin{columns}
    \column{.3\textwidth}
    \begin{tikzpicture}[scale=.55]
      ...
      \begin{scope}[rotate around={\angle:({sqrt(\a*\a-\b*\b)},0)}]
        ...
        node[pos=.45,below]{\tiny$\bm{r}$}
        node[left]{\tiny $\bm{v}_r$};
        \draw[->](7.65,0)--(7.65,.85) node[below]{\tiny $\bm{v}_\theta$};
...
%      node[midway,left]{\tiny$r_0$};
    ...
    \column{.7\textwidth}
    \begin{itemize}
    \item $\bm{v}_\theta$ means a centripetal acceleration toward $M$
    \item Changes in $\bm{v}_r$ (i.e.\ accceleration in the radial direction)
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  ...
  \eq{-.2in}{
    r=\left[\frac{L^2}{GMm^2}\right]\frac{1}{1+e\cos\theta}
...
%        \draw[very thick,->](0,0)--(3,0) node[midway,below]{$\bm{r}_1$};
%        \draw[very thick,->](0,0)--(-1.8,0) node[midway,above]{$\bm{r}_2$};
...
%  \end{center}
%
%  \vspace{-.1in}The vectors $\bm{r}_1$ and $\bm{r}_2$ are relative to the
%  center of mass, and the relative position $\bm{r}$ velocity $\bm{v}$ and
%  acceleration $\bm{a}$ between $m_1$ and $m_2$ are therefore:
%  
%  \eq{-.2in}{
%    \bm{r}=\bm{r}_2 - \bm{r}_1\quad
%    \bm{v}=\bm{v}_2 - \bm{v}_1\quad
%    \bm{a}=\bm{a}_2 - \bm{a}_1
%  }
%\end{frame}
%
%
...
%
%  \eq{-.2in}{
%    m_1\bm{a}_1+m_2\bm{a}_2=0
%  }
%
%  \vspace{-.2in}Substituting the expression for relative acceleration, we have:
%
%  \eq{-.2in}{
%    \bm{a}=\bm{F}_g\left[\frac{1}{m_1}+\frac{1}{m_2}\right]=
%    \bm{F}_g\left[\frac{m_1+m_2}{m_1m_2}\right]=\frac{\bm{F}_g}{\mu}
%  }
%  
...
%    \begin{tikzpicture}
%      \begin{scope}[rotate=23]
%        \draw[very thick,->](0,0)--(4.8,0) node[midway,below]{$\bm{r}$};
