...
\begin{frame}{Law of Universal Gravitation}
  ...
  where $G=\SI{6.674e-11}{N.m^2/kg^2}$ is the
  \textbf{universal gravitational constant}, $r=|\mb{r}_{12}|$ is the distance
...
\begin{frame}{What If You Are Inside Another Mass?}{A Spherical Shell}
  \begin{columns}
    ...
    \column{.7\textwidth}
    That $\mb{g}_{inside}=\mb{0}$ can be calculated by:
    \begin{itemize}
    ...
    \item Using \textbf{Gauss's law} for gravity, similar to finding the
      ...
      \eq{-.2in}{
        \oint\mb{g}\cdot d\mb{A}=-4\pi GM_{encl}
      }
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Gravitational Potential Energy}
  ...
  \vspace{-.25in}{\Large
    \begin{align*}
      ...
      -\int_{r_1}^{r_2}\frac{Gm_1m_2}{r^2}\hat{\mb{r}}\cdot d\mb{r}\\
...
\begin{frame}{Relating Gravitational and Centripetal Force}
  ...
  $v_{orbit}$, which does not depend on the mass of the small object in orbit:
...
\begin{frame}{Escape Speed from Circular Orbits}
  ...
  We can then solve for the initial speed $v_i=v_{esc}$
  (\textbf{escape speed} or \textbf{escape velocity}):

  \eq{-.2in}{
    \boxed{v_{esc}=\sqrt{\frac{2GM}{r_i}}}
  }
...
\begin{frame}{What if I'm not escaping from the surface?}
  ...
  $v_{orbit}$, so escaping from that orbit requires only an additional
  speed of
  
  \eq{-.2in}{
    \Delta v=v_{esc}-v_{orbit}=(\sqrt2-1)v_{orbit}
  }
  \begin{itemize}
  \item What if $v_{orbit}<v<v_{esc}$?
  \item What if $v<v_{orbit}$?
  \end{itemize}
\end{frame}
...
\begin{frame}{Orbital Energies}
  ...
  \eq{-.3in}{
    K_{orbit}=\frac12mv_{orbit}^2=\frac{1}{2}m
...
%      node[midway,left]{\tiny$r_0$};
    ...
    \item Changes in $\mb{v}_r$ (i.e.\ accceleration in the radial direction)
      also means a force along $\hat{\mb{r}}$
...
\begin{frame}{Binary System}
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.35]
      ...
      node[above]{\footnotesize $\mb{F}_g$};
