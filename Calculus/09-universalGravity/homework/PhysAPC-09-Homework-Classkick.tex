...
\begin{center}
  {\LARGE\textbf{I NEED \underline{TWO} MORE FREE-RESPONSE QUESTIONS\\
...
%TL\item Five equal masses $M$ are equally spaced on the arc of a semicircle of
%TL  radius $R$ as shown in the figure below. A mass $m$ is located at the center
%TL  of curvature of the arc. If $M$ is \SI{3}{\kilo\gram}, $m$ is
%TL  \SI{21}{\kilo\gram}, nad $R$ is \SI{10}{\centi\metre}, what is the force on
%TL  $m$ due to the five masses?
%TL  \begin{center}
%TL    \begin{tikzpicture}[scale=.6]
%TL      \tikzstyle{balloon1}=[ball color=gray!50];
%TL      \tikzstyle{balloon2}=[ball color=red!60!black];
%TL      \draw[->](0,0)--(5,0) node[right]{$x$};
%TL      \draw[->](0,0)--(0,5) node[above]{$y$};
%TL      \draw[ultra thick,blue!80](4,0) arc(0:180:4);
%TL      \shade[balloon1] (0,0) circle (.1) node[below]{$m$};
%TL      \foreach \theta in {0,45,...,180} {
%TL        \begin{scope}[rotate=\theta]
%TL          \shade[balloon2] (4,0) circle (.3) node[white]{$M$};
%TL        \end{scope}
%TL      }
%TL    \end{tikzpicture}
%TL  \end{center}
