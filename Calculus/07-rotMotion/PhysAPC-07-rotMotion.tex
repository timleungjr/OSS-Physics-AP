...
\begin{frame}{Equation of Motion}{Newton's Second Law}
  ...
  $\bm{F}_{net}$ causes the center of mass to accelerate (linearly), what
...
\begin{frame}{Torque}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  $\bm{F}_{net}=\bm{0}$, therefore $\bm{a}=\bm{0}$. But (obviously) it
...
%  \item In the example below, an applied force $\bm{F}_a$ is applied away from
%    the pivot at an angle $\phi$. This generates a torque around the pivot.
  ...
  \begin{center}
    \begin{tikzpicture}[scale=2.5]
      ...
      \draw[ultra thick,red,->](0,0)--(1.8,0) node[midway,below]{$\bm{r}$};
      \draw[ultra thick,blue,->](1.2,-.7)--(1.8,0)node[pos=0,below]{$\bm{F}_a$};
...
\begin{frame}{Torque}
  In scalar form, we can express torque $\bm{\tau}$ as the force $\bm{F}_a$,
  the \textbf{moment arm} $\bm{r}$ and the angle $\phi$ between $\bm{F}_a$ and
  $\bm{r}$:
...
\begin{frame}{Rotational Equilibrium: First Law of Motion}
  ...
  not changing, i.e.\ the translational momentum $\bm{p}$ is constant.
\end{frame}  
...
\begin{frame}{Rotational Equilibrium: First Law of Motion}
  ...
  not changing, i.e.\ $\alpha=\bm{0}$, or that the
  \textbf{angular momentum} $\bm{L}$ is constant.
\end{frame}
...
\begin{frame}{Example Problem}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=2.5]
      ...
      \draw[ultra thick,orange,->](0,0)--(1,1)node[right]{\SI{30}{N}};
...
\begin{frame}{Angular Momentum}
  ...
  \begin{columns}
    ...
    \vspace{-.15in}The direction of $\bm{L}$ depends on the direction of
    rotation. Expanding the terms:
    
    \eq{-.45in}{
      \bm{L}=\bm{r}\times(m\bm{v})=m\bm{r}\times(\bm{\omega}\times\bm{r})
      =mr^2\bm{\omega}
    }
    ...
    \begin{tikzpicture}[scale=2.5]
      \begin{scope}[rotate=70]
        ...
        \draw[ultra thick,red,->](0,0)--(2,0)node[pos=.5,right]{$\bm{r}$};
        \draw[ultra thick,->](2,0)--(2,.5)node[above]{$\bm{v}$};
      \end{scope}
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Angular Momentum and Moment of Inertia}
  ...
  \eq{-.2in}{
    \bm{p}=m\bm{v}\quad\quad\quad \bm{L}=I\bm{\omega}
  }
  \begin{itemize}
...
\begin{frame}{Second Law of Motion for Rotational Motion}
  ...
  \eq{-.2in}{
    \bm{F}=\frac{d\bm{p}}{dt}\quad\quad\bm{\tau}=\frac{d\bm{L}}{dt}
  }
  ...
  \eq{-.2in}{
    \bm{F}=m\bm{a}\quad\quad \bm{\tau}=I\bm{\alpha}
  }  
\end{frame}
...
\begin{frame}{Kinetic Energy of a Rotating System}
  ...
  In this case, $I_{CM}$ is calculated at the center of
  mass. For simple problems, we only need to compute rotational kinetic energy
  at the pivot:

  \eq{-.2in}{
    \boxed{K=\frac{1}{2}I_P\omega^2}
  }
  
  In this case, the $I_P$ is calculated at the pivot.
  \textbf{IMPORTANT:} $I_{CM}\neq I_P$
\end{frame}
