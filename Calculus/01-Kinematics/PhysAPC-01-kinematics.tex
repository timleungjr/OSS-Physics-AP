...
\begin{frame}{Distance}
  ...
  The unit for distance is also a \emph{meter} (\si{\metre}).
  \begin{columns}
    \column{.7\textwidth}
    \begin{itemize}
...
\begin{frame}{Instantaneous Velocity}
  ...
  $\mb{v}$ of an object is the time rate of change of position. The unit for
  ...
  Since $\mb{x}$ has $x$, $y$ and $z$ components in the $\iii$, $\jjj$ and
  $\kkk$ directions (which are all linearly independent), we can take the time
...
\begin{frame}{Integrating Velocity to Get Position/Displacement}
 ...
 \eq{-.2in}{ \mb{x}(t)= \left(
    \int v_x\iii + \int v_y\jjj + \int v_z\kkk \right) dt + \mb{x}_0
  }
\end{frame}
...
\begin{frame}{Acceleration as Functions of Velocity and Position}
  ...
  \item Damping force: $a(v)=-bv^n$ ($b$ is a damping constant, and $n$ is
    usually 1)
