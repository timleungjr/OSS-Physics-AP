...
\begin{frame}{Instantaneous Velocity}
  Since position $\mb{x}$ is continuously differentiable in time $t$, the
  \textbf{instantaneous velocity} can be found at any time by differentiating
  $\mb{x}$ with respect to $t$. The unit for velocity is
...
\begin{frame}{Integrating Velocity to Get Position/Displacement}
  Conversely, if instantaneous velocity $\mb{v}(t)$ is the time rate of change
  of position $\mb{x}(t)$, then $\mb{x}(t)$ is the time integral of $\mb{v}(t)$:
...
\begin{frame}{Average Velocity}
  \textbf{Average velocity} $\overline{\mb{v}}$ of an object is the
  \emph{finite} change in position $\Delta\mb{x}$ over a \emph{finite} time
  interval $\Delta t$:
  ...
  \eq{-.3in}{
    \boxed{
      ...
      \frac{\mb{x}-\mb{x}_0}{t-t_0}=
      \frac{\int_{t_0}^t\mb{v}dt}{t-t_0}
    }
  }
...
\begin{frame}{Instantaneous \& Average Acceleration}
  ...
  \eq{-.27in}{
    \boxed{
      \overline{\mb{a}}=
      \frac{\Delta\mb{v}}{\Delta t}
      =\frac{\mb{v}-\mb{v}_0}{t-t_0}
      =\frac{\int_{t_0}^t\mb{a}dt}{t-t_0}
    }
  }
...
\begin{frame}{Simple Harmonic Motion}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.5]
      \draw[->,thick] (0,0)--(4.5,0) node[right]{$t$};
      \draw[->,thick] (0,-2.5)--(0,2.5) node[above]{$d$};
...
\begin{frame}{Relative Motion}
  \begin{columns}
    ...
    \end{itemize}
    It is obvious that $\textcolor{green!80!black}{\mb{x}_{AB}(t)}$ and
