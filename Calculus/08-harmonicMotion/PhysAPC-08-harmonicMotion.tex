...
\subtitle{Advanced Placement Physics C Mechanics1}
...
\begin{frame}{Hooke's Law}
  Hooke's law for an ideal spring relates the force exerted by a
  compressed/stretched) spring onto another object (\textbf{spring force}
  $\mb{F}_s$) to the stiffness of the spring (\textbf{spring constant} $k$) and
  spring displacement $\mb{x}$:
...
\begin{frame}{Mass on a Spring}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.1]
      ...
      node[pos=1,right]{\footnotesize $\mb{N}$};
      \draw[ultra thick,->,red](a)--(4.5,1)
      node[pos=1,above]{\footnotesize $\mb{F}_s$};
      \draw[->,thick](7,1)--(7.8,1) node[pos=1,right]{\footnotesize $x$};
    \end{tikzpicture}
  \end{center}
  $\mb{w}$ and $\mb{N}$ cancel out, so net force is due only to spring force
  $\mb{F}_s=-k\mb{x}$. This is true both when the spring is in compression or
  extension. (In the diagram above, the spring is in extension.)
\end{frame}
...
\begin{frame}{Mass on a Spring}
  ...
  \begin{itemize}
  \item The solution to the equation is a function $x(t)$ where the second time
    derivative $\ddot{x}$ looks like $x$ but with a negative sign
  \item The obvious choices are the trigonometric functions $\sin(t)$ and
    $\cos(t)$
  \item Starting with this general form:
    
    \eq{-.25in}{
      x(t)=A\cos(\omega t-\phi)
    }

    \vspace{-.15in}$\cos$ is usually preferred over $\sin$ because $\cos(0)=1$,
    and oscillations usually begin at maximum amplitude $A$ at $t=0$.
...
\begin{frame}{Side Note \#2}
  ...
  \eq{-.25in}{
    ...
    \frac{dx}{dt}=\omega e^{\omega t}=\omega x(t)
  }

  However, the second derivative of $e^{\omega t}$ does not have the negative
  sign that is needed to solve the problem. But if the exponential function is
  \emph{imaginary}:
...
\begin{frame}{Vertical Spring-Mass System}
  ...
  \begin{columns}
    ...
    But since $\mb{w}=m\mb{g}$ is a constant, the only change is the addition
...
\begin{frame}{Vertical Spring-Mass System}
  ...
  \begin{columns}
    ...
    \column{.85\textwidth}
    The constant $B$ is found by substituting $x$ and $\ddot{x}$ into the ODE.
...
\begin{frame}{What About a Simple Pendulum?}
  \begin{columns}
    ...
    \item It has already been shown in the previous topic on circular motion
      that when the mass is deflected by an angle $\alpha$, the force in the
      angular direction is $F_\theta=-mg\sin\alpha$
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=10]
        ...
        node[pos=1,left]{\footnotesize $mg\sin\alpha$};
      ...
      \draw[->](0,-2) arc(270:280:2) node[pos=0.5,below]{$\alpha$};
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{The Simple Pendulum}
  \begin{columns}
    \column{.75\textwidth}
    Substitute $F_\alpha$ into second law of motion, and cancelling mass term, we
    get:

    \eq{-.35in}{
      F_\alpha=ma_\alpha\;\;\longrightarrow\;\;
      -g\sin\alpha=L\frac{d^2\alpha}{dt^2}
    }

    Solving this ODE in its present form is difficult because of the
    $\sin\alpha$ term. However, the series expansion of the sine function

    \eq{-.35in}{
      \sin(\alpha)=\alpha-\frac{\alpha^3}{3!}+\frac{\alpha^5}{5!}-
      \frac{\alpha^7}{7!}+\cdots
    }
    
    shows that for small angles ($\alpha<15^\circ$), $\sin\alpha\approx\alpha$
...
\begin{frame}{The Simple Pendulum}
  \begin{columns}
    ...
    For small angles of $\alpha$, the ODE reduces to the same form as the
    spring-mass system!
...
%  \end{center}
%  \vspace{-.15in}
  The 2nd-order ODE is obtained by applying second law of motion:
...
\begin{frame}{Damped Oscillator}
  ...
  an exponential decay term and a sinusoidal term:
  ...
  and angular frequency $\omega'$ now given by:
