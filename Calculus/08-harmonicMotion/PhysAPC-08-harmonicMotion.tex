...
\begin{frame}{Mass on a Spring}
  ...
  \eq{-.1in}{
    \boxed{\frac{d^2x}{dt^2}+\frac{k}mx=0}
  }
...
\begin{frame}{What About a Simple Pendulum?}
  \begin{columns}
    ...
    \item It has already been shown previously that when the mass is deflected
      by an angle $\phi$, the force in the tangential direction is
      $F_t=-mg\sin\phi$
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=10]
        ...
        \begin{scope}[red!80!black,very thick,->]
          \draw[dotted](0,-5)--(-0.5,-5) node[left]{$mg\sin\theta$};
...
\begin{frame}{The Simple Pendulum}
  \begin{columns}
    \column{.75\textwidth}
    Substitute $F_t$ into second law of motion, and cancelling mass term, we
    get:

    \eq{-.2in}{
      F_t=ma_t\;\;\rightarrow\;\; -g\sin\phi=\ell\frac{d^2\phi}{dt^2}
    }
    ...
    shows that for small angles (less than \ang{10}), $\sin\phi\approx\phi$
...
\begin{frame}{The Simple Pendulum}
  \begin{columns}
    ...
    \eq{-.1in}{
      \frac{d^2\alpha}{dt^2}+\frac{g}\ell\alpha=0
    }
    ...
    \begin{tikzpicture}
      ...
      \end{scope}
      \draw[dashed,thin](0,0)--(0,-5);
      \draw[->](0,-2) arc(270:280:2) node[pos=0.5,below]{$\phi$};
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Ordinary Differential Equation for the Pendulum}
  The solution for $\phi(t)$ is very similar to the spring-mass system:
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{scope}[very thick,->,red!80!black]
        ...
        \draw(3.5,1)--(3.5,2) node[above]{$\bm{N}$};
        \draw(3.5,1)--(2.5,1) node[above]{$\bm{F}_s$};
        \draw(3.5,.9)--(2,.9) node[below]{$\bm{F}_{ext}$};
        \draw(3.5,1)--(4.5,1) node[below]{$\bm{F}_D$};
      \end{scope}
    \end{tikzpicture}
  \end{center}
