...
\begin{frame}{Electric Field and Electric Potential Difference}
  Recall that the relationship between electrostatic force ($\bm{F}_q$) and
  ...
  \eq{-.25in}{
    \Delta U_q=-\int\bm{F}_q\cdot\dl\bm{r}\quad\quad
    \bm{F}_q=-\nabla U_q=-\diffp{U_q}r\hat{\bm{r}}
  }

  \vspace{-.08in}Dividing both sides of the equations by $q$, we get the
  relationship between electric field ($\bm{E}$), electric potential ($V$) and
  electric potential difference ($\Delta V$):
  
  \eq{-.2in}{
    \Delta V=-\int\bm{E}\cdot\dl\bm{r}\quad\quad
...
\begin{frame}{Cylindrical Capacitors: Electric Field}
  \begin{columns}
    ...
    \eq{-.2in}{
      \oint \bm{E}\cdot\dl A=2\pi rLE=\frac{Q}{\epsilon_0}
    }
...
\begin{frame}{Cylindrical Capacitors: Voltage Across the Cylinders}
  ...
  \eq{-.2in}{
    \Delta V= -\int_a^b E\dl r=\frac{Q}{2\pi L\epsilon_0}\int_b^a\frac 1r\dl r
