...
\gentitle{14}{UNIVERSAL GRAVITATION}
...
\begin{multicols*}{2}
  \begin{questions}
    ...
    \begin{choices}
      \choice\SI{1.0e3}{\metre\per\second}
      \choice\SI{3.5e3}{\metre\per\second}
      \choice\SI{7.8e3}{\metre\per\second}
      \choice\SI{5.0e6}{\metre\per\second}
...
\begin{questions}
  ...
  \begin{parts}
    ...
    GS would be greater than or less than the given period.
  ...
  \def\arraystretch{1}
  \begin{parts}
    \part\textbf{Write} an algebraic expression for the gravitational force
    between Saturn and one of its moons.
    \vspace{\stretch1}
    ...
    \part\textbf{Indicate} which quantities should be graphed to yield a
    straight line whose slope could be used to determine Saturn's mass?
    \vspace{\stretch1}
    
    \part\textbf{Complete} the data table by calculating the two quantities to
    be graphed. \textbf{Label} the top of each column, including units.
    \newpage
    
    \part\textbf{Plot} the graph on the axes below. \textbf{Label} the axes
    with the variables used and appropriate numbers to indicate the scale.
    \uplevel{
      \centering
      \begin{tikzpicture}[xscale=2.3,yscale=1.6]
        \draw[gray,step=.2] grid (6,6);
        \draw[thick] grid (6,6);
        \draw[axes] (0,0)--(6.3,0);
        \draw[axes] (0,0)--(0,6.3);
      \end{tikzpicture}
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      \draw[thick] circle (2);
      \draw[thick] (-0.1,2) rectangle (.1,-2);
