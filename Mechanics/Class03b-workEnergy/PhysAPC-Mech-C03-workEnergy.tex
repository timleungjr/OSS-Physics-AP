...
\title{Classes 3 and 4: Work and Energy}
...
\begin{frame}{Mechanical Work}
  \textbf{Mechanical work} $\dl W$ is done when a force $\vec F(\vec r)$
  displaces an object by $\dl\vec r$. When the force moves an object along a
  path $\mathcal C$, the total work done by the force is defined as:

  \eq{-.1in}{
    \boxed{
      W=\int_{\mathcal C}\dl W= \int_{\mathcal C}\vec F(\vec r)\cdot\dl\vec r
    }
  }
  ...
  \item No work done if the force is perpendicular to displacement, when
    $\vec F\cdot\dl\vec r=0$ (i.e.\ the force does not cause the displacement)
  \item No work done if there is no displacement ($\dl\vec r=\vec 0$)
...
\begin{frame}{Work by Constant Force}
  For a constant or average force\footnote{The average force is called a
  \textbf{spatial average} that is based on \emph{distance} instead of time;
  i.e.
  \begin{displaymath}
...
\begin{frame}{Definition of Work}
  ...
  \item This quantity is important if we want to know the mechanism (how and
    how much) in which energy is transformed
  \end{itemize}

  \vspace{.15in}\textbf{Total work done on an object}
  \begin{itemize}
  ...
  \item This quantity is important if we want to know how net work changes
    motion
  \end{itemize}
\end{frame}
...
\begin{frame}{Kinetic Energy}
  ...
  $\vec F_\text{net}$ is perpendicular to motion, e.g.\ uniform circular motion}
  is given by:
...
\begin{frame}{Work-Energy Theorem}
  ...
  \item It does not matter \emph{what} the net force consists of
  \end{itemize}
\end{frame}
...
\begin{frame}{Work Done by Gravity---Path Independence}
  The
  %change in gravitational potential energy (i.e.\
  work done by gravity
  is \textbf{path independent:} it depends only on the weight ($\vec F_g$) of
  the object, and the end points ($h_0$ and $h_1$), but not the path.
  \begin{center}
    \begin{tikzpicture}
      \draw[thick,dashed] (0,0)--(12,0) node[right]{$h_0$};
      \draw[thick,dashed] (0,-2)--+(12,0) node[right]{$h_1$};
    ...
    \end{tikzpicture}
  \end{center}
  $W_g$ is the same in all above cases because they all have the same $h_0$ and
  $h_1$.
\end{frame}
...
\begin{frame}{Energy Diagrams}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.8]
      ...
      \draw[axes] (0,0)--(0,5) node[above]{$U(x)$};
...
\section{Internal Energy (aka Thermal Energy)}
...
\begin{frame}{Internal Energy}
  \textbf{Internal energy} (or \textbf{thermal energy}) of a system of
  molecules is the sum of all their kinetic and potential energies at the
  olecular level:
  ...
  It is proportional to the molecules' \textbf{absolute temperature}, measured
  in \emph{kelvin}. This is part of a major field in physics called
  thermodynamics that is not covered in this course\footnote{The introductory
  discussion is covered in AP Physics 2}; we are only including them here for
  completeness.\footnote{For those who are interested, for ideal and monatomic
  gases, $E_\text{int}=\frac32Nk_BT$; for diatomic gases,
  $E_\text{int}\approx\frac52Nk_BT$; and for solids, $E_\text{int}\approx3Nk_BT$.}
\end{frame}
...
\begin{frame}{Law of Conservation of Energy}
  ...
  A \textbf{system} is a predefined collection of objects that apply forces (and
  therefore they \emph{may} do work) on each other. It can be:
  \begin{itemize}
  \item\textbf{Isolated:} The system does not exchange energy with its
    surroundings, therefore $W_\text{ext}=0$. There are either no external
    forces, or external forces do not do work to the system
    %\item\textbf{Closed:} There are forces that are external to the system, but
    %  they do not do work
  \item\textbf{Open:} External forces on the system do mechanical work to the
    system
  \end{itemize}
\end{frame}
...
\begin{frame}{Law of Conservation of Energy}
  ...
  \eq{-.1in}{
    \boxed{
      E_\text{sys} = \underbracket[1pt]{\sum K + \sum U}_\text{mechanical} +
      \underbracket[1pt]{\sum E_\text{int}}_\text{thermal}
    }
  }
  
  Not surprisingly, the \emph{change} in system energy is the net change
...
\begin{frame}{Law of Conservation of Energy}
  ...
  \item\textbf{Negative} if work is done \text{by} the system to the
    surrounding
  \end{itemize}
\end{frame}
...
\begin{frame}{Simple Pendulum System}
  \begin{columns}
    \column{.7\textwidth}
    Assuming that there are no friction, drag or other damping forces, the
    simple pendulum system (consists of the mass and Earth) is a closed system
    \begin{itemize}
...
\begin{frame}{Isolated System with Changing Internal Energy}
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.6]
      ...
      \draw[thick,
        decoration={aspect=.3,segment length=2mm, amplitude=2.5mm, coil},
        decorate] (2.5,3.8)--(2.5,2.25) node[midway,right=7]{$k$};
      \draw[mass] (2,2.25) rectangle (3,1.25) node[midway]{$m$};
    \end{tikzpicture}
