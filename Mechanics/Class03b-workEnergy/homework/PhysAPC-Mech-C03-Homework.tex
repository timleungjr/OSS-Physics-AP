...
\begin{questions}
  ...
  \uplevel{
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.95,thick]
        ...
        \draw (3,0)--(9,0);
        \draw (5,1) rectangle (6,2) node[midway]{\SI2{\kilo\gram}};
        \draw (4,0) rectangle (7,1) node[midway]{\SI4{\kilo\gram}};
        \draw[vectors] (7,.5)--(8,.5) node[right]{$\vec F$};
      \end{tikzpicture}
    \end{center}
  }
  ...
  \question What is the acceleration of the \SI2{\kilo\gram} block relative to
  the \SI4{\kilo\gram} block if a force is applied to the 4-kg block that
...
\begin{questions}
  ...
  \begin{parts}
    ...
    \textbf{A}, \textbf{derive} an expression for the magnitude of the
    centripetal force acting on the compartment at point $C$. If you need to
    draw anything besides what you have shown in part \textbf{A} to assist in
    ...
    \part\textbf{Solve} the differential equation you wrote in part \textbf{E}.
    %(\ref{diffeq}).
    \vspace{\stretch1}
  ...
  \begin{parts}
    ...
    \uplevel{
      ...
      below. Express your answers to parts \textbf{B}, \textbf{C}, and
      \textbf{D} in terms of $y$, $L$, $M$, and fundamental constants.
      \cpic{.45}{rope1}
    }
    ...
    $v_h$ from part \textbf{A} or $v_r$ from part \textbf{D} is greater after
    the block and the end of the rope have traveled this distance.

    \vspace{.2in}
    \underline{\hspace{.4in}} $v_h$ is greater.\hspace{.5in}
    \underline{\hspace{.4in}} $v_r$ is greater.\hspace{.5in}
    \underline{\hspace{.4in}} The speeds are equal.
  ...
  \begin{parts}
    ...
    friction is larger than the value calculated in part \textbf{C}.
