...
\begin{questions}
  ...
  \begin{parts}
    ...
    (\ref{diagram-a}), determine an expression for the magnitude of the
  ...
  \begin{parts}
    ...
    from part (\ref{vh}) or $v_r$ from part (\ref{vr}) is greater after the
    block and the end of the rope have traveled this distance.
  ...
  % TAKEN FROM THE 2023 AP PHYSICS C MECH EXAM SET 2 FREE-RESPONSE QUESTION 1
  ...
  \begin{parts}
    \part Calculate the minimum value of the coefficient of static friction
    $\mu_s$ that would keep the two-block system at rest.
    \newpage
    ...
    \uplevel{
      ...
      \begin{center}
        \begin{tikzpicture}[scale=2]
          ...
          \draw[thick,->] (0,0)--(1.7,0) node[right]{$t$ [s]};
    ...
    \part On the dots below, which represent the blocks, draw and label the
    forces (not components) that act on each block. Each force must be
    ...
    \part Calculate the coefficient of kinetic friction $\mu_k$ between block 1
    and the table.
    \label{mu_k}
    \vspace{\stretch1}
    
    \part Careful measurements determine that the coefficient of kinetic
    friction is larger than the value calculated in part (\ref{mu_k}) . Does
    the following explanation sufficiently account for the observed discrepancy?
    ...
    \part Draw a best-fit line for the data points.
    \part Using the straight line, calculate an experimental value for the
    ...
    $m_2=\SI{.20}{\kilo\gram}$ is repeated. Would the acceleration of the
    system be greater than, less than, or equal to the acceleration of the
    system in the original experiment?
