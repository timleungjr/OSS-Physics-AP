...
\title{Class 15: Orbital Mechanics}
...
\begin{frame}{Newton's Thought Experiment}
  In \emph{Treatise of the System of the World}, the third book in
  \emph{Principia}, Newton presented this thought experiment. A cannonball is
  launched from a high mountain, parallel to Earth's surface:

  \vspace{-.2in}\begin{columns}[T]
    \column{.25\textwidth}
    \begin{center}
      \begin{tikzpicture}[scale=.8]
        \node at (0,0) {\pic{.81}{earth-1200px}};
        \fill[red] (0,1.7) circle (.05);
        \draw[functions,->] (0,1.7) arc (90:30:1 and 1.45);
        \draw[gray] circle (1.7);
      \end{tikzpicture}
    \end{center}
    {\scriptsize If initial velocity is slow, the object falls back onto the
      planet after travelling a short distance\par}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.8]
        \node at (0,0) {\pic{.81}{earth-1200px}};
        \fill[red] (0,1.7) circle (.05);
        \draw[functions,->] (0,1.7) arc (90:-55:1.5 and 1.5);
        \draw[gray] circle (1.7);
      \end{tikzpicture}
    \end{center}
    {\scriptsize With a higher initial velocity, the projectile travels a
      longer distance, but eventually, it still falls\par}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.8]
        \node at (0,0) {\pic{.81}{earth-1200px}};
        \fill[red] (0,1.7) circle (.05);
        \draw[functions,->] (0,1.7) arc (90:-270:1.7);
      \end{tikzpicture}
    \end{center}
    {\scriptsize With a sufficiently high initial speed, called the
      \textbf{orbital speed}, the object stays on a circular path around the
      planet. It is now in orbit\par}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.8]
        ...
        \fill[red] (0,1.7) circle (.05);
        \draw[gray] circle (1.7);
        \draw[functions,->,domain=0:2.2] plot(\x,{-.3*\x*\x+1.7});
      \end{tikzpicture}
    \end{center}
    {\scriptsize At even higher initial speed, called the \textbf{escape speed},
      the object escapes the gravitational pull of the planet, and never
      returns\par}
  \end{columns}
\end{frame}
...
\begin{frame}{Orbital Speed}
  \textbf{Orbital speed} is the speed required for an object to stay in a
...
\begin{frame}{Orbital Speed}
  To calculate the orbital speed, we begin with these asumptions:
  \begin{columns}
    \column{.35\textwidth}
    \begin{tikzpicture}[scale=.8]
      \draw[gray] circle (3) node[above=88]{\scriptsize orbital path};
      \node at (0,0) {\pic{.4}{earth-1200px}};
      \node at (0,0) {$M$};
      \foreach \x in {0,3} \draw (\x,0)--+(0,-1);
      \draw[vectors,red] (3,0)--+(-1.2,0) node[above]{$\vec F_g$};
      \shade[balloon1] (3,0) circle (.16) node[white]{\scriptsize$m$};
      \draw[<->] (0,-.8)--(3,-.8) node[midway,fill=black!2]{$r$};
    \end{tikzpicture}
    
    \column{.65\textwidth}
    \begin{itemize}
    \item A small mass ($m$) orbits around a much larger mass ($M$), i.e.\
      $M\gg m$
    \item The orbit is perfectly circular
    \item The only force acting on the objects is gravity; centripetal force is
      provided by gravity
    \end{itemize}
    Following these assumptions:
    \begin{itemize}
    \item $M$ is treated as stationary; only $m$ moves
...
%    
%    \column{.65\textwidth}
%    To calculate the magnitude of Orbital Speed, we make these asumptions:
%    \begin{itemize}
...
\begin{frame}{Orbital Speed}
  ...
  \eq{-.1in}{
    ...
    m{\color{magenta}a_c}\quad\to\quad
  ...
  $v$, we find the orbital speed to be:

  \eq{-.15in}{
    v=\boxed{v_\text{orbit}=\sqrt{\frac{GM}r}}
  }

  This equation is only applicable for perfectly circular orbits. The direction
  of the orbital vector is tangent to the circular orbit.
\end{frame}
...
\begin{frame}{Escape Speed}
  ...
  \eq{-.1in}{
    \underbracket[1pt]{K_i + U_i}_\text{launch} =
    \underbracket[1pt]{K_\infty + U_\infty}_\text{escaped}
  }
  \begin{itemize}
  \item Initial gravitational potential energy at launch is:
...
\begin{frame}{Escape Speed}% from Circular Orbits}
  ...
  where $r_i$ is the initial distance from the centre of the planet/star. There
...
\begin{frame}{Escape Speed}
  ...
  gravitational pull. But the equation assumes that:
  \begin{itemize}
  \item Mass of the object remains constant
  \item All the work is done by gravity
  \end{itemize}
  This is not the case for launching a \emph{real} rocket, where we must
  consider the change in mass when the rocket is burning fuel, and the
  (positive) work done by the rocket's thrust and negative work done by the
  enormous drag while travelling at hypersonic speeds while in the atmosphere
\end{frame}



\begin{frame}{Example Problem}
  \textbf{Example:} Determine the escape speed for a \SI{1.60e4}{\kilo\gram}
  rocket leaving the surface of Earth.
...
%  Both objects have the same escape speed:
%  \begin{center}
...
\begin{frame}{Escaping from Orbit}
  These two objects with mass $m$ have the same escape speed:
  \begin{center}
    \begin{tikzpicture}[scale=1.5]
      \node at (0,0) {\pic{.07}{earth-1200px}};
      \node[left] at (-.2,0) {$M$};
      \draw[gray!50] circle (1);
      \draw[mass] (0,1) circle (.04) node[above]{$m$};
      \draw[axes] (0,0)--(0,1) node[midway,right]{$r$};
      \fill (2,0) circle (.03);
      \draw[axes,dashed] (0,1) to[out=0,in=135] (2,0);
    \end{tikzpicture}
    \hspace{.3in}
    \begin{tikzpicture}[scale=1.5]
      \node at (0,0) {\pic{.283}{earth-1200px}};
      \node[left] at (0,0) {$M$};
      \draw[mass] (0,1) circle (.04) node[above]{$m$};
      \draw[axes] (0,0)--(0,1) node[midway,right]{$r$};
      \fill (2,0) circle (.03);
  ...
  Both are at a distance $r$ from the centre of the planet of mass $M$, but the
  one in orbit (left) already has orbital speed $v_\text{orbit}$, so escaping
  ...
  \begin{columns}[T]
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.4]
        \node at (0,0) {\pic{.3}{earth-1200px}};
        \draw[axes,magenta] (0,4) arc (90:-260:4);
        \draw[axes] (0,4)--+(2,0) node[right]{$\vec v_\text{orbit}$};
        \draw[mass] (0,4) circle (.1) node[above]{$P$};
        \draw[->] (0,0)--(0,4) node[pos=.6,right]{$r$};
      \end{tikzpicture}
    \end{center}
    
    {\scriptsize At orbital speed ($v=v_\text{orbit}$), the orbit is circular.
      Distance to the centre of the planet $r$ is constant\par}

    \column{.25\textwidth}
    \begin{center}
      \begin{tikzpicture}[scale=.4]
        \node at (0,0) {\pic{.3}{earth-1200px}};
        \draw[gray!50] circle (4);
        \draw[axes,violet,domain={0:4}] plot(\x,-.1*\x*\x+4);
        %node[below]{$v_\text{esc}$};
        \draw[axes] (0,4)--+(2.5,0) node[right]{$\vec v_\text{esc}$};
        \draw[mass] (0,4) circle (.1) node[above]{$P$};
        \draw[->] (0,0)--(0,4) node[pos=.6,right]{$r$};
      \end{tikzpicture}
    \end{center}

    {\scriptsize At escape speed ($v=v_\text{esc}$), the object leaves the planet
      in a parabolic path. The object will not return to the planet\par}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.4]
        \node at (0,0) {\pic{.3}{earth-1200px}};
        \draw[gray!50] circle (4);
        \draw[thick,cyan] (0,.5) ellipse (3.2 and 3.5);
        \draw[axes] (0,4)--+(1.7,0) node[right]{$\vec v$};
        \draw[mass] (0,4) circle (.1) node[above]{$P$};
        \fill (0,-3) circle (.1) node[below]{$P'$};
        \draw[->] (0,0)--(0,4) node[pos=.6,right]{$r$};
      \end{tikzpicture}
    \end{center}

    {\scriptsize\vspace{-.02in}At speed \emph{slightly below} orbital speed
      ($v<v_\text{orbit}$), the orbital path is elliptical. The object will be
      closer to the planet on the other side of the planet, at $P'$\par}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.4]
        \node at (0,0) {\pic{.3}{earth-1200px}};
        \draw[gray!50] circle (4);
        \draw[thick,orange] (0,-.5) ellipse (4.2 and 4.5);
        \draw[axes] (0,4)--+(2.2,0) node[right]{$\vec v$};
        \draw[mass] (0,4) circle (.1) node[above]{$P$};
        \fill (0,-5) circle (.08) node[below]{$P''$};
        \draw[->] (0,0)--(0,4) node[pos=.6,right]{$r$};
      \end{tikzpicture}
    \end{center}
    \vspace{-.17in}{\scriptsize At speed \emph{slightly above} orbital
      speed ($v_\text{orbit}<v<v_\text{esc}$), the orbital path is also
      elliptical. The object will gain altitude on the other side of the planet
      at $P''$\par}
  \end{columns}
\end{frame}
...
\begin{frame}{Orbital Kinetic Energy}
  We can obtain the \textbf{orbital kinetic energy} in a perfectly circular
  orbit by applying the orbital speed expression in our expression of kinetic
  energy:
  ...
  This equation uses the expression for orbital speed $v_\text{orbit}$,
  therefore it relies on the same assumption, i.e.\ a perfectly circular orbit
\end{frame}



\begin{frame}{Total Orbital Energy}
  Total orbital energy is the sum of kinetic and gravitational potential
  energies while in orbit:

  \eq{-.1in}{
    E_\text{orbit}={\color{cyan}K_\text{orbit}}+{\color{magenta}U_g}
  }
  
  Substituting the expressions for $K_\text{orbit}$ and $U_g$ derived earlier:

  \eq{-.1in}{
    E_\text{orbit}=
    {\color{cyan}\frac{GMm}{2r}} +
    {\color{magenta}{\left[-\frac{GMm}r\right]}}=
    \boxed{-\frac{GMm}{2r}}
  }
\end{frame}
...
\begin{frame}{Binding Energy}
  Binding energy $E_\text{binding}$ is the energy required to remove the mass
  $m$ from its orbit, and bring it to $r=\infty$, where total energy
  $E_\infty=0$. To do so, we need to make up for the deficit
...
\begin{frame}{Properties of Gravitational Force}
  Two properties of gravity are crucial to understanding of orbital mechanics:
  \begin{enumerate}
  \item Gravity is a \emph{conservative force}
  \item Gravity is a \emph{central force}
   \end{enumerate}
  These two properties are true regardless of the shape of the orbit, and even
  for objects that are not in orbit at all! 
\end{frame}
...
\begin{frame}{Gravitational Force as a Conservative Force}
%  Two properties of gravity are crucial to understanding of orbital mechanics:
%  \begin{enumerate}
  Gravity is a \textbf{conservative force}:
  \begin{itemize}
  \item Positive work done by gravity converts gravitational potential energy
    $U_g$ into kinetic energy $K$ by the same amount;
  \item Negative work done by gravity converts $K$ into $U_g$
  \item Work done by gravity is path independent
  \item In the absense of any friction, drag or other external forces, the
    total mechanical energy of the star-planet system is constant
  \item The star-planet system is an isolated system
  \end{itemize}
\end{frame}
...
\begin{frame}{Gravitational Force as a Conservative Force}
  Gravity is a \textbf{central force}, in that
  \begin{itemize}
  \item Gravitational force $\vec F_g$ is always in the $-\hat r$ direction,
    i.e.\ $\vec F\times\vec r=\vec 0$
  \item Therefore gravity doesn't generate any torque
...
%    $m$.
%  \item $m$ does not actually orbit about the centre of $M$, but rather, the
%    centre of mass between $M$ and $m$.
%  \end{itemize}
%\end{frame}
...
\begin{frame}{Reality of Orbital Motion}
  ...
  \begin{itemize}
  \item Most AP C problems will be circular instead of elliptical, but you must
  ...
  \item The analysis on the slides shown assumes a small mass $m$ orbiting
    ...
    \item Just as planets experience a gravitational force by the Sun, the Sun
      experiences a gravitational force from the planets (3rd law of motion)
...
%  When two objects are orbiting each other, the centre of mass of the system is
...
%  centre of mass, and the relative position $\vec r$ velocity $\vec{v}$ and
