  ...
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=.6]
      ...
      \begin{scope}[rotate=-atan(3/4)]
        ...
        \draw (4,-.5)--(4,5.5)
        node[pos=0,right]{line of action of $\vec F$};
        \draw (0,-1)--(0,2);
...
\begin{frame}{But there is no rotational motion, is there?}
  ...
  \uncover<4>{
    ...
    ($\vec L=m(\vec r\times\vec v)$) are constant.
  }
\end{frame}
...
\begin{frame}{Mechanical Work}
  ...
  \eq{-.1in}{
    W_\text{trans}=\int_{x_1}^{x_2}F\dl x
  }

  For rotational motion, mechanical work is defined similarly as:

  \eq{-.1in}{
    W_\text{rot}=\int_{x_1}^{x_2}F\dl x=\int_{\theta_1}^{\theta_2}F(r\dl\theta)
    \quad\to\quad
    \boxed{
  ...
  All we have done is to replace $F\to\tau$ and $\dl x\to\dl\theta$
...
\begin{frame}{Rotational Kinetic Energy}
  ...
  \eq{-.1in}{
    =I\int\diff{\theta}t\dl\omega=m\int_{\omega_0}^{\omega_1}\omega\dl\omega
  }
...
\begin{frame}{Work-Energy Theorem}
  Like the translational kinetic energy, the definition of emph{rotational}
  kinetic energy came from this integration, in that net \emph{rotational} work
  equals to the change in \emph{rotational} kinetic energy:
  ...
  \begin{itemize}
  \item $K_\text{rot}$ increases ($\Delta K_\text{rot}>0$) if net work is
    positive ($W_\text{net,rot}>0$); the object's angular speed increases
  \item $K_\text{rot}$ decreases ($\Delta K_\text{rot}<0$) if net work is
    negative ($W_\text{net,rot}>0$); the object's angular speed decreases
  \item It does not matter \emph{what} the net torque consists of
  \end{itemize}
\end{frame}
...
\begin{frame}{Rotational Kinetic Energy}
  ...
  \eq{-.1in}{
    K_\text{rot}=\sum_i\frac12m_iv_i^2=\sum_i\frac12m_i(r_i\omega)^2
    =\frac12\left[\sum_i m_ir_i^2\right]\omega^2 = \frac12I\omega^2
  }
...
\begin{frame}{Power}
  ...
  which energy is being transformed. For a rotational system:
  ...
  It is defined the same way as it was in the translational system:
\end{frame}
...
\begin{frame}{Conservation of Momentum}
  ...
  linear momentum of the total system is conserved:

  \eq{-.1in}{
    \sum\vec p_i=\sum\vec p_i'
  }
  
  But if there is no external force, there is also no external torque as well,
  therefore, the total \emph{angular} momentum must also be conserved in a
  collision:
