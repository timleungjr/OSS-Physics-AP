...
\begin{frame}{Oscillatory Systems}
  In Classes \#3 and \#4 (Work and Energy), we studied several oscillatory
  systems using the law of conservation of energy. But it is clear that the law
  of conservation of energy does not tell us \emph{why} the masses oscillate.
  ...
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=.85]
      \draw[mass] (3,.5) rectangle +(1,1) node[midway]{$m$};
      \draw[thick,
        decoration={aspect=.3,segment length=6, amplitude=2.5mm, coil},
        decorate] (0,1)--(3,1)  node[midway,above=7]{$k$};
    ...
    \begin{tikzpicture}
      \draw[mass] (.7,1.9) rectangle +(.6,.6) node[midway]{$m$};
      \draw[thick,
        decoration={aspect=.3,segment length=6, amplitude=2.5mm, coil},
        decorate] (1,5)--(1,2.5) node[midway,left=7]{$k$};
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=15]
        ...
        \shade[ball color=blue] (0,-3) circle (.2)  node[below=5]{$m$};
      \end{scope}
      \draw[dashed,thin] (0,0)--(0,-3);
    \end{tikzpicture}

    \vspace{-.3in}\begin{displaymath}
      K+U_g=\text{constant}
    \end{displaymath}
  \end{columns}  
\end{frame}
...
\begin{frame}{Oscillatory Systems}
  ...
  systems. These types of oscillatory motion is called \textbf{harmonic motion}
\end{frame}
...
\begin{frame}{Horizontal Spring-Mass System}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.1]
      ...
      node[above,text width=1.5cm,align=center]
      {\scriptsize equilibrium\\(unstretched)\\$x=0$\par};
...
\begin{frame}{Horizontal Spring-Mass System}
  ...
  %  }
  %  
  %  Starting with the general form and
  We can take the time derivatives of $x(t)$ to obtain the velocity and
  acceleration of the mass as functions of time:
  ...
  amplitude, and $\theta_0$ is a phase constant, based on initial conditions.
  This kind of motion is called a \textbf{simple harmonic motion}, and a
...
\begin{frame}{Horizontal Spring-Mass System: Angular Frequency}
  ...
  %In many physics textbooks,
  The term ``natural frequency'' may refer to either $\omega_0$ or $f_0$
...
\begin{frame}{Periodic vs.\ Simple harmonic}
  A simple harmonic motion is strictly periodic, but not all periodic motions
  are simple harmonic motions. To ``qualify'' a periodic motion as simple
  harmonic, we require a restoring force in the form of Hooke's law.
\end{frame}
...
\begin{frame}{Relationship to Circular Motion}
  ...
  \begin{columns}
    ...
    \item An object moving in uniform circular motion with radius $R$
      and constant angular frequency $\omega_0$. The angular position
      $\theta(t)$ of the object is:
...
\begin{frame}{Displacement, Velocity and Acceleration}
  \begin{columns}
    ...
    \begin{align*}
      ...
      a(t)&=-A\omega_0^2\cos(\omega_0t)=-\omega_0^2x
    \end{align*}
  ...
  \vspace{-.2in}\begin{columns}[T]
    ...
    \begin{center}
      \begin{tikzpicture}[scale=.95]
        ...
        \draw[vectors] (2.5,1.65)--(4,1.65) node[midway,above]{
          \scriptsize$x(0)=+A$};
    ...
    \begin{align*}
      x(t)&=A\cos(\omega_0t)\\
      v(t)&=-A\omega\sin(\omega_0t)\\
      a(t)&=-A\omega^2\cos (\omega_0t)
    \end{align*}    
    \column{.47\textwidth}
    \begin{center}
      \begin{tikzpicture}[scale=.95]
    ...
    \begin{align*}
      x(t)&=A\sin(\omega_0t)\\
      v(t)&=A\omega\cos(\omega_0t)\\
      a(t)&=-A\omega^2\sin(\omega_0t)
    \end{align*}
  \end{columns}
\end{frame}
...
\begin{frame}{Maximum Speed}
  ...
  \eq{-.1in}{
    v(t) =-\underbracket[1pt]{A\omega_0}_{v_\text{max}}\sin(\omega_0 t)
  }

  \vspace{-.05in}Substituting the expression for $\omega_0=\sqrt{k/m}$, we get
  
  \eq{-.1in}{
    v_\text{max}=A\omega_0=A\sqrt{\frac km}
  }
...
\begin{frame}{Maximum Acceleration}
  ...
  \eq{-.1in}{
    a(t)=-\underbracket[1pt]{A\omega_0^2}_{a_\text{max}}\cos(\omega_0t)
  }

  \vspace{-.05in}Again, substituting the expression for $\omega_0=\sqrt{k/m}$,
  we get:
  
  \eq{-.1in}{
    a_\text{max}=A\omega_0^2=A\frac km
  }
...
\begin{frame}{Side Note \#1}
  \textbf{Side Note \#1:} For anyone with more experiences with calculus, you
  will know that the solution to any ODE is the linear combination of
  ...
  \eq{-.1in}{
    x(t)=c_1\sin(\omega_0t)+c_2\cos(\omega_0t)
  }
...
\begin{frame}{Vertical Spring-Mass System}
  \begin{columns}
    \column{.27\textwidth}
    \input{vertical-spring-mass}

    \column{.73\textwidth}
    For a vertical spring-mass system, we must consider
    {\color{orange}weight $F_g=mg$} as well:

    \eq{-.1in}{
      {\color{orange}F_g}-kx=m\diff[2]xt
    }
    ...
    \vspace{-.3in}{\large
      \begin{align*}
        x(t) &= A\cos(\omega_0t+\theta_0) +B\\
        v(t) &= -A\omega_0\sin(\omega_0t+\theta_0)\\
        a(t) &= -A\omega_0^2\cos(\omega_0t+\theta_0)
      \end{align*}
    }
  \end{columns}
\end{frame}
...
\begin{frame}{Vertical Spring-Mass System}
  \begin{columns}
    ...
    Angular frequency (natural frequency) $\omega_0$, frequency $f_0$ and
    period $T_0$ remains the same as the horizontal case:

    \eq{-.1in}{
      \boxed{\omega_0=\sqrt{\frac km}}\quad
      \boxed{f_0=\frac1{2\pi}\sqrt{\frac km}}\quad
      \boxed{T_0=2\pi\sqrt{\frac mk}}
    }
  \end{columns}
\end{frame}
...
\begin{frame}{What About a Simple Pendulum?}
  \begin{columns}
    ...
    \item It has been shown in Class \#7 (circular motion) that when the mass is
...
\begin{frame}{Solutions to the ODE for the Simple Pendulum}
  Again, whether the solution for $\theta(t)$ is a cosine or a sine
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=25]
        ...
        node[below=6]{$m$} node[above right]{$v(0)=0$};
      \end{scope}
    ...
    \begin{tikzpicture}
      ...
      \shade[ball color=blue] (0,-3.5) circle (.2) node[below=6]{$m$};
      \draw[vectors] (0,-3.5)--+(1,0) node[right]{$v(0)=v_\text{max}$};
...
\begin{frame}{Velocity and Acceleration}
  ...
  \vspace{-.05in}The time derivative $\dot\theta$ gives us its angular
...
\begin{frame}{The meaning of $\omega_0$}
  ...
  \begin{columns}
    ...
    \column{.73\textwidth}
    The meaning of $\omega_0$ for the pendulum is more abstract than in
    ...
    \item Since we are using a small-angle approximation, we can ``project'' the
      ...
      \vspace{-.3in}{\large
        \begin{align*}
          ...
          &\approx\ell\theta_\text{max}\cos(\omega_0 t)
        \end{align*}
      }

      \vspace{-.05in}$x(t)$ is the pendulum's \textbf{lateral displacement}.
      It is also a simple harmonic motion
      
    \item We can relate the oscillation on the $x$ axis to a uniform circular
      motion on the $xy$-plane. $\omega_0$ is the angular frequency of this
      circular motion.
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Damped Oscillators}
  ...
  approximate the \emph{average}. Whether this is \emph{accurate} will depend
  on the specific problem that needs to be solved.
\end{frame}
...
\begin{frame}{Damped Oscillators}
  ...
  \eq{-.1in}{
    \boxed{
      x(t)=\underbracket[1pt]{A{\color{magenta}e^{-\lambda t}}}_\text{amplitude}
      {\color{cyan}\cos(\omega t+\theta_0)}
    }
    \quad\text{\normalsize where}\quad
    \lambda=\frac b{2m}
  }

  the ``natural frequency''\footnote{Since the motion is not strictly periodic,
  this should more accurately be called a ``quasi-frequency'', but no one does
  that.} for the damped oscillator is given by:

  \eq{-.1in}{
    \omega=\sqrt{\omega_0^2-\lambda^2}
  }
  \begin{itemize}
  \item\vspace{-.05in}Note that $\omega<\omega_0$ (natural frequency decreases)
    because of the damping factor $b$
  \item As $b$ increases, $\omega$ decreases
  \item The motion of the damped oscillator is \textbf{quasi-periodic}
  \end{itemize}
  \vspace{.1in}
\end{frame}
...
\begin{frame}{Under-Damped Oscillator}
  \centering
  \begin{tikzpicture}
    \begin{axis}[
        width=4in,
...
\begin{frame}{Critically-Damped System}
  \textbf{Critical damping} occurs when the damping constant is large enough
  that the natural frequency $\omega$ is zero, i.e.:
  
  \eq{-.1in}{
    \sqrt{\omega_0^2-\lambda_c^2}=0
    \quad\text{\normalsize where}\quad
    \lambda_c=\frac{b_c}{2m}
  ...
  \eq{-.1in}{
    b_c=2m\omega_0=\sqrt{4mk}
  }

  Critical or near-critical damping is desired in many engineering designs
...
\begin{frame}{Critically-Damped System}
  ...
  At critical damping, the solution for $x(t)$ is the sum of two exponential
  decay functions:

  \eq{-.1in}{
    \boxed{
      x(t)=(c_1+c_2t)e^{-\lambda_c t}
    }
  }
...
%  }
%
  The coefficients $c_1$ and $c_2$ depend on initial conditions. A critically
  damped system returns to its equilibrium position in the shortest time with
  \emph{no} oscillation
\end{frame}



\begin{frame}{Critically-Damped System: Finding the Coefficients}
  To illustrate the process in finding the coefficients, following the
  previous examples, if the mass starts from rest at $A_0$:
  \begin{displaymath}
    x(0)=(c_1+\cancel{c_2t})\cancel{e^{-\lambda t}}=A_0 \quad\to\quad c_1=A_0
  \end{displaymath}
  Taking the derivative with respect to time:
  \begin{displaymath}
    v(t)=-(c_2\lambda t +c_1\lambda -c_2)e^{-\lambda t}
    \quad\to\quad
...
\begin{frame}{Comparing Damped System}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[
        samples=200,
        domain=0:1.6]{1.005*exp(-x)*cos(deg(sqrt(99)*x)-5.74)};
      \addplot[
        samples=200,
        domain=0:1.6,dash dot]{1.048*exp(-3*x)*cos(deg(sqrt(91)*x)-17.46)};
      \addplot[
        samples=200,
        domain=0:1.6,dashed]{1.154*exp(-5*x)*cos(deg(sqrt(75)*x)-30)};
      \addplot[
        color=red!80!black,
        samples=100,
        domain=0:1.6,very thick]{(1+10*x)*exp(-10*x)};
    \end{axis}
  \end{tikzpicture}
\end{frame}


\begin{frame}{Over-Damped System}
  When $b>b_c$, the system is \textbf{over-damped}. The solution for $x(t)$ is
  a slow exponential decay function:
  ...
  We can easily verify that both $r_1$ and $r_2$ are real and negative. The
  coefficients $c_1$ and $c_2$ also depend on initial conditions. 
\end{frame}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[
        color=red!80!black,
        samples=100,
        domain=0:1.6]{(1+10*x)*exp(-10*x)};
      \addplot[
        color=cyan,
        samples=100,
        domain=0:1.6,
        very thick]{1.077*exp(-2.679*x)-0.077*exp(-37.32*x};
    \end{axis}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Forced Harmonic Motion}
  ...
  \eq{-.1in}{
    \boxed{x(t)=A\cos(\omega_\text{ext}t+\theta_0)}
  }
  
  where the amplitude of the oscillation $A$ and phase shift $\theta_0$ are
  given by:

  \eq{-.05in}{
    A=\frac F{\sqrt{m^2(\omega_0^2-\omega_\text{ext}^2)^2+b^2\omega_\text{ext}^2}}
    \quad\quad
    \tan\theta_0=\frac{b\omega_\text{ext}}{m(\omega_0^2-\omega_\text{ext}^2)}
  }
  \vspace{.5in}
\end{frame}
...
\begin{frame}{Resonance}
  ...
  \vspace{.05in}{\scriptsize A note about how this was plotted. The functions
    plotted here are $A(\omega_\text{ext})$ from the previous slide, using
    $F=1$, $m=1$, $k=1$, which gives us $\omega_0$. We are also setting
    $\omega_0\approx\omega$.\par}
\end{frame}
...
\begin{frame}{Resonance}
  \eq{0in}{
    \boxed{\tan\theta_0=\frac{b\omega_a}{m(\omega_0^2-\omega_\text{ext}^2)}}
  }
  ...
  phase shift of $\theta_0=\pi/2$. Taking derivative of $x(t)$ for velocity
  $v(t)$, and substituting $\theta_0=\pi/2$:
