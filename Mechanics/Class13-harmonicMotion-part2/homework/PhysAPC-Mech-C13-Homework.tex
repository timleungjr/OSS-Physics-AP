...
\begin{questions}
  ...
  \end{choices}
    ...
    %can be described as
    %\begin{choices}
    %  \choice harmonic motion with a period of \SI2\second
    %  \choice harmonic motion with a period of \SI1\second
  ...
  \question A pendulum of length $L$ has a period of $t$ on Earth. A planetary
  explorer takes the same pendulum of length $L$ to another planet where its
  period is $t/2$. The gravitational acceleration on the surface of this planet
  is most nearly
  \begin{choices}
    \choice $8g$
    \choice $4g$
...
%    \vspace{.7in}
  ...
  \uplevel{  
    \textbf{Questions \ref{first}--\ref{last}}: A harmonic oscillator follows
    the equation $\dfrac{d^2x}{dt^2}=-4x$. The spring constant $k$ is
    \SI4{\newton\per\metre}.
  }
...
%  (b)\hspace{5pt}
%  \begin{tikzpicture}[scale=.5]
%    \fill[gray!50] rectangle (12.75,-.75);
%    \fill[gray!50] (-.75,-.75) rectangle (0,3);
...
%  \newpage
  ...
  \begin{parts}
    \part In terms of the quantities given above, \textbf{write} but do NOT
    solve the differential equation that could be used to determine the angular
    displacement $\theta$ of the torsion pendulum as a function of time $t$.
    \vspace{\stretch1}
    
    \part Using the analogy to a mass oscillating on a spring,
    \textbf{determine} the period of the torsion pendulum in terms of the given
    quantities and fundamental constants, as appropriate.
    \vspace{\stretch1}
    \newpage
    ...
    \uplevel{
      ...
      \begin{center}
        \begin{tabular}{|c|c|c|c|}
          ...
          Average Time for Ten Oscillations [\si\second] &
          Period $T$ [\si\second] & $T^2$ [\si{\second\squared}] \\\hline
    ...
    \part On the graph below, \textbf{plot} the data points.
    \uplevel{
      \centering
      \begin{tikzpicture}[xscale=2.8]
        \draw[gray,xstep=.2,ystep=.25] grid (5.1,7.2);
        \draw[thick] grid (5.1,7.2);
        \draw[axes] (0,0)--(5.3,0) node[pos=0,below left]{$O$}
        node[midway,below=15]{
          $I$ of Disk [\si{\kilo\gram.\metre\squared}]};
        \draw[axes] (0,0)--(0,7.5) node[pos=.65,left=30,rotate=90]{
          $T^2$ [\si{\second\squared}]};
        \foreach \y in {2,4,...,14}
        \draw[thick] (0,\y/2)--+(-.05,0) node[left]{$\y$};
        \foreach \x in {0.02,0.04,0.06,0.08,0.10}
        \draw[thick] (\x*50,0)--+(0,-.1) node[below]{$\x$};
      \end{tikzpicture}
    }
    \part\textbf{Draw} a straight line that best represents the data.
    
    \part\textbf{Determine} the equation for your line.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the torsion constant $\beta$ of the rod from your
    line.
    \vspace{\stretch1}
    
    \part\textbf{Explain} the physical significance of the intercept of your
  ...
  \cpic{.5}{oscillation1}
  \begin{parts}
    \part\textbf{Determine} the equation for $v(t)$, including numerical values
    for all constants.
    \vspace{\stretch1}
    
    \part Given that the equilibrium position is at $x=0$, \textbf{determine}
    the equation for $x(t)$, including numerical values for all constants.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the value of $k$.
    \vspace{\stretch1}
    \newpage
    ...
    \uplevel{
      ...
      \begin{center}
        \begin{tikzpicture}
          ...
          \draw[vectors] (7,.8)--(6,0) node[pos=0,above]{Rough Surface};
          \draw[vectors] (5.9,2.1)--(5.1,1.5)
          node[pos=0,right]{Equilibrium Position};
          \draw[very thick,
    ...
    \part On the dots below that represent the block, \textbf{draw} and
    \textbf{label} all the forces (not components) that act on the block when
    the spring is \underline{compressed} a distance $x=d/2$ and the block is
    moving in the direction indicated below each dot.
    ...
    \part\vspace{.2in}\textbf{Sketch} $v$ versus $t$ in this case. Assume that
  ...
  \uplevel{
    ...
    \begin{tikzpicture}
      ...
      \fill[lightgray] (2.5,0) rectangle +(2,.2);
      \draw[very thick] (0,0)--+(2,0);
      \draw[very thick] (2.5,0)--+(2,0);
      \draw[thick] (1,0)--(1,-.2);
      \draw[thick,
        decoration={aspect=.3,segment length=4, amplitude=9,coil},
        decorate] (1,-.2)--(1,-1.8);
      \draw[thick] (1,-1.8)--(1,-2);
      
      \draw[thick] (3.5,0)--(3.5,-.3);
      \draw[thick,
        decoration={aspect=.3,segment length=6, amplitude=8.5,coil},
        decorate] (3.5,-.3)--(3.5,-2.7);
      \draw[thick] (3.5,-2.7)--(3.5,-3);

      \draw[line width=3] (2.25,-6.5)--(2.25,-3.4) arc (180:90:.4)--(4.35,-3)
      arc (90:0:.4)--(4.75,-6.5);

      \draw[thick,fill=lightgray] (2.1,-6.5) rectangle (4.9,-6.7)
      node[right]{$M$};

      \draw[dashed] (1.1,-2)--(1.8,-2);
      \draw[dashed] (1.1,-3)--(2.2,-3);
      \draw[thick,<->] (1.3,-2)--(1.3,-3) node[midway,right]{$D$};
      
      \draw[dashed] (2,-6.5)--(1,-6.5);
      \draw[dashed] (2,-4)--(1,-4);
      \draw[thick,<->] (1.5,-6.5)--(1.5,-4) node[midway,fill=white]{$H$};

      %\draw[thick,fill=lightgray] (3.5,-4) circle (.1) node[right]{$M$};
      ...
      \begin{scope}[thick]
        ...
        \draw[bend left=285, looseness=1.50] (2.center) to (0.center);
        \draw[bend left] (2.center) to (3.center);
        \draw[bend left=75] (3.center) to (1.center);
      \end{scope}
      \node at (3.9,-4){$M$};
    \end{tikzpicture}      
    %\pic{.3}{hung-spring}
  ...
  \begin{parts}
    \part\textbf{Determine} the speed of the clay at the instant it hits the
    pan.
    \vspace{\stretch1}
    
    \part\textbf{Determine} the speed of the pan just after the clay strikes it.
    \vspace{\stretch1}
    
    \part\textbf{Determine} the period of the simple harmonic motion that
    ensues.
    \vspace{\stretch1}
    \newpage
    
    \part\textbf{Determine} the distance the spring is stretched (from its
    initial unstretched length) at the moment the speed of the pan is a maximum.
    ...
    caught in midair before hitting anything else. \textbf{Indicate} whether the
    period of the resulting simple harmonic motion of the pan is greater than,
    less than, or the same as it was in part \textbf{C}.
