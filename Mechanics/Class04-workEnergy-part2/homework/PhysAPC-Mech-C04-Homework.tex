...
%    \end{choices}
  ...
  and spends a total time in the air of $T$, then returning to the original
  location, the average power of the gravitational force is
  \begin{choices}
    \choice $P=2mgh/T$
    \choice $P=-2mgh/T$
    \choice 0
  ...
  \end{choices}
    ...
    %in metres. The potential energy at $x=\SI2\metre$ is
    %\begin{choices}
    %  \choice zero
    %  \choice\SI{20}\joule
    %  \choice\SI{40}\joule
  ...
  $F(x)=2-4x$, where the force is measured in newtons and position in metres.
  ...
  $U(x)=2x^2+x-6$, where force is in newtons and displacement is in metres. A
  ...
  \question A particle of mass $m$ moves according to the displacement equation
  ...
  gravitational field. The net work against the gravitational field depends on
  the
  \begin{choices}
  ...
  \uplevel{
    ...
    \begin{center}
      \begin{tikzpicture}
        ...
        \draw[axes] (0,0)--(0,3) node[above]{$U(x)$};
        \draw[smooth,samples=30,domain=-2.5:2.5] plot(\x,{.4*\x*\x+1});
        \fill (0,1) circle (.07) node[below left]{$\frac12U_0$};
        \fill (0,2) circle (.07) node[left]{$U_0$};
        \draw[dashed,gray] (1.6,0)--(1.6,2) node[pos=0,below,black]{$x_0$}
        --(0,2);
      \end{tikzpicture}
    \end{center}
  }

  \question Assuming that no non-conservative forces are present, if a particle
  of mass $m$ is released from position $x_0$, what is the maximum speed it
  will achieve?
  \label{q:well1}
  \begin{choices}
    \choice $\sqrt{4U_0/m}$
...
\begin{questions}
  ...
  $x= 0.5t^3+2t$, where $x$ is in metres and $t$ is in seconds.
  \cpic{.3}{student}
  \begin{parts}
    \part\textbf{Determine} the speed of the box at time $t=0$.
    \vspace{\stretch1}
    
    \part\textbf{Determine} the kinetic energy of the box as a function of time
    $t$.
    \vspace{\stretch1}
      
    \part\textbf{Determine} the net force acting on the box as a function of
    time $t$.
    \vspace{\stretch1}

    \part\textbf{Determine} the power being delivered to the box as a function
    of time $t$.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the net work done on the box in the interval $t=0$
    to $t=\SI2\second$.
    \vspace{\stretch1}
  \end{parts}
  \newpage
...
%  \newpage
  ...
  $U(x)=4.0x^2$, where $U$ is in joules and $x$ is in metres. When the object
  passes the point $x=\SI{-0.50}\metre$, its velocity is
  $+$\SI{2.0}{\metre\per\second}. All forces acting on the object are
  conservative.
  \begin{parts}
    \part\textbf{Calculate} the total mechanical energy of the object.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the $x$-coordinate of any points at which the
    object has zero kinetic energy.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the magnitude of the momentum of the object at
    $x=\SI{.60}\metre$.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the magnitude of the acceleration of the object as
    it passes $x=\SI{.60}\metre$.
    \vspace{\stretch1}
    \newpage
    
    \part On the axes below, \textbf{sketch} graphs of the object's position
    $x$ versus time $t$ and kinetic energy $K$ versus time $t$. Assume that
    $x=0$ at time $t=0$. The two graphs should cover the same time interval
  ...
  \end{center}
  \begin{parts}
    \part Assuming no energy is lost, \textbf{derive} the equation for
    conservation of mechanical energy that would apply to this situation.
    \vspace{\stretch1}
    
    \part On the grid below, \textbf{plot} $v^2$ versus $x^2$. Label the axes,
    ...
    \part\textbf{Draw} a best-fit straight line through the data.
    \newpage
    
    \part Use the best-fit line to \textbf{determine} the mass $m$ of the
    glider.
    \vspace{\stretch1}

    \uplevel{
      The track is now tilted at an angle $\theta$ as shown below. When the
    ...
    \part Assuming no energy is lost, \textbf{derive} the new equation for
    conservation of mechanical energy that would apply to this situation.
    \vspace{\stretch1}
    
    \part\textbf{Indicate} whether the graph of $v^2$ versus $x^2$ for this new
    experiment be a straight line:
  ...
  \begin{parts}
    ...
    \part\textbf{Outline} a procedure for performing the experiment. Include a
    diagram of your experimental setup. Label the equipment in your diagram.
    Also include a description of the measurements you would make and a symbol
