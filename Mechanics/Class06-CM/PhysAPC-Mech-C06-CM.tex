\documentclass[12pt,compress,aspectratio=169]{beamer}
\input{../../mybeamer}

\title{Class 6: Centre of Mass}
\subtitle{AP Physics C--Mechanics}
\input{../../me}
\input{../../mycommands}

\begin{document}

\begin{frame}
  \maketitle
\end{frame}



\begin{frame}{Centre of Mass}
  Finding an object's centre of mass is important, because
  \begin{itemize}
  \item The laws of motion are formulated by treating objects as point masses
    (for real-life objects, we let the forces apply to the centre of mass)
  \item Objects can have \emph{rotational} motion in addition to
    \emph{translational} motion as well (we will examine that a bit more in a
    very-important topic later)
  \end{itemize}
\end{frame}



\begin{frame}{Start with a Definition}
  The \textbf{centre of mass} (``CM'') is the \emph{weighted average of the
  masses in a system.} The ``system'' may be:
  \begin{itemize}
  \item A collection of individual particles
  \item A continuous distribution of mass with constant density. In this case,
    CM is also the geometric centre (\textbf{centroid}) of the object
  \item A continuous distribution of mass with varying density
  \item If the masses are inside a \emph{uniform} gravitational field, then the
    CM is also its \textbf{centre of gravity} (``CG'')
  \end{itemize}
\end{frame}


\section{Finding the Centre of Mass}

\begin{frame}{Simple Example}
  Two equal masses $m$ along the $x$-axis, located at $x_1$ and $x_2$. Where is
  the centre of mass of the system?
  \begin{center}
    \begin{tikzpicture}
      \draw[axes] (-1,0)--(8,0) node[right]{$x$};
      \draw[mass] (2,0) circle (.2) node{$m$};
      \draw[mass] (6,0) circle (.2) node{$m$};
      \draw[thick] (0,.85)--(0,-.45) node[below]{$O$};
      \fill (4,0) circle (.05) node[above]{cm};
      \begin{scope}[very thick,->|]
        \draw (0,.35)--(2,.35) node[pos=0,left]{$x_1$};
        \draw (0,.75)--(6,.75) node[pos=0,left]{$x_2$};
        \draw[violet] (0,-.3)--(4,-.3) node[pos=0,left]{$x_\text{cm}$};
      \end{scope}
    \end{tikzpicture}
  \end{center}
  The centre of mass is at the half-way point between the masses:

  \eq{-.1in}{
    x_\text{cm}=\frac{x_1+x_2}2
    \quad\quad\text{or}\quad\quad
    x_\text{cm}=\frac{mx_1+mx_2}{2m}
  }
\end{frame}



\begin{frame}{Slightly More Challenging}
  What if one of the masses are increased to $2m$? This is still not a
  difficult problem; you can still \emph{guess} the right answer without
  knowing the equation for centre of mass. 
  \begin{center}
    \begin{tikzpicture}
      \draw[axes] (-4,0)--(4,0) node[right]{$x$};
      \draw[mass] (-2.5,0) circle (.3) node{$m$};
      \draw[mass] (2.5,0) circle (.42) node{$2m$};
      \fill (5/6,0) circle (.05) node[below]{cm};
    \end{tikzpicture}
  \end{center}
  The answer is still simple. The centre of mass is no longer half way between
  the two masses, but now $\frac13$ the total distance from the larger masses.
  We can show using a weighted average:
  
  \eq{-.1in}{
    x_\text{cm}=\frac{mx_1+(2m)x_2}{m+2m}
  }
\end{frame}



\begin{frame}{Many Point Masses}
  The weighted average concept can now be applied to cases when there are
  masses in two or more dimensions:
  \begin{center}
    \begin{tikzpicture}
      \draw[axes] (-3,0)--(3,0) node[right]{$x$};
      \draw[axes] (0,-1.5)--(0,1.5) node[above]{$y$};
      \draw[mass] (-1.3,1) circle (.4) node{$m_1$};
      \draw[mass] (-1.5,-.5) circle (.3) node{$m_2$};
      \draw[mass] (1,.3) circle (.25) node{$m_3$};
      \draw[mass] (0,.3) circle (.2) node{$m_4$};
      \draw[mass] (2,-1) circle (.25) node{$m_5$};
    \end{tikzpicture}
  \end{center}
\end{frame}



\begin{frame}{Centre of Mass Equation}
  For discrete number of point masses, the centre of mass is defined
  as the weighted average:

  \eq{-.2in}{
    \boxed{
      \vec x_\text{cm}=\frac{\sum_{i=1}^N \vec x_i m_i}{\sum_{i=1}^N m_i}
    }
  }
  where $\sum m_i$ in the denominator is the total mass of the system of objects
  \begin{center}
    \begin{tabular}{l|c|c}
      \rowcolor{pink}
      \textbf{Quantity} & \textbf{Symbol} & \textbf{SI Unit} \\ \hline
      Position of centre of mass (vector) & $\vec x_\text{cm}$ & \si\metre \\
      Position of point mass $i$ (vector) & $\vec x_i$ & \si\metre \\
      Point mass $i$ & $m_i$ & \si{\kilo\gram}
    \end{tabular}
  \end{center}
  Components:

  \eq{-.1in}{
    x_\text{cm}=\frac{\sum x_i m_i}{\sum m_i}\quad\quad
    y_\text{cm}=\frac{\sum y_i m_i}{\sum m_i}\quad\quad
    z_\text{cm}=\frac{\sum z_i m_i}{\sum m_i}
  }
\end{frame}



\begin{frame}{An Example}
  \textbf{Example:} Consider the following masses and their coordinates
  which make up a ``discrete mass'' rigid body''
  \begin{align*}
    m_1&=\SI{5.0}{\kg} &\vec x_1&=3.0\iii-2.0\kkk\\
    m_2&=\SI{10.0}{\kg}&\vec x_2&=-4.0\iii+2.0\jjj+7.0\kkk\\
    m_3&=\SI{1.0}{\kg}&\vec x_3&=10.0\iii-17.0\jjj+10.0\kkk
  \end{align*}
  What are the coordinates for the centre of mass of this system?
\end{frame}



\begin{frame}{Continuous Mass Distribution}
  When the number of masses approaches infinity, this becomes a continuous
  distribution of mass. Taking the limit of masses $N\rightarrow\infty$ gives
  the integral form of our equation:

  \eq{-.1in}{
    \boxed{\vec x_\text{cm}=\frac{\int\vec x\dl m}{\int\dl m}}
  }

  What is the infinitesimal mass $\dl m$ then?
\end{frame}



\begin{frame}{Densities}
  Linear mass density (for 1D problems)

  \eq{-.1in}{
    \gamma(x) = \diff mx\quad\rightarrow\quad \dl m =\gamma(x)\dl x
  }

  Surface mass density (for 2D problems)

  \eq{-.1in}{
    \sigma(x,y)=\diff mA\quad\rightarrow\quad \dl m =\sigma(x,y)\dl A
  }

  Volume density (for 3D problems)

  \eq{-.1in}{
    \rho(x,y,z)=\diff mV\quad\rightarrow\quad \dl m =\rho(x,y,z)\dl V
  }
  %The densities do not have to be constant
\end{frame}



\begin{frame}[t]{Example: Finding the CM in a continuous mass distribution}
  A uniform thin rod of length $L$ with mass $M$ lies along the $x$-axis with
  the left edge located at $x=0$. What is the $x$ coordinate of its centre of
  mass?
  \begin{center}
    \begin{tikzpicture}[thick]
      \draw[axes] (0,0)--(8.5,0) node[right]{$x$};
      \draw (0,0)--+(0,-.3) node[below]{$O$};
      \draw (8,0)--+(0,-.3) node[below]{$L$};
      \draw[mass] (0,0) rectangle (8,.2);
    \end{tikzpicture}
  \end{center}
\end{frame}


\begin{frame}[t]{Example: Finding the CM in a continuous mass distribution}
  A thin rod of length $L$ lies along the $x$-axis with the left edge located
  at $x=0$. The rod's linear mass density varies linearly along the $x$-axis:
  $\gamma=\alpha x$ What is the $x$ coordinate of its centre of mass?
  \begin{center}
    \begin{tikzpicture}[thick]
      \draw[axes] (0,0)--(8.5,0) node[right]{$x$};
      \draw (0,0)--+(0,-.3) node[below]{$O$};
      \draw (8,0)--+(0,-.3) node[below]{$L$};
      \shade[left color=white,right color=cyan!50] rectangle (8,.2);
      \draw rectangle (8,.2);
    \end{tikzpicture}
  \end{center}
\end{frame}



\begin{frame}{Example: Finding the CM in a continuous mass distribution}
  On first glance, you may think that the last example is just a math problem,
  but think about what object would have a mass distribution that varies
  linearly! In fact, what we are solving for is the centroid of a triangle:
  \begin{center}
    \begin{tikzpicture}
      \draw[axes] (0,0)--(8.5,0) node[right]{$x$};
      \draw[axes] (0,-.5)--(0,.5) node[above]{$y$};
      \draw[mass] (0,0)--(8,.3)--(8,-.3)--cycle;
      \draw[thick] (5.33,0) circle (.15);
      \fill (5.48,0) arc (0:90:.15)--(5.33,0);
      \fill (5.18,0) arc (180:270:.15)--(5.33,0);
    \end{tikzpicture}
  \end{center}
  which is $1/3$ the height of the triangle from the base, or a distance of
  $2L/3$ from the top of the triangle.
\end{frame}



%\begin{frame}{An Example with Integrals}
%  \begin{columns}
%    \column{.65\textwidth}
%    \textbf{Example:} A triangular plate is placed in a Cartesian coordinate
%    system with two of its edges along the $x$ and $y$-axis. The length of the
%    edges along the axes are $a$ and $b$ respectively. Assuming that the
%    surface area density $\sigma$ is uniform, determine the coordinate of its
%    centre of mass.
%
%    \column{.35\textwidth}
%    \begin{tikzpicture}[scale=.8]
%      \draw[axes] (0,0)--(4,0) node[right]{$x$};
%      \draw[axes] (0,0)--(0,5) node[above]{$y$};
%      \draw[fill=lightgray] (0,0)--(3,0) node[midway,below]{$a$}
%      --(0,4)--cycle node[midway,left]{$b$};
%      \uncover<2->{
%        \draw[fill=red] (1,0)--(1.2,0)--(1.2,2.4)--(1,2.67)
%        node[midway,right]{$\dl m=y\dl x$}--cycle;
%      }
%    \end{tikzpicture}
%  \end{columns}
%\end{frame}



\begin{frame}{Centroid}
  For objects with uniform mass distribution (i.e.\ constant density), the CM
  is also its \textbf{centroid}. The locations of centroids can be found in
  most physics and math textbooks (no need to remember for this course). Here
  are some 2D examples:
  \begin{center}
    \begin{tikzpicture}[scale=.45]
      \draw[mass] rectangle (7,4) node[midway,above=35]{Rectangular Area};
      \draw[<->] (-.3,0)--+(0,4) node[midway,left=-1]{\small $a$};
      \draw[<->] (0,-.3)--+(7,0) node[midway,below=-1]{\small $b$};
      
      \draw[<->] (3.5,0)--+(0,2) node[midway,right=-1]{\small $\frac a2$};
      \draw[<->] (0,2)--+(3.5,0) node[midway,above=-1]{\small $\frac b2$};

      \fill[red] (3.5,2) circle (.08) node[right]{\small cm};
    \end{tikzpicture}
    \begin{tikzpicture}[scale=.45]
      \draw[mass] (0,0)--(5.5,4)--(8,0)--(0,0);
      \draw (5.5,0)--+(0,4);
      \node[above] at (4,4) {Triangular Area};
      \draw[<->] (-.3,0)--+(0,4) node[midway,fill=black!2]{\small $h$};
      \draw[<->] (0,-.3)--+(5.5,0) node[midway,below=-1]{\small $b$};
      \draw[<->] (5.5,-.3)--+(2.5,0) node[midway,below=-1]{\small $a$};
      
      \draw[<->] (11.5/3,0)--+(0,4/3) node[midway,left=-1]{\small $\frac h3$};
      \draw[<->] (11.5/3,4/3)--(8,4/3) node[midway,above=-1]{
        \small $\frac{b+2a}3$};
      \draw (8,0)--(8,1.5);
      \draw (-.5,4)--(6,4);
      \fill[red] (11.5/3,4/3) circle (.08) node[above]{\small cm};
    \end{tikzpicture}
    \begin{tikzpicture}[scale=.45]
      \draw[mass] (0,0)--(0,4.5)--(7,3)--(7,0)--(0,0);
      \node[above] at (4.5,4) {Trapezoidal Area};
      \draw[<->] (-.3,0)--+(0,4.5) node[midway,left=-1]{\small $a$};
      \draw[<->] (7.3,0)--+(0,3) node[midway,right=-1]{\small $b$};
      \draw[<->] (0,-.3)--+(7,0) node[midway,below=-1]{\small $L$};
      
      \draw[<->] (3.3,0)--+(0,1.9) node[midway,right=-1]{
        \small $\frac{a^2+ab+b^2}{3(a+b)}$};
      \draw[<->] (0,1.9)--+(3.3,0) node[midway,above=-1]{
        \small $\frac{L(a+2b)}{2(a+b)}$};

      \fill[red] (3.3,1.9) circle (.08) node[right]{\small cm};
    \end{tikzpicture}

    \begin{tikzpicture}[scale=.5]
      \draw[mass] circle (2.5);
      \node[above] at (0,2.5) {Circular Area};
      \draw[->,rotate=-50] (0,0)--(2.5,0) node[midway,above=-1]{\small$r$};
      \fill[red] circle (.08) node[left]{\small cm};
    \end{tikzpicture}
    \hspace{.1in}
    \begin{tikzpicture}[scale=.5]
      \draw[black!2] circle (2.5);
      \draw[mass] (2.5,0) arc (0:180:2.5)--(2.5,0);
      \node[above] at (0,2.5) {Semi-Circular Area};
      \draw[dash dot] (0,0)--(0,2.7);
      \draw[<->] (0,0)--+(0,1.1) node[midway,right=-1]{\small$\frac{4r}{3\pi}$};

      \draw[->,rotate=-45] (0,0)--(-2.5,0) node[midway,above=-1]{\small$r$};
      \fill[red] (0,1.1) circle (.08) node[above]{\small cm};
    \end{tikzpicture}
    \hspace{.1in}
    \begin{tikzpicture}[scale=.5]
      \draw[black!2] circle (2.5);
      \draw[mass] (0,0)--(0,2.5) arc (90:180:2.5)--(0,0);
      \node[above] at (-1.1,2.5) {Quarter-Circular Area};
      \draw (-1.1,-.5)--(-1.1,1.1)--(.5,1.1);
      
      \draw[<->|] (.3,1.1)--(.3,0) node[midway,right=-1]{
        \small$\frac{4r}{3\pi}$};
      \draw[<->|] (-1.1,-.3)--(0,-.3) node[midway,below=-1]{
        \small$\frac{4r}{3\pi}$};
      
      \draw[->,rotate=-45] (0,0)--(-2.5,0) node[midway,above=-1]{\small$r$};
      \fill[red] (-1.1,1.1) circle (.08) node[left]{\small cm};
    \end{tikzpicture}
  \end{center}
\end{frame}




\begin{frame}{Compound Shapes}
  For compound shapes, the centre of mass is the weighted average of the centre
  of mass of each component. For example, for the T-beam below:
  \begin{center}
    \begin{tikzpicture}[scale=.7]
      \draw[axes] (0,0)--(4.5,0) node[right]{$x$};
      \draw[axes] (0,0)--(0,4.5) node[left]{$y$};
      \draw[blue!70!black,mass] (0,4)--(4,4)--(4,3)--(2.5,3)
      --(2.5,0)--(1.5,0)--(1.5,3)--(0,3)--(0,4);
    \end{tikzpicture}
    \hspace{.1in}
    \begin{tikzpicture}[scale=.7]
      \draw[lightgray,fill=gray!20,thick] (0,4)--(4,4)--(4,3)--(2.5,3)--(2.5,0)
      --(1.5,0)--(1.5,3)--(0,3)--(0,4);
      \draw[lightgray,dotted,thick] (1.5,3)--(2.5,3);
      \draw[axes] (0,0)--(4.5,0) node[right]{$x$};
      \draw[axes] (0,0)--(0,4.5) node[left]{$y$};
      \fill[blue!70!black] (2,3.5) circle (.08) node[right]{$\vec x_1$};
      \fill[blue!70!black] (2,1.5) circle (.08) node[right]{$\vec x_2$};
    \end{tikzpicture}
    \hspace{.1in}
    \begin{tikzpicture}[scale=.7]
      \draw[lightgray,thick] (0,4)--(4,4)--(4,3)--(2.5,3)
      --(2.5,0)--(1.5,0)--(1.5,3)--(0,3)--(0,4);
      \draw[axes] (0,0)--(4.5,0) node[right]{$x$};
      \draw[axes] (0,0)--(0,4.5) node[left]{$y$};
      \fill[blue!30] (2,3.5) circle (.08) node[right]{$\vec x_1$};
      \fill[blue!30] (2,1.5) circle (.08) node[right]{$\vec x_2$};
      \fill[red!70!black] (2,2.64) circle (.08) node[right]{$\vec x_\text{cm}$};
    \end{tikzpicture}
  \end{center}
\end{frame}


%\begin{frame}{A Difficult Example to Try at Home}
%  Not typically an AP problem, this example shows how we can use integral to
%  find the centre of mass for something very complicated.
%  \begin{columns}
%    \column{.6\textwidth}
%    \textbf{Example 3:} Find the $x$-coordinate of the centre of mass in the
%    shape bound by the two functions shown on the right.
%
%    \column{.4\textwidth}
%    \begin{tikzpicture}[scale=3]
%      \draw[->](0,0)--(1.25,0) node[right]{ $x$};
%      \draw[->](0,0)--(0,1.25) node[above]{ $y$};
%      \draw[fill=green!40]
%      plot[smooth,samples=40,domain=0:1] (\x,{\x*\x*\x})--
%      plot[smooth,samples=40,domain=1:0] (\x,{\x^(.5)});
%      
%      \draw[red!70,thick]  plot[smooth,samples=40,domain=0:1] (\x,{\x*\x*\x});
%      \draw[blue!70,thick] plot[smooth,samples=40,domain=0:1] (\x,{\x^(.5)});
%      \node at (.4,.85){\textcolor{blue!70}{ $y=\sqrt{x}$}};
%      \node at (.9,.3){\textcolor{red!70}{ $y=x^3$}};
%    \end{tikzpicture}
%  \end{columns}
%\end{frame}



\begin{frame}{Symmetric Configurations}
  \begin{itemize}
  \item Any plane of symmetry, mirror line, axis of rotation, point of inversion
    \emph{must} contain the centre of mass.
  \item Caveat: only works if the density distribution is also symmetric
  \item Again: if density is uniform, CM is also geometric centre (centroid)
  \end{itemize}
\end{frame}



\begin{frame}{``Negative Mass'': A Mathematical Trick}
  \begin{itemize}
  \item Where there is a ``hole'' in the geometry, treat it as having negative
    mass density $-\sigma$ in that region.
  \item Negative masses don't exist, so this is really just a trick.
  \item\textbf{Example:} What is the centre of mass of this shape?
    \begin{center}
      \begin{tikzpicture}[scale=.9,thick]
        \draw[mass] circle (2);
        \draw[fill=black!2] (0,1) circle (1);
        \fill circle (.04);
        \draw[axes,rotate=45] (0,0)--(-2,0) node[midway,below]{$r$};
        \fill (0,1) circle (.04);
        \draw[axes,rotate around={-45:(0,1)}] (0,1)--(1,1) node[right]{$r/2$};
        \draw[axes](-2.4,0)--(2.4,0) node[right]{$x$};
        \draw[axes](0,-2.4)--(0,2.4) node[above]{$y$};
      \end{tikzpicture}
    \end{center}
  \end{itemize}
\end{frame}



\begin{frame}{Negative Mass Example}
  \begin{itemize}
  \item This is how we would think of it:
    \begin{center}
      \begin{tikzpicture}[scale=.55]
        \draw[mass] circle (2);
        \draw[thick,fill=black!2] (0,1) circle (1);
        \draw[axes] (-2.2,0)--(2.5,0) node[right]{$x$};
        \draw[axes] (0,-2.2)--(0,2.5) node[above]{$y$};
        \fill circle (.06);
        \draw[axes,rotate=45] (0,0)--(-2,0) node[pos=1.1]{\small$r$};
        \fill (0,1) circle (.06);
        \draw[axes,rotate around={-45:(0,1)}] (0,1)--(1,1)
        node[right]{\small$r/2$};

        \draw[mass] (7,0) circle (2) node[above right]{$A$};
        \draw[axes] (4.7,0)--(9.5,0) node[right]{$x$};
        \draw[axes] (7,-2.2)--(7,2.5) node[above]{$y$};
                
        \draw[thick] (14,1) circle (1) node[right]{$B$};
        \draw[axes] (11.8,0)--(16.5,0) node[right]{$x$};
        \draw[axes] (14,-2.2)--(14,2.5) node[above]{$y$};
        
        \node at (3.75,0) {\huge=};
        \node at (11,0) {\huge -};
      \end{tikzpicture}
    \end{center}
  \item Let the origin of the coordinate system to located at the centre of $A$
  \item Based on symmetry: $x_\text{cm}=0$; only have to find $y$-coordinate.
  \end{itemize}

  \eq{-.2in}{
    y_\text{cm}
    =\frac{\sum y_i m_i}{\sum m_i}
    =\frac{m_A(0) + m_B (r/2)}{m_A+m_B}
    =\frac{-\sigma\pi\left(r/2\right)^2(r/2)}
    {\sigma\pi r^2-\sigma\pi\left(r/2\right)^2}
    =\frac{-r}6
  }
\end{frame}



\section{Momentum and Centre of Mass}

\begin{frame}{Velocity of the Centre of Mass}
  As the masses that makes up the system move over time, so does the CM,
  i.e.\ the position of the CM is a function of time.  Taking time derivative
  of the equation for $\vec x_\text{cm}$ to get the velocity at the CM:

  \eq{-.1in}{
    \vec v_\text{cm}=\diff{\vec x_\text{cm}}t
    =\frac1{m_\text{tot}}\diff{}t\left(\int\vec x\dl m\right)
    =\frac1{m_\text{tot}}\int\diff{\vec x}t\dl m
    =\frac{\int\vec v\dl m}{m_\text{tot}}
  }

  Or, in the form that is familiar, the velocity of the CM is the
  weighted sum of the velocities of the distribution of mass:
  
  \eq{-.1in}{
    \vec v_\text{cm} = \frac{\int\vec v\dl m}{m_\text{tot}}
  }
\end{frame}



\begin{frame}{Velocity and Momentum}
  We can also rearrange the equation for the velocity of the centre of mass to
  relate it to momentum, because the term $\vec v\dl m=\dl{\vec p}$ is the
  momentum of the infinitesimal mass $\dl m$, and
  $\int\vec v\dl m=\vec p_\text{net}$ is the net momentum of the entire mass
  distribution:
  
  \eq{-.05in}{
    \vec v_\text{cm} = \frac{\int\vec v\dl m}{m_\text{tot}}
    \quad\longrightarrow\quad
    \vec p_\text{net}=m_\text{tot}\vec v_\text{cm}
  }

  During a collision, there is no net force, so there is no change in the net
  momentum of the system. The centre of mass will continue to move at the same
  velocity before/after the collision, as if the collision never occurred.
\end{frame}



\begin{frame}{Centre of Mass During Collision}
  \vspace{.1in}
  During a collision,
  %\footnote{As we have studied in conservation of momentum in
  %Physics 12 and in our previous class},
  there are no external forces, therefore the velocity of the CM remains
  constant. Consider this perflectly inelastic 1D collision in between two
  masses:
  \begin{center}
    \begin{tikzpicture}[scale=.8,thick]
      \begin{scope}[violet]
        \draw (1,0) rectangle (2,1) node[midway]{$m_1$};
        \draw (4,0) rectangle (5,1) node[midway]{$m_2$};
        \draw[vectors] (.8,1.3)--(2.5,1.3) node[right]{$v_1$};
        \draw[vectors] (4,1.3)--(5,1.3) node[right]{$v_2$};
        \node[above] at (3,1.5) {Before Collision};
      \end{scope}
      \draw[vectors] (2.7,.5)--(3.7,.5) node[above]{$v_\text{cm}$};
      \begin{scope}[orange]
        \draw (10,0) rectangle (11,1) node[midway]{$m_1$};
        \draw (11,0) rectangle (12,1) node[midway]{$m_2$};
        \draw[vectors] (10.2,1.3)--(11.8,1.3) node[right]{$v'$};
        \node[above] at (11,1.5) {After Collision};
      \end{scope}
      \draw (0,0)--(14,0);
      \draw (2.7,.5) circle (.15);
      \fill (2.7,.5)--(2.85,.5) arc (0:90:.15)--cycle;
      \fill (2.7,.5)--(2.55,.5) arc (180:270:.15)
      node[below=-2] (c){cm}--cycle; 

      \draw (11,.5) circle (.15);
      \fill (11,.5)--(11.15,.5) arc (0:90:.15)--cycle;
      \fill (11,.5)--(10.85,.5) arc (180:270:.15) node[below=-2] {cm}--cycle; 

      \node[text width=2.9in,draw,violet,below right] (a) at (-.5,-.6)
           {Using the definition of the velocity of the CM, we find
             that \emph{before} the collision:
             \vspace{-.1in}
             \begin{displaymath}
               v_\text{cm} = \frac{\sum m_iv_i}{\sum m_i}
               =\frac{m_1v_1+m_2v_2}{m_1+m_2}
             \end{displaymath}\par
           };
      \draw[axes,violet] (a)--(c);
           
      \node[text width=3in,draw,orange,below left] at (14.5,-.6)
           {Using momentum conservation, we find that the final
             velocity \emph{after} the collision is:
             \vspace{-.08in}
             \begin{displaymath}
               v'=\frac{m_1v_1+m_2v_2}{m_1+m_2}=v_\text{cm}
             \end{displaymath}
             \par
           };
    \end{tikzpicture}
  \end{center}
  If we define an inertial (i.e. a non-accelerating) frame of reference that
  moves with the CM, then before the collision, the total momentum is
  zero, and after the collision, total momentum is still zero. The frame is
  called the ``centre of momentum frame''
\end{frame}



\begin{frame}{Acceleration of the Centre of Mass}
  The \emph{acceleration} of the CM is, not surprisingly, also a function of
  time. Taking time derivative of the equation for $\vec v_\text{cm}$ to get the
  acceleration at the CM:

  \eq{-.1in}{
    \vec a_\text{cm}=\diff{\vec v_\text{cm}}t
    =\frac1{m_\text{tot}}\diff{}t\left(\int\vec v\dl m\right)
    =\frac1{m_\text{tot}}\int\diff{\vec v}t\dl m
    =\frac{\int\vec a\dl m}{m_\text{tot}}
  }

  Or, in the form that is familiar, the acceleration of the CM is the weighted
  sum of the accelerations of the masses that make up the system:
  
  \eq{-.1in}{
    \boxed{
      \vec a_\text{cm} = \frac{\int\vec a\dl m}{m_\text{tot}}
    }
  }
\end{frame}



\begin{frame}{Acceleration of the Centre of Mass}
  
  \eq{0in}{
    \vec a_\text{cm} = \frac{\int\vec a\dl m}{m_\text{tot}}
  }
  \begin{itemize}
  \item By the second law of motion, $\vec a\dl m=\dl{\vec F}$ is the net force
    acting on the infinitesimal mass $\dl m$
  \item Therefore $\int\vec a\dl m=\vec F_\text{net}$ is the net force on the
    entire system
  \end{itemize}
  In other words,
  
  \eq{-.1in}{
    \boxed{
      \vec F_\text{net}=m_\text{tot}\vec a_\text{cm}
    }
  }
  
  \vspace{-.1in}When a net force is applied to an object (or a system of
  objects), the object's (or the system's) acceleration is evaluated at the CM.
\end{frame}


\begin{frame}{Acceleration of the Centre of Mass}
  Alternatively, we can also take the derivative of net momentum with respect
  to time:%\footnote{i.e\ applying the 2nd law of motion to this distribution of
  %masses}:
  
  \eq{-.1in}{
    \diff{\vec p_\text{net}}t =
    \diff{}t (m_\text{tot}\vec v_\text{cm})
  }

  If the system mass is constant, then this equation reduces to:

  \eq{-.1in}{
    \diff{\vec p_\text{net}}t
    =m_\text{tot}\diff{\vec v_\text{cm}}t
    \quad\longrightarrow\quad
    \boxed{
      \vec F_\text{net}=m_\text{tot}\vec a_\text{cm}
    }
  }
  
  Aagin, we see that when a net force is applied to an object (or a system of
  objects), the object's (or the system's) acceleration is evaluated at the CM.
  \vspace{.3in}
\end{frame}



\begin{frame}[b]{The Exploding Canonball Problem}
%  \begin{center}
%    \begin{tikzpicture}[scale=1.2]
%      \draw[very thick] (0,0)--(12,0);
%      \begin{scope}[rotate around={35:(.5,.5)}]
%        \draw[thick] (.5,.7) arc (90:270:.2)--(1.7,.3)--(1.7,.7)--(.5,.7);
%        \draw[line width=3] (1.7,.28)--(1.7,.72);
%        %\draw[thick] (.3,.3) rectangle +(2,.4);
%      \end{scope}
%      \draw[thick] (.5,.5) circle (.1);
%    \end{tikzpicture}
%  \end{center}
\end{frame}


%\begin{frame}{What This All Means}
%  \begin{itemize}
%  \item Newton was right all along by treating all objects as point masses
%    located at the CM
%  \end{itemize}
%\end{frame}

\end{document}
