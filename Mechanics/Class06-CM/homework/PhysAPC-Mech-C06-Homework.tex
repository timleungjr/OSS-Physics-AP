...
\gentitle{6}{MOMENTUM AND CENTRE OF MASS}
...
\begin{questions}
  ...
  %smaller sphere. How far has the centre of mass of the entire object moved?
  %\begin{choices}
  %  \choice The centre of mass has not moved, because both spheres still have
  ...
  rest. Which of the following statements must be true?
  \begin{choices}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.8]
      \fill[gray!60] (-.2,1.5) rectangle(0,-1.5);
      \draw[very thick] (0,-1.5)--(0,1.5);
      \draw[dashed] (0,0)--(2,0) node[right]{\small Normal};
      \draw[vectors,rotate=30] (0,0)--(2,0) node[right]{$v$};
      \draw[vectors,rotate=-30] (2,0)--(0,0) node[pos=0,right]{$v$};
      \draw[thick] (1,0) arc(0:-30:1) node[midway,right]{\small$\theta$};
      \draw[thick] (1.2,0) arc(0:30:1.2) node[midway,right]{\small$\theta$};
    \end{tikzpicture}
  \end{center}
  \begin{choices}
    \choice $mv$
  ...
  \question A known net force $F(t)$ acts on an unknown mass for a known time
...
%    \end{choices}
  ...
  \question A small mass $m$ is moving with a speed $v$ toward a stationary
  mass $M$. The speed of the centre of mass of the system is
  \begin{choices}
...
%    \end{choices}
  ...
  velocity of the centre of mass of the projectile at point $P$ after the
  explosion?
  \label{cm1}
  \begin{choices}
  ...
  \question Which of the following statements is true of the centre of mass of
  the projectile after the explosion?
  \label{cm2}
  \begin{choices}
    \choice The centre of mass will continue on a parabolic path and land on
    the ground at the place where it would have landed had it not exploded.
    \choice The centre of mass will alter its parabolic path and land on the
    ...
    exploded.
    \choice The centre of mass will fall straight downward from the point of
    explosion.
    \choice The centre of mass will travel straight upward from the point of
    explosion.
  \end{choices}
...
%    \end{choices}
  ...
  \question A moving object is changing its momentum during a time interval. If
  a graph of momentum vs.\ time is plotted, the net force acting on the mass at
  any time can be determined by finding the
  \begin{choices}
...
\begin{questions}    
  ...
  string. The centre of mass of the block is \SI{1.2}{\metre} below the pivot
  ...
  negligible.
  \begin{parts}
    \part\textbf{Calculate} the speed of the dart just before it strikes the
    block.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the horizontal distance $d$ between the launching
    point of the dart and a point on the floor directly below the block.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the speed of the block just after the dart strikes.
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the angle $\theta$ through which the dart and block
    on the string will rise before coming momentarily to rest.
    \vspace{\stretch1}
    \newpage
    ...
    \textbf{Indicate} whether the angle $\theta$ that the dart and block swing
  ...
  \end{displaymath}
  \begin{parts}
    \part\textbf{Calculate} the mass $M$ of the object.
    \vspace{\stretch1}
    
    \part Assuming an initial position of $x=0$, \textbf{derive} an expression
    for the position of the car-object system after the collision as a function
    of time $t$.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the resisting force on the car-object
    system after the collision as a function of time $t$.
    \vspace{\stretch1}
    
    \part\textbf{Derive} the impulse delivered to the car-object system from
    $t=0$ to $t=\SI{2.0}\second$.
    \vspace{\stretch1}
  \end{parts}
  \newpage
...
%  \newpage
  ...
  launching device. Express your answers to parts (\ref{projectile-a}) and
  (\ref{projectile-b}) in terms of $v_x$, $J$, $F_\text{avg}$, and fundamental
  constants, as appropriate.
  \begin{parts}
    \part\textbf{Derive} an expression for the time required for the projectile
    to travel the length of the launching device.
    \label{projectile-a}
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the mass of the projectile.
    \label{projectile-b}
    \vspace{\stretch1}
    ...
    \part\textbf{Derive} an expression for the work done in stopping the
    projectile.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the average force $F_b$ exerted on
    the projectile as it comes to rest in the block.
    \label{this}
    \vspace{\stretch1}
    \newpage
    
    \uplevel{
      Now a new projectile and block are used, identical to the first ones, but
      the block is not clamped to the table. The projectile is again fired into
      the block of wood and travels a new distance $d_n$ into the block while
      the block slides across the table a short distance $D$. Assume the
      following: the projectile enters the block with speed $v_x$, the average
      force $F_b$ between the projectile and the block has the same value as
      determined in part (\ref{this}), the average force of friction between
    ...
    \part\textbf{Derive} an expression for $d_n$ in terms of $d$, $D$, $f_T$,
    and $F_b$, as appropriate.
    %\vspace{\stretch1}
    
    %\part\textbf{Derive} an expression for $d_n$ in terms of $d$, the mass $m$
