...
\begin{frame}{First Law of Motion}
  \begin{center}
    \fcolorbox{black}{yellow!15}{
      \begin{minipage}{.65\textwidth}
        \textbf{Law \#1: An object remains in its state of rest or uniform
          motion, until a net external force is applied to it.}
      \end{minipage}
    }
  \end{center}
  Under the first law of motion, these equations are equivalent:
  ...
  \begin{columns}[T]
    \column{.4\textwidth}
    \begin{itemize}
    \item No net external force
    \item Vector sum of all forces on the object is zero
    \item Forces on the object are \emph{balanced}
    \item\textbf{State of equilibrium}
    \end{itemize}
    
    \column{.31\textwidth}
    \begin{itemize}
    \item Constant velocity (constant magnitude \underline{\emph{and}}
      direction)
    \item\textbf{Uniform motion}
    \item Object moves along a straight line with constant speed
    \end{itemize}
...
\begin{frame}{First Law of Motion}
  ...
  \item Common examples:
    ...
    \item A car travelling along a straight line on a levels stretch highway at
      \SI{100}{\kilo\metre\per\hour} has many forces acting on it, but the net
      force is zero 
    \end{itemize}
...
\begin{frame}{Second Law of Motion}
  ...
  \begin{itemize}
    ...
    \eq{-.1in}{
      \boxed{\vec F_\text{net}(t)=\sum\vec F(t)=m\vec a(t)}
    }
  \item\textcolor{red!80!black}{Like the first law, the equation is also a
...
\begin{frame}{Third Law of Motion}
  ...
  \begin{itemize}
  \item Forces are \emph{always} created in pairs
...
\begin{frame}{Forces}
  A \textbf{force} is a ``push'' or ``pull'' upon an object resulting from the
  object's \emph{interaction} with another object
  \begin{itemize}
  \item Whenever there is an interaction between two objects, there is a force
    on the objects
  \item The interaction could be by contact or at a distance
  \item When the interaction ceases, the two objects no longer experience the
    force
  \item The unit of force is a \textbf{newton}, defined using base SI units:
    $\SI1\newton=\SI1{\kilo\gram\metre\per\second\squared}$
  \end{itemize}
  There are two broad categories of forces:
  \begin{itemize}
  \item The majority of forces are \textbf{contact forces} that act between
    objects in contact with one another
  \item There are also some \textbf{non-contact forces} act between two objects
    without them touching each other. They are also called
...
\begin{frame}{Common Forces}
  ...
  \item Buoyant force $\vec F_B$ (discussed in fluid mechanics, in AP Physics 1)
...
\begin{frame}{Normal Force}
  \begin{columns}
    ...
    \item\textbf{Special case:} When an object is on a horizontal surface
      ...
      equal to the magnitude of the weight of the object, i.e.\ $F_n=mg$
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Friction}
  ...
  \item Depends on:
    \begin{itemize}
    \item Normal force $F_n$: The force the two surfaces are pressed against
      each other
...
\begin{frame}{Tires}
  Friction is often the force that slows things down, but sometimes friction is
  responsible for speeding up and object. For example, the forward acceleration
  of a car is caused by the static friction between the tires and the road.
  \begin{center}
    \begin{tikzpicture}
      \node at (0,0) {\pic{.5}{graphics/all-season-tires}};
      \draw[vectors,yellow] (.05,-1.45)--+(1,0) node[right]{$\vec F_s$};
      \draw[vectors,orange,rotate around={70:(.05,-.28)}]
      (1.1,-.28) arc (0:-200:1.06)
      node[left]{\textbf{Dir.\ of Rotation}};
    \end{tikzpicture}
  \end{center}
  Tires also generate a force called \emph{rolling resistance} as it rolls
...
\begin{frame}{Spring Force}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  When a spring acts on an object, the object's acceleration is usually
  non-constnt, so solving motion involving springs would usually require
  calculus, or conservation of energy.
\end{frame}
...
\begin{frame}{Free-Body Diagrams}
  ...
  by assuming that all forces acting at the centre of mass (``CM''),
