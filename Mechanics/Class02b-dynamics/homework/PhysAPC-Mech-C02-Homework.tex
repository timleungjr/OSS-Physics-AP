...
%    \label{q:2blocks2}
  ...
  \question A block of mass \SI4{\kilo\gram} slides down a rough incline with a
  constant speed. The angle the incline makes with the horizontal is \ang{30}.
  The coefficient of friction acting between the block and incline is most
  nearly
  \begin{center}
    \begin{tikzpicture}[thick]
      ...
      \begin{scope}[rotate=-30]
        ...
        \draw[very thick,->] (-2.4,.4)--(-1.,.4) node[right]{$v$};
      \end{scope}
    \end{tikzpicture}
  \end{center}
  \begin{choices}
  ...
  \uplevel{
    \textbf{Questions \ref{pos1}--\ref{pos2}}: The position of a
    \SI2{\kilo\gram} object is described by the equation $x=2t^2-3t^3$, where
    $x$ is in meters and $t$ is in seconds.
  }

  \question The net force acting on the object at a time of \SI1{\second} is
  \begin{choices}
    \choice\SI{-4}\newton
    \choice\SI{-8}\newton
    \choice\SI{-14}\newton
  ...
  \uplevel{
    \textbf{Questions \ref{q:striaght1}--\ref{q:striaght2}}: An object of
    mass $m$ moves along a straight line with a speed described by the
    equation $v=c+bt^3$.
  }
...
%    \columnbreak
  ...
  \uplevel{
    ...
    \begin{center}
      \begin{tikzpicture}[scale=1.1,thick]
        ...
        \begin{scope}[rotate=-30]
          \draw (-2,0) rectangle (-2.8,.8);
          \draw[vectors] (-2,1)--(-2.8,1) node[left]{$v$};
        \end{scope}
      \end{tikzpicture}
    \end{center}
  }
  ...
  \begin{oneparchoices}
    ...
    \begin{tikzpicture}[vectors]
      \fill circle (.08);
      \draw (0,0)--(0,1)  node[above]{$\vec F_n$};
      \draw (0,0)--(0,-1) node[below]{$\vec F_g$};
      \draw[rotate=60] (0,0)--(0,1) node[left]{$\vec f$};
    \end{tikzpicture}
    \choice
    \begin{tikzpicture}[vectors]
      \fill circle (.08);
      \draw[rotate=-30] (0,0)--(0,1) node[above]{$\vec F_n$};
      \draw[rotate=-30] (0,0)--(0,-1) node[below]{$\vec F_g$};
      \draw[rotate=60] (0,0)--(0,1) node[left]{$\vec f$};
    \end{tikzpicture}
    \choice
    \begin{tikzpicture}[vectors]
    ...
    \begin{tikzpicture}[vectors]
      \fill circle (.08);
      \draw[rotate=120] (0,0)--(0,1) node[left]{$\vec F_n$};
      \draw (0,0)--(0,-1) node[below]{$\vec F_g$};
      \draw[rotate=60] (0,0)--(0,-1) node[right]{$\vec f$};
    \end{tikzpicture}
  \end{oneparchoices}
...
%  \newpage
  ...
  \uplevel{
    \centering
    \begin{tikzpicture}[scale=.9]
      \draw[very thick] rectangle(2,1.5) node[midway]{$m$};
  ...
  \begin{parts}
    \part On the figure below, \textbf{Draw} and \textbf{label} the forces (not
    components) that act on the block. Forces should be drawn as arrows
    starting at, and pointing away from, the figure.
    \vspace{.7in}
    \begin{center}
      \tikz{\draw[thick] rectangle (.7,.5);}
    \end{center}
    \vspace{.7in}
    
    \part\textbf{Derive} an expression for the normal force exerted by the
    surface on the block.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the coefficient of kinetic friction
    $\mu$
    between the block and the surface.
    \vspace{\stretch1}
    
    \part On the axes below, \textbf{sketch} graphs of the speed $v$ and
    displacement $x$ of the block as functions of time $t$ if the block started
    from rest at $x=0$ and $t=0$.
    
    \begin{tikzpicture}[very thick,->]
      \draw (0,0)--(7.5,0) node[right]{$t$};
      \draw (0,0)--(0,4) node[above]{$v$} node[pos=0,left]{$O$};
    \end{tikzpicture}
    \begin{tikzpicture}[very thick,->]
      \draw (0,0)--(7.5,0) node[right]{$t$};
      \draw (0,0)--(0,4) node[above]{$x$} node[pos=0,left]{$O$};
    \end{tikzpicture}
    \newpage
    
  \item If the applied force is large enough, the block will lose contact with
    the surface. \textbf{Derive} an expression for the magnitude of the greatest
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=1.3,very thick]
      ...
      \draw(-1.5,0)--(5.5,0);
      \draw rectangle(1.8,1) node[midway]{$m$};
  ...
  but the box is subject to a drag force of magnitude $kv$ where $v$ is the
  speed of the box and $k$ is a positive constant. Express all your answers in
  terms of the given quantities and fundamental constants, as appropriate.
  \begin{parts}
    \part The dot below represents the box. \textbf{Draw} and \textbf{label}
    the forces (not components) that act on the box. Forces should be drawn as
    arrows starting at, and pointing away from, the dot.
    \label{fbd}
    \vspace{.7in}
    \begin{center}
      {\tikz\fill circle (.15);}
    ...
    be used to determine the speed $v$ of the box as a function of time $t$. If
    you need to draw anything other than what you have shown in part \textbf{A}
    to assist in your solution, use the space below. Do NOT add
    anything to the figure in part \textbf{A}.
    \label{diffeq}
    \vspace{\stretch1}
    
    \part\textbf{Determine} the magnitude of the terminal velocity of the box.
    \vspace{\stretch1}
    
    \part Use the differential equation from part \textbf{B} to
    \textbf{derive} the equation for the speed $v$ of the box as a function of
    time $t$. Assume that $v=0$  at time $t=0$.
    \vspace{\stretch1}
    \newpage
    
    \part On the axes below, \textbf{sketch} a graph of the speed $v$ of the
    box as a function of time $t$. Explicitly \textbf{label} any intercepts,
    asymptotes, maxima, or minima with numerical values or algebraic
    expressions, as appropriate.

    \uplevel{
      \centering
      \begin{tikzpicture}[very thick,->]
        \draw (0,0)--(4,0) node[pos=0,left]{$O$} node[right]{$t$};
...
%    \vspace{\stretch1}
%    
%    \part\textbf{Derive} an expression for the speed $v_D$ of the passenger compartment
...
%      by a braking force $-k\vec v$, where $k$ is a constant and $\vec v$ is the
...
%    }
%
%    \part Write, but do NOT solve, the differential equation for $v(t)$.
%    \vspace{\stretch1}
%    \label{diffeq}
%    
...
%  \begin{parts}
%    \part\textbf{Derive} an expression for the speed $v_h$ of the hanging block as
...
%    \newpage
%    
%    \part\textbf{Derive} an expression for the work done by gravity on the rope as a
%    function of $y$, assuming $y$ is initially zero.
%    \vspace{\stretch1}
%    
%    \part\textbf{Derive} an expression for the speed $v_r$ of the rope as a function of
%    $y$.
%    \vspace{\stretch1}
%    
...
%    that the sliding block does not hit the pulley. Indicate whether $v_h$
%    from part (a) or $v_r$ from part (d) is greater after the block and the
%    end of the rope have traveled this distance.
%
%    \vspace{.2in}
...
%  \end{parts}
  ...
  % TAKEN FROM THE 2024 AP PHYSICS C MECH EXAM SET 1 FREE-RESPONSE QUESTION 1
  \uplevel{
    \centering
    \begin{tikzpicture}
      \draw[thick] ellipse (.5 and 1);
      \draw[thick] (3,-1) arc (-90:90:.5 and 1);
      \foreach \y in {-1,1} \draw[thick] (0,\y)--+(3,0);
      \node at (1.8,0) {Cylinder};
      \draw[vectors] (5,-1)--+(0,2) node[above]{$F_\text{drag}$};
      \fill (.75,1.75) ellipse (.03 and .6);
      \fill (1.2,2.3) ellipse (.03 and .7);
      \fill (1.6,1.75) ellipse (.03 and .6);
      \fill (2.2,1.75) ellipse (.03 and .45);
    \end{tikzpicture}

    Figure 1
  }
  \question A student drops a cylinder of mass $m$ from rest. The air exerts a
  drag force of magnitude $F_\text{drag}$ on the cylinder, as shown in Figure 1.
  The student models the magnitude of the drag force as $F_\text{drag} = Bv^2$,
  where $v$ is the speed of the cylinder and $B$ is a positive constant with
  appropriate units.
  \begin{parts}
    \part \textbf{Derive}, but do NOT solve, a differential equation that could
    be used to determine the speed $v$ of the cylinder as a function of time
    $t$. Express your answer in terms of given quantities and physical
    ...
    \uplevel{
      \begin{center}
        \begin{tikzpicture}[scale=.9,thick]
          \draw[->] (0,0)--(9,0) node[right]{$t$} node[pos=0,below left]{$O$};
          \draw[->] (0,0)--(0,5) node[above]{$v$};
          \draw[very thick] (0,0) to [out=70,in=180] (3,4)--(8,4);
          \draw (.1,4)--(-.1,4) node[left]{$v_\text{max}$};
        \end{tikzpicture}\\
        Figure 2
      \end{center}
      The student correctly sketches the speed $v$ of the cylinder as a
      function of time $t$, as shown in Figure 2.
    }
    \part\textbf{Draw} a vertical line on the sketch in Figure 2 to indicate the
    earliest time at which Fdrag on the cylinder is equal to the magnitude of
    the weight of the cylinder. Label this time as $t_1$ on the time axis.
    \vspace{.2in}
    
    \part\textbf{Justify} the location of $t_1$. Explicitly reference
    appropriate features of the sketch in Figure 2.
    \vspace{\stretch2}
    \newpage

    \part Rather than dropping the cylinder from rest, the student throws the
    cylinder upward with a nonzero initial speed. The cylinder is in the
    same orientation as when the cylinder was previously dropped. The student
    allows the cylinder to fall toward the ground.

    \vspace{.15in}\textbf{Indicate} whether the magnitude of the cylinder's
    maximum downward speed after being thrown upward would be greater than,
    less than, or equal to the maximum speed $v_\text{max}$ in Figure 2.

    \vspace{.15in}
    \underline{\hspace{.4in}} Greater than\hspace{.4in}
    \underline{\hspace{.4in}} Less than\hspace{.4in}
    \underline{\hspace{.4in}} Equal to

    \vspace{.1in}Briefly \textbf{justify} your answer.
    \vspace{\stretch1}
    ...
    \uplevel{
      \begin{center}
        \pic{.7}{graph}
      \end{center}
      The student conducts an experiment to better understand the relationship
      between maximum speed $v_\text{max}$ and mass. The student collects data
      to determine the maximum speed for cylinders dropped from rest, each with
      the same physical size and shape but a different mass $m$. The student
      then graphs $v_\text{max}^2$ as a function of mass.
    }

    \part\textbf{Draw} the best-fit line for the data.
    
    \part Use the best-fit line to \textbf{calculate} an experimental value for
    $B$
    \vspace{\stretch{1.5}}
  \end{parts}
