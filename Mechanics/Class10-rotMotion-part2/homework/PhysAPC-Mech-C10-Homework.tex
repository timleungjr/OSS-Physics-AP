...
\runningheader{
  \fontfamily{phv}\small  \textcolor{gray}{AP PHYSICS C: MECHANICS $\blacksquare$}
...
%    \columnbreak
    ...
    \question Which of the following diagrams best represents the
    gravitational, normal and frictional forces acting on the sphere as it
...
\begin{questions}
  ...
  \begin{parts}
    \part\textbf{Determine} the linear acceleration $a$ of the falling block in
    ...
    \newpage
    \part On the grid below, \textbf{plot} the quantities determined in (b)i,
    ...
    \part Use your graph above to \textbf{calculate} the magnitude of the
    acceleration.
    \vspace{\stretch1}

    \part\textbf{Calculate} the rotational inertia of the pulley in terms of
    $m$, $R$, $a$, and fundamental constants.
    \vspace{\stretch1}
    ...
    value. Give one \textbf{explanation} for this discrepancy.
    \vspace{\stretch2}
  \end{parts}
  \newpage
  ...
  \question A ring of mass $M$, radius $R$, and rotational inertia
  $MR^2$ is initially sliding on a frictionless surface at constan velocity
  $v_0$ to the right, as shown above. At time $t=0$ it encounters a surface
  with coefficient of friction $\mu$ and begins sliding and rotating. After
  traveling a distance $L$, the ring begins rolling without sliding. Express
  all answers to the following in terms of $M$, $R$, $v_0$, $\mu$, and
