...
%    \label{ball2}
  ...
  radius of \SI{2.7}{\kilo\metre}. Which of these values \emph{best} represents
  the angle of the wings relative to the horizontal if the airplane experiences
  no change in altitude during the turn? Assume that the wings are completely
  horizontal during level flight.
  \begin{choices}
    \choice\ang{8}
    \choice\ang{16}
    \choice\ang{20}
...
%    \end{choices}
  ...
  shown above. The ball swings under the influence of gravity from point $X$
  ...
  \uplevel{
    ...
    \begin{tikzpicture}[scale=.7,thick]
      ...
      \begin{scope}[rotate=60]
        ...
        \fill (0,-4) circle (.13) node[right=4]{$m$};
        \draw[<->] (.5,0)--(.5,-4) node[midway,above right]{$l$};
      \end{scope}
      \draw[<->] (0,-1) arc (270:330:1) node[pos=.6,below=3]{\ang{60}};
    \end{tikzpicture}
  }
  \question A pendulum bob of mass $m$ is released from rest as shown in the
  figure above. What is the tension in the string as the pendulum swings
...
%    \end{choices}
  ...
  angular speed $\omega$, the floor is lowered, and the riders stick to the
  wall. Which of the following statements is true?
  \begin{choices}
    \choice The weight of each rider provides the centripetal force keeping the
    rider moving in a circle.
    \choice The normal force applied by the wall must equal the weight of the
    rider.
    \choice The difference between the normal force applied by the wall and the
  ...
  \question At which point would the bug need to apply the most adhesive force
  ...
  \question Which of the following expressions correct gives the speed at which
  the ball is travelling in terms of $\theta$, $\ell$ and universal constants?
  \label{conical2}
  \begin{choices}
    \choice $\sqrt{\ell g}$
...
\begin{questions}
  ...
  \end{center}
  \begin{parts}
    \part\textbf{Determine} the speed of the mass attached to the string the
    instant the tring is cut.
    \vspace{\stretch1}
    
    \part\textbf{Sketch} the forces acting on the mass (not components) when it
    is in the position shown below. Forces should be drawn as arrows starting
    at, and pointing away from, the mass.
    \begin{center}
      \begin{tikzpicture}[scale=2,thick]
        \draw (0,-2)--(2.5,-2);
        \draw (2.5,-2) arc (-90:90:1);
    ...
    \part\textbf{Derive} an expression for the object's speed as a function of
    $\theta$
    \vspace{\stretch1}
    
    \part\textbf{Determine} the object's centripetal acceleration as a function
    of $\theta$
    \vspace{\stretch1}
    
    \part\textbf{Calculate} the angle $\theta$ the mass will fall of the track
    \vspace{\stretch1}
  \end{parts}
  \newpage
  ...
  \begin{parts}
    \part\textbf{Derive} an expression for the height of the ramp $h$. Express
    your answer in terms of $v_0$, $m$, and fundamental constants, as
    appropriate.
    \vspace{\stretch1}
    
    \uplevel{
      A short time after passing point $P$, the block is in contact with the
      wall and moves with a speed of $v$.
    }
 
    \part\textbf{Indicate} whether the vertical component of the net force on
    the block upward, downward, or zero:
      
    \vspace{.1in}
    \underline{\hspace{.4in}} Upward\hspace{.3in}
    \underline{\hspace{.4in}} Downward\hspace{.3in}
    \underline{\hspace{.4in}} Zero
    
    \vspace{.1in}\textbf{Justify} your answer.
    \vspace{\stretch1}
    
    \part On the figure below, \textbf{draw} an arrow starting on the block to
    ...
    \uplevel{
      Express your answers to the following in terms of $v_0$, $v$, $m$, $R$,
      $\mu$, and fundamental constants, as appropriate.
    }

    \part\textbf{Derive} an expression for the magnitude of the normal force $N$
    exerted on the block by the circular wall as a function of $v$.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the magnitude of the tangential
    acceleration of the block at the instant the block has attained a speed of
    $v$.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for $v(t)$, the speed of the block as a
  ...
  negligible. \textbf{Derive} expressions for each of the following in terms of
  $m_1$, $m_2$, $L$, and fundamental constants.
  \begin{parts}
    \part The speed of the person just before the collision
    with the object.
    \vspace{\stretch1}
