...
\begin{frame}{Polar Coordinate System in 2D}
  \begin{columns}
    ...
    times previously (Sass Class \#1 on kinematics if you need a refresher).
  \end{columns}
\end{frame}
...
\begin{frame}{Velocity and Angular Velocity}
  \begin{columns}[T]
    ...
    \vspace{-.1in}\textbf{Angular velocity} $\omega$, or
    \textbf{angular frequency}, is the time derivative of position (i.e.\ how
    quickly angular position is changing with time). It is measured in
    \textbf{radian per second} (\si{\radian\per\second}):
...
\begin{frame}{Angular Acceleration}
  ...
  \begin{itemize}
  \item Tangential acceleration $a_t$ is along (parallel or anti-parallel) to
    direction of motion
...
\begin{frame}{Acceleration: The General Case}
  \begin{columns}
    ...
    \item\textcolor{cyan}{\textbf{Tangential acceleration} $\vec a_t$} depends
      on radius $r$ and angular acceleration $\alpha$. The direction of the
      acceleration is tangent to the circle
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Dynamics of Circular Motion Problems}
  ...
  \item\textbf{Tangential force} $\vec F_t$ along the direction of motion
    (i.e.\ along the angular direction $\hat{\theta}$) causes the tangential
    acceleration:
  ...
  \item\textbf{Centripetal force} ($\vec F_c$) towards the centre of the motion
    (i.e.\ the inward radial direction $-\hat{r}$) causes centripetal
    acceleration:
...
\begin{frame}{What About a Pendulum?}
  ...
  not travel a full circle, we can still think of its path a a segment of a
  circle
  \begin{columns}[T]
    \column{.35\textwidth}
    \begin{tikzpicture}[scale=.75]
    ...
    \begin{tikzpicture}[scale=.75]
      ...
      \begin{scope}[rotate=45]
        ...
        \begin{scope}[vectors,red]
          ...
          node[right,fill=yellow!20,opacity=.8]{$F_g\cos\theta$};
          \draw (0,-5)--(0,-3.9)
