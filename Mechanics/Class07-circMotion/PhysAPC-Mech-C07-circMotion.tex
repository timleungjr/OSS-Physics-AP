...
\begin{frame}{Polar Coordinate System in 2D}
  \begin{columns}
    ...
    \item A simpler way is to think of position as just two
...
\begin{frame}{Angular Position and Angular Velocity}
  ...
  \begin{columns}[T]
    ...
    in angle is its \textbf{angular displacement}:

    \eq{-.1in}{
      \boxed{\Delta\theta(t)=\theta(t)-\theta_0}
    }

    \vspace{-.1in}where $\theta_0$ is the initial angular position
  \end{columns}
\end{frame}
...
\begin{frame}{Velocity and Angular Velocity}
  \begin{columns}
    ...
    \vspace{-.05in}The speed ($v$) of the object in circular motion is related
    to the angular velocity ($\omega$) by:
    ...
    \item\vspace{-.2in}If $\omega>0$, the motion is counter-clockwise
...
\begin{frame}{Period \& Frequency}
  \begin{columns}
    ...
    \vspace{.2in}{\small(\textbf{Note:} Even for non-uniform circular motion,
...
\begin{frame}{Centripetal Acceleration \& Centripetal Force}
  ...
  the centre of motion, as $\hat r$ is the outward radial direction.
\end{frame}
...
\begin{frame}{Acceleration: The General Case}
  \begin{columns}
    ...
    \item\textcolor{magenta}{\textbf{Centripetal acceleration} $\vec a_c$}
      ...
      of the circle
    \item \textcolor{cyan}{\textbf{Tangential acceleration} $\vec a_t$}
...
\begin{frame}{Dynamics of Circular Motion Problems}
  ...
  \begin{itemize}
  \item\textbf{Tangential force} $\vec F_t$ along the direction causes the
    tangential acceleration:

    \eq{-.1in}{
      \boxed{
        \vec F_t=m\vec a_t=mr\alpha
      }
    }
