...
\begin{frame}
  \centering\Large
  \textbf{WELCOME TO AP PHYSICS C--MECHANICS}
\end{frame}
...
\begin{frame}{Prerequisites}
  \begin{itemize}
  \item\textbf{Physics 11 and 12} or \textbf{AP Physics 1 and 2}. You will need
    to be comfortable with the topics covered in high-school level physics
    courses.
  \item\textbf{Calculus}. Physics C exams are calculus based, and you will be
    ...
    challenges. There will also be some basic differential equations.
...
\begin{frame}{Kinematics}
  \textbf{Kinematics} is a discipline within mechanics concerning the motion of
  bodies. It describes the mathematical relationship between 
  \begin{itemize}
  ...
  \end{itemize}
  Kinematics does not deal with what causes of motion to change. The quantities
  that are highlighted are vectors.
\end{frame}



\begin{frame}{Position}
  \textbf{Position} ($\vec r(t)$) is the location of an object in a predefined
  coordinate system as a continuous (and continuously differentiable) function
  of time. It is measured in \textbf{metres} (\si\metre).
  \begin{columns}
    \column{.7\textwidth}
...
\begin{frame}{Displacement}
  ...
  \eq{-.1in}{
    \vec r_0=x_0\iii+y_0\jjj + z_0\kkk
  }
...
\begin{frame}{Instantaneous Velocity}
  ...
  position or displacement with respect to time. It is a continuous function,
  but not necessarily continuously differentiable. It is related to position
  $\vec r(t)$ by:
  ...
  \eq{-.13in}{
    ...
    \boxed{
      \vec r(t)=\int\vec v(t)\dl t + \vec{\mathcal R}
    }
  }

  $\vec{\mathcal R}$ is the constant of integration based on\footnote{Note that
  $\vec{\mathcal R}$ is \emph{not} necessarily the initial position!} initial
  condition at $t=t_0$. The SI unit of velocity is \textbf{metres per second}
  (\si{\metre\per\second}). Displacement from $t_0$ to $t$ can also be found by
  doing a \emph{definite} integral:

  \eq{-.13in}{
    \boxed{
      \Delta\vec r(t)=\int_{t_0}^t\vec v(t)\dl t
    }
  }
  \vspace{.2in}
\end{frame}
...
\begin{frame}{Average Velocity}
  Likewise, the \textbf{average velocity} $\vec v_\text{avg}(t)$ is the
  displacement $\Delta\vec r(t)$ over a finite time interval $\Delta t$:

  \eq{-.1in}{
    \boxed{
      \vec v_\text{avg}(t) = \frac{\Delta\vec r(t)}{\Delta t}=
      \frac{\vec r(t)-\vec r_0}{t-t_0}
      = \frac{\int_{t_0}^t\vec v(t)\dl t}{t-t_0}
    }
  }
  \uncover<2>{
    \begin{tikzpicture}[overlay]
      \draw[thick,red] (9.4,.2) rectangle +(2.27,1.75);
      \fill[fill=red!10,opacity=.3] (9.4,.2) rectangle +(2.27,1.75);
      \node[text width=278,draw=red,thick,fill=red!5] (t) at (4,-1) {
        \color{red}This type of averaging is called a ``time average''. Later,
        when we discuss mechanical work, we will also use another type of
        averaging called ``spatial average''.};
      \draw[axes,red] (t) to[out=0,in=270] (10.4,.2);
    \end{tikzpicture}
  }
\end{frame}
...
\begin{frame}{Instantaneous \& Average Speed}
  ...
  (i.e.\ $v=|\vec v|$). Since $s\geq 0$, instantaneous speed must be
  non-negative, (i.e.\ $v\geq 0$).
...
\begin{frame}{Instantaneous \& Average Acceleration}
  ...
  If velocity $\vec v(t)$ is a differentiable function, then
  ...
  \eq{-.1in}{
    ...
    \quad\longleftrightarrow\quad
    \boxed{
      \vec v(t)=\int\vec a(t)\dl t+\vec{\mathcal V}
    }
  }
...
\begin{frame}{If You Are Curious}
  ...
  The next two derivatives are called \textbf{crackle} and
...
\begin{frame}{Maximum Range}
  ...
  \item For a given initial speed $v_0$ and range $R$, launch angle $\theta$ is
    ...
    But there is another angle $\theta_2$ that \emph{gives the same range}!
