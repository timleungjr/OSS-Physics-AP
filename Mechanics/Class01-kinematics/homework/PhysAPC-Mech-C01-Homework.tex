...
\begin{questions}
  ...
  horizontal and the dart reaches a maximum height of 1 metre. If the dart were
  fired straight up into the air along the vertical, the dart would reach a
  height of
  \begin{choices}
    \choice\SI1\metre
    \choice\SI2\metre
  ...
  velocity $v_0$. Due to air resistance, the filters fall with an acceleration
  proportional to the velocity of fall, that is, $a=-kv$, where $k$ is a
  positive constant. The velocity of the falling filters as a function of time
  of fall is
  \begin{choices}
    \choice $-kv^2$
    \choice $-\dfrac1{2kv^2}$
...
%    \end{choices}
  ...
  in a straight line with an acceleration described by the equation $a=4t^2$ in
  \si{\metre\per\second\squared}. What is the position of the object at
  $t=\SI3\second$?
  \begin{choices}
    \choice\SI6\metre
    \choice\SI1\metre
  ...
  \uplevel{
    \textbf{Questions \ref{q:fall1}--\ref{q:fall2}}: An object is released from
    rest and falls through a resistive medium. The resistance causes the
    velocity of the object to change according to the equation
    $v=16t-\dfrac12t^4$, where $v$ is in \si{\metre\per\second}, and time is in
    seconds.
  }
  
  \question Which of the following is a possible equation for the acceleration
...
%    \label{car2}
  ...
  time graph below. The object is initially at rest. Which of the following
  statements is true about the motion of the object?
  \begin{center}
    \begin{tikzpicture}[scale=.47]
      \draw[axes] (0,0)--(6.3,0) node[right]{$t$ [s]};
  ...
  \begin{choices}
    ...
    \choice The velocity of the object is zero at $t=\SI2\second$.
    \choice The velocity of the object is zero at $t=\SI4\second$.
    \choice The displacement of the object is zero at $t=\SI4\second$.
    \choice The acceleration of the object is zero at $t=\SI2\second$.
  \end{choices}
  \newpage
  ...
  \question Two velocity vectors $\vec v_1$ and $\vec v_2$ each have a
  magnitude of \SI{10}{\metre\per\second}. Graph 1 shows the velocity
  $\vec v_1$ at $t=\SI0\second$, and then the same object has a velocity
  $\vec v_2$ at $t=\SI2\second$, shown in Graph 2. Which of the following
  vectors best represents the average acceleration vector that causes the
  object's velocity to change from $\vec v_1$ to $\vec v_2$ ?
  \begin{center}
    \begin{tikzpicture}
      \draw (-1,0)--(1,0);
      \draw (0,-1)--(0,1) node[pos=0,below]{Graph 1};
      \draw[vectors] (0,0)--(.8,0) node[above]{$\vec v_1$};
    \end{tikzpicture}
    \hspace{.2in}
    \begin{tikzpicture}
      \draw (-1,0)--(1,0);
      \draw (0,-1)--(0,1) node[pos=0,below]{Graph 2};
      \draw[vectors] (0,0)--(0,.8) node[right]{$\vec v_2$};
    \end{tikzpicture}
  \end{center}
  \begin{oneparchoices}
    \choice
...
\begin{questions}
  ...
  \begin{displaymath}
    x=2t^3-15t^2+36t-8
  \end{displaymath}
  where $x$ is in metres and $t$ in seconds. Answer the following questions.
  \begin{parts}
    \part Find its displacement between $t=3$ and \SI5\second.
    \vspace{\stretch1}
    
    \part Derive an expression for the velocity of the object with respect to
    time.
    \vspace{\stretch1}
    
    \part Derive an expression for the acceleration of the object with
    respect to time.
    \vspace{\stretch1}
    ...
    \part At each of those points (from part \textbf{D} above), is the
...
%  \newpage
  ...
  \begin{parts}
    ...
    \begin{subparts}
      \subpart\textbf{Derive} an expression for the acceleration $a$ of the
      block.
      \vspace{\stretch1}
      
      \subpart\textbf{Derive} an expression for the velocity $v$ of the block.
      \vspace{\stretch1}
      
      \subpart\textbf{Derive} an expression for the position $x$ of the block.
      \vspace{\stretch1}
    \end{subparts}

    \part\textbf{Derive} an expression for the position $x_\text{min}$ of the
    block when it is closest to the motion sensor. Express your answer in terms
    of $m$, $D$, $v_0$, $\theta$, and fundamental constants, as appropriate.
    \vspace{\stretch2}
    \newpage
    
    \part On the axes provided below, \textbf{sketch} graphs of position $x$,
    velocity $v$, and acceleration $a$ as functions of time $t$ for the motion
    of the block while it goes up and back down the ramp. Explicitly
    \textbf{label} any intercepts, asymptotes, maxima, or minima with numerical
    values or  algebraic expressions, as appropriate.
    \begin{center}
      \begin{tikzpicture}[scale=1.7,axes]
    ...
    $\mu_k$. \textbf{Derive} an expression for the distance the block slides
    before stopping. Express your answer in terms of $m$, $D$, $v_0$, $\theta$,
    ...
    provided below, \textbf{sketch} a graph of the velocity $v$ as a function
    of time $t$ for the motion of the block while it goes up and back down the
    ramp, arriving at the bottom of the ramp at time $t_f$. Explicitly
    \textbf{label} any intercepts, asymptotes, maxima, or minima with numerical
    values or algebraic expressions, as appropriate.
    \begin{center}
  ...
  constants.
  \begin{parts}
    \part\textbf{Derive} an expression for the velocity of the skier as a
    function of time during the acceleration. Assume the skier starts from rest.
    \vspace{\stretch3}
    
    \part\textbf{Derive} an expression for the work done by the net force on
    the skier from rest until terminal speed is reached.
    \vspace{\stretch1}
    
    \part\textbf{Determine} the magnitude of the force exerted by the rope on
    the skier at terminal speed.
    \vspace{\stretch1}
    
    \part\textbf{Derive} an expression for the total impulse imparted to the
    skier during the acceleration.
    \vspace{\stretch1}
    \newpage
    ...
    are the same as in the original model. On the axes below, \textbf{sketch}
    the graphs of the force exerted by the rope on the skier for the two
    models, from $t=0$ to a time $t>T$. \textbf{Label} the original model $F_1$
    and the new model $F_2$.
    \begin{center}
