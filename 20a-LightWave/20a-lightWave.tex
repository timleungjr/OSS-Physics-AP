...
\begin{frame}{Double-Slit Interference}
  \framesubtitle{Constructive \& Destructive Interference}
  ...
  A dark fringe (destructive interference) occurs when the path difference
  is a half-number ($n+\frac{1}{2}$) multiple of wavelength ($\lambda$), i.e.
...
\begin{frame}{Approximation of The Wavelength of Light}
  We can estimate the wavelength of light based on the distances between
  fringes using the small-angle approximation:
  
  \eq{-.25in}{
    \theta\approx\tan\theta\approx\sin\theta
  }
  
  \vspace{-.15in}The distance from slits to the screen ($L$), and the distance
  of the $n$-th bright fringe from the centre ($y_n$) to $\theta_n$ can be
  approximated by:

  \eq{-.2in}{
    \tan\theta_n=\frac{y_n}{L}\approx\sin\theta_n
  }
  
  \vspace{-.1in}Substituting our small-angle approximation into the constructive
  interference equation:

  \eq{-.2in}{
    n\lambda\approx\frac{y_nd}{L}
    \quad\longrightarrow\quad
    \boxed{\lambda\approx\frac{\Delta y d}{L}}
  }
  
  This equation applies equally to dark fringes as well. 
\end{frame}
...
%      Distance from slits to screen & $x$        & \si{\metre}
%    \end{tabular}
%  \end{center}
%
