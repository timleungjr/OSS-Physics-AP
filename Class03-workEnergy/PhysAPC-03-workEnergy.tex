...
\begin{frame}{In One Dimension}
  For motion confined that moves in only one direction in a one-dimensional
  coordinate system (which is common for AP Physics C), the previous
...
\begin{frame}{Gravitational Force \& Gravitational Potential Energy}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.6]
      ...
      \draw[vectors,red] (0,4)--(0,3) node[below]{$m\vec g$};
    \end{tikzpicture}
  \end{center}
  \begin{itemize}
...
\begin{frame}{Example: Gravity}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=.7]
      \draw[thick,fill=cyan!20] (7.75,0) arc(75:105:30);
      \draw[thick,fill=cyan] (0,3) circle(.2) node[right=3]{$m$};
