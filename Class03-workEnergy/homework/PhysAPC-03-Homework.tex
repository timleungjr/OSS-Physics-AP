...
<<<<<<< HEAD
<<<<<<< HEAD
%    \question A convervative force acting on an object varies with the equation
...
=======
    ...
    The potential energy at $x=\SI2\metre$ is
=======
    \question The conservative force acting on an object varies with the
...
<<<<<<< HEAD
    ...
    % THIS QUESTION SHOULD BE MOVED TO THE CIRCULAR MOTION QUESTIONS
...
<<<<<<< HEAD
    ...
    \part Indicate below whether the work done on the box by the student in the
    interval $t=0$ to $t=\SI2\second$ s would be greater than, less than, or
    equal to the answer in part (c). Justify your answer.
...
=======
%\question The theoretical formula for the potential energy associated with the
>>>>>>> 0187f47 (A whole bunch of stuff that's changed for Fall of 2022?)
...
=======
%  \begin{parts}
%  \part Sketch $U$ versus $x$ using $U_0=\SI{4}{\pico\joule}$ and
%    $a=\SI{2.5}{\femto\metre}$.
%  \part Find the force $F_x$.
%  \part Compare the magnitude of the force at the separation $x=2a$ to that at
%    $x=a$.
%  \part Compare the magnitude of the force at the separation $x=5a$ to that at
%    $x=a$.
%  \end{parts}
...
%      \fill[blue!30!black] circle(1);
=======
%  \question A small block of mass $m$ slides without friction along the
%  loop-the-loop track shown in the figure. The block starts from point $P$ a
%  distance $h$ above the bottom of the loop.
%  \begin{center}
%    \begin{tikzpicture}[scale=1.2]
%      \fill[blue!30] circle(1);
>>>>>>> 2022-09-fall
%      \fill[white] circle(.88);
%      \fill circle(.05);
%      \draw[thick,->](0,0)--(.88,0)node[midway,above]{\scriptsize$R$};
%      \draw[dashed](-4,-1)--(2,-1);
%      \fill[rotate=15,blue!30](0,-1)rectangle(2,-.88);
%        \begin{scope}[rotate=-45]
%          \fill[blue!30](0,-1)rectangle(-4,-.88);
%          \fill[blue](-3,-.88)rectangle(-3.3,-.58)
%          node[white,above,rotate=-45]{\scriptsize$m$};
%        \end{scope}
%        \draw[<->](-3,-1)--(-3,1.5) node[midway,fill=white]{\scriptsize$h$};
...
%  \question A \SI2{\kilo\gram} block $m$ is released \SI4{\metre} from a
%  massless spring with a spring constant of $k=\SI{100}{\newton\per\metre}$
%  that is fixed along a frictionless plane inclined at \ang{30}, as shown in
%  the figure below. %(Plese give answer to $3$ significant figures.)
%  \begin{center}
%    \begin{tikzpicture}[scale=.7]
