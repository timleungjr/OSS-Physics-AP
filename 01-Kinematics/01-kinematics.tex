...
\begin{frame}{Partial Derivatives}
  ...
  example, gravitational potential energy $U_g$ has three variables: masses
  $m_1$ and $m_2$ and the distance $r$ between them:
  ...
  the $\partial$ symbol for partial derivatives). In this example, the partial
...
\begin{frame}{Definite Integrals}
  ...
  \eq{-.3in}{ \Delta\mb{x}=\int_{t_0}^{t_1} \mb{v}(t)dt }
...
\begin{frame}{Motion Graphs}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Bottomline: regardless of the type motion,
  \begin{itemize}
  \item The $v-t$ graph is the slope of the $d-t$ graph
  \item The $a-t$ graph is the slope of the $d-t$ graph
  \end{itemize}
\end{frame}
...
\begin{frame}{Relative Motion}
  \framesubtitle{Relative Motion Notation}
...
\begin{frame}{Relative Motion}
  ...
  \textbf{Galilean law of velocity addition}\footnote{This equation was
    thought to be so obvious that it didn't even have a name until Einstein
    found it to be incorrect for speeds close to the speed of light}:
