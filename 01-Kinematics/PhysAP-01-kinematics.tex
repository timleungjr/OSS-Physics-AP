...
\begin{frame}{Notes on  Writing Vectors}
  In this course, vectors are \emph{printed} using a bold face font, while the 
  ``arrow on top'' notation is used when \emph{writing}.
  \vspace{.1in}
  \begin{columns}
    \column{.43\textwidth}
  ...
  The magnitude of these vectors are expressed in one of two ways:
  \vspace{.1in}
  \begin{columns}
    \column{.5\textwidth}
...
\begin{frame}{Instantaneous Velocity}
  ...
  If position $\mb{x}$ is a continuously differentiable function in time $t$,
...
\begin{frame}{Instantaneous Acceleration}
  In the same way that velocity rate of change in position,
