...
\begin{frame}{Calculating Path Length Difference}
  \begin{columns}
    \column{.4\textwidth}
    \begin{tikzpicture}[scale=.9]
      ...
      \draw[gray](0,0)--(6,0);
      \draw[gray](0,3)--(6,3);
      \draw[<->](5.5,0)--(5.5,3) node[midway,left]{$d$};
      \draw[dashed](2,4.5)--(2,.5);
      \draw[very thick,red,->](0,5)--(2,3)--(4,5);
      \draw[->](2,3.5) arc(90:135:.5)
      node[midway,above]{$\theta_1$};
      \draw[very thick,blue!80!black,->]
      (-.05,5)--
      (1.95,3) node[pos=1,left]{A}--(3.2,0)--
      (4.45,3) node[pos=0,below]{B} node[pos=1,right]{C}--(6.45,5);
      \draw[->](2,2) arc(270:290:1) node[pos=.6,below]{$\theta_2$};
      \draw[dashed](4.45,3)--(3.2,4.25) node[pos=1,left]{D}
      node[pos=.2,left]{$\theta_1$};
      \node at (.25,.25){$n_2$};
      \node at (.25,3.25){$n_1$};
    \end{tikzpicture}

    \column{.6\textwidth}
    Optical path length difference $\Gamma$:

    \eq{-.4in}{
      \Gamma=n_2(AB+BC)-n_1AD
    }

    From basic geometry and the law of reflection, we know that
...
\begin{frame}{Calculating Path Length Difference}
  \begin{columns}
    ...
    \column{.6\textwidth}
    We can also calculate the length AD, and apply Snell's law:

    \vspace{-.45in}{\Large
      \begin{align*}
        n_1AD&=n_1AC\sin\theta_1\\
        &=(2d\tan\theta_2)(n_2\sin\theta_2)
      \end{align*}
    }
  \end{columns}
\end{frame}
...
\begin{frame}{Calculating Path Length Difference}
  \begin{columns}
    ...
    \vspace{-.3in}{\Large
      \begin{align*}
        \Gamma&=2dn_2\left[\frac{1}{\cos\theta_2}-\tan\theta_2\sin\theta_2\right]
        \\
        &=2dn_2\left[\frac{1-sin^2\theta_2}{\cos\theta_2}\right]\\
        &=2dn_2\cos\theta_2
      \end{align*}
    }

    %\vspace{-.25in}As was the case with light passing through an opening,
    Path length difference $2dn\cos\theta_2$ determines the condition for
    constructive and destructive interference.
    %Similar to double-slit interference problems, the path length
    %difference of $2dn\cos\theta_2$ is significant.
  \end{columns}
\end{frame}
...
\begin{frame}{Soap Bubble}
  \begin{center}
    \begin{tikzpicture}[scale=.5]
      ...
      \draw[gray](0,0)--(6,0);
      \draw[gray](0,3)--(6,3);
      \draw[thick,red,->](0,5)--(2,3) node[pos=1,right]{\tiny$\pi$}--(4,5);
      \draw[thick,blue!80!black,->] (-.05,5)--(1.95,3)--(3.2,0)--(4.45,3)
      --(6.45,5);
      \node at (.5,.3){\small $n_\mathrm{soap}$};
      \node at (.5,3.3){\small $n_\mathrm{air}$};
      \node at (.5,-.5){\small $n_\mathrm{air}$};
    \end{tikzpicture}
  \end{center}
...
\begin{frame}{Soap Bubble}
  \begin{center}
    \pic{.6}{soap-bubble.jpg}
  \end{center}
  \begin{itemize}
  \item When incident light is a white-light, some colours experience
    constructive interference, while other colours there is destructive
    interference
  \item The colour pattern comes from the variations of thickness of the film
    (it is not constant!)
  \end{itemize}
\end{frame}
...
\begin{frame}{Oil Film on Water}
  \begin{center}
    \begin{tikzpicture}[scale=.5]
      ...
      \draw[thick,blue!80!black,->] (-.05,5)--(1.95,3)--(3.2,0)--(4.45,3)
      --(6.45,5);
      \node at (.55,.3){\small $n_\mathrm{oil}$};
      \node at (.55,3.3){\ $n_\mathrm{air}$};
      \node at (.5,-.5){\small $n_\mathrm{water}$};
    \end{tikzpicture}
  \end{center}
  \vspace{-.2in}Light travels through air ($n_\mathrm{air}=1$) and strikes a oil
  film ($n_\mathrm{oil}>1$). Below the oil film is water, which has a lower
  refractive index than oil
  \begin{itemize}
  \item Upper boundary: Reflected light has a phase shift of \ang{180} ($\pi$)
    because $n_\mathrm{air}<n_\mathrm{oil}$ (fast to slow medium)
  \item Lower boundary: Reflected light has no phase shifts (slow to fast
    medium)
  \end{itemize}
\end{frame}

\begin{frame}{Oil Film on Water}
  Like the soap bubble, we observe \emph{constructive interference} if path
  length difference is a half-number multiple of wavelength:
   
  \eq{-.2in}{
    2dn_\mathrm{oil}\cos\theta_2=\left(m-{\frac{1}{2}}\right)\lambda
  }

  And \textbf{destructive interference} if path length difference is a
  whole-number multiple of wavelength:
    
  \eq{-.27in}{
    2dn_\mathrm{oil}\cos\theta_2=m\lambda
  }

  Assuming that the refractive index of the oil film is \emph{higher} than
  water:

  \eq{-.3in}{
    n_\mathrm{oil}>n_\mathrm{water}
  }
\end{frame}
...
\begin{frame}{Anti-Reflection Coating}
  \begin{center}
    \begin{tikzpicture}[scale=.5]
      ...
      \draw[gray](0,3)--(6,3);
      \draw[thick,red,->](0,5)--(2,3) node[pos=1,right]{\tiny$\pi$}--(4,5);
      \draw[thick,blue!80!black,->] (-.05,5)--(1.95,3)--(3.2,0)
      node[pos=1,right]{\tiny$\pi$}--(4.45,3)--(6.45,5);
      \node at (.55,.3){\small $n_\mathrm{coating}$};
      \node at (.55,3.3){\ $n_\mathrm{air}$};
      \node at (.5,-.5){\small $n_\mathrm{glass}$};
    \end{tikzpicture}
  \end{center}
  
  \vspace{-.2in}The anti-reflective coating on eyeglasses and camera lenses
  has a refractive index lower than glass (but higher than air), i.e.:

  \eq{-.3in}{
    n_\mathrm{air}<n_\mathrm{coating}<n_\mathrm{glass}
  }
  \begin{itemize}
  \item\vspace{-.2in} Upper boundary: Reflected light has a phase shift of
    $\pi$, same as the soap bubble and oil film (fast to slow medium)
  \item Lower boundary: Reflected light also has a phase shifts of $\pi$!
  \end{itemize}
\end{frame}


\begin{frame}{Anti-Reflection Coating}
  The interference conditions for anti-reflection coating is oppposite to the
  soap bubble and oil film, because the phase shifts occur on both boundaries.

  \begin{itemize}
  \item\emph{Constructive interference} if path length difference is a
    whole-number multiple of wavelength:

    \eq{-.25in}{
      \boxed{2dn_\mathrm{film}\cos\theta_2=m\lambda}
    }
  \item\emph{Destructive interference} if path length difference is a
    half-number multiple of wavelength:
  
    \eq{-.2in}{
      \boxed{2dn_\mathrm{film}\cos\theta_2=\left(m-{\frac{1}{2}}\right)\lambda}
    }
  \end{itemize}
\end{frame}
