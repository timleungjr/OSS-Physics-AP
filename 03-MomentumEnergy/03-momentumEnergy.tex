...
\begin{frame}{Example}
  \textbf{Example 1:} A force $F=4.0x\bm{\hat{\imath}}$ (in newtons) acts
...
\begin{frame}{Work and Kinetic Energy}
  ...
  \item There may be multiple forces acting on an object; each of the forces
    can add or take away potential energies from an object
  \end{itemize}
\end{frame}
...
%  }
  \item When there are non-conservative forces, instead of \emph{trying} to
    ...
    \eq{-.2in}{
      \boxed{K_+U+W_\mathrm{non-conservative}=K'+U'}
  }
...
\begin{frame}
  ...
  \item In Physics 12, we often used the ``average force'' to compute impulse.
    In reality, the average force really is a ``force'' that gets the same
    impulse as the integral on the last slide, i.e.
