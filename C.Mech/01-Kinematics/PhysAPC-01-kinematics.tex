...
\begin{frame}{Instantaneous Velocity}
  ...
  $\mb{v}$ can be found at any time $t$. The \textbf{instantaneous velocity}
  $\mb{v}$ of an object is the time rate of change of position:

  \eq{-.2in}{
    \mb{v}(t)= \frac{d\mb{x}(t)}{dt}
  }
  
  Since $\mb{x}$ has $x$, $y$ and $z$ components in the
  $\bm{\hat{\imath}}$, $\bm{\hat{\jmath}}$ and $\bm{\hat{k}}$
  directions\footnote{They are linearly independent!}, we can take the time
...
\begin{frame}{Instantaneous \& Average Speed}
  ...
  \item\vspace{-.1in}Since distance of \emph{any} path is always positive
    $s>0$, instantaneous speed must also be positive
...
\begin{frame}{Acceleration as Functions of Velocity and Position}
  ...
  \item Gravitational or electrostatic forces:
    $\displaystyle a(x)=\frac{Gm_s}{x^2}\quad a(x)=\frac{kq_s}{x^2}$
  \item Spring force: $\displaystyle a(x)=-\frac{k}{m}x$
  \item Damping force: $a(v)=b v^n$ ($b$ is a damping constant)
  ...
  requires solving a differential equation (see kinematics handout).
\end{frame}
...
\begin{frame}{Kinematic Equations}
  ...
  sheet. You will still be required to integrate when necessary.
\end{frame}
...
\begin{frame}{Relative Motion}
  ...
  relative to $C$. If we add another frame of reference (``$D$''), the equation
  becomes:
