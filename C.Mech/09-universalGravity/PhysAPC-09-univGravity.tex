...
\begin{frame}{Law of Universal Gravitation}
  ...
  between all massive objects, %The force $m_1$ exerts on $m_2$ is
  given by the law of universal gravitation:
...
\begin{frame}{Gravitational Field}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  The \emph{direction} of the gravitational field is toward $m$ (that's what the
...
\begin{frame}{Relating Gravitational Field \& Gravitational Force}
  \vspace{.2in}
  $\mb{g}$ itself doesn't \emph{do} anything unless/until another mass $m$
  enters the field. Then, $m$ experiences a gravitational force $\mb{F}_g$
...
\begin{frame}{Relating Gravitational Potential Energy to Force}
  ...
  \item ``Steepest descent'': the direction of $\mb{F}_g$ is the shortest path
    to decrease $U_g$ 
...
\begin{frame}{Relating Gravitational and Centripetal Force}
  ...
  \vspace{-.1in}Solving for $v$, we obtain the \textbf{orbital speed}
  $v_\mathrm{orbit}$ (usually known as \textbf{orbital velocity}), which does
  not depend on the mass of the small object in orbit:
...
\begin{frame}{Escape Speed from Circular Orbits}
  ...
  We can then solve for the initial speed $v_i=v_\mathrm{esc}$
  (\textbf{escape speed} or \textbf{escape velocity}):

  \eq{-.32in}{
    \boxed{v_\mathrm{esc}=\sqrt{\frac{2GM}{r_i}}}
  }
