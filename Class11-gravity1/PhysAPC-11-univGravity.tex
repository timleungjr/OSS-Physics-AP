...
\begin{frame}{What If You Are Inside Another Mass?}{A Spherical Shell}
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.45]
      ...
      \draw[thick,fill=white] circle(4.9);% node[above]{$m_1$};
...
\begin{frame}{What If You Are Inside Another Mass?}{A Spherical Shell}
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.45]
      \begin{scope}[thick]
        \draw[fill=gray]  circle(5);% node[above]{$m_1$};
        \draw[fill=white] circle(4.9);% node[above]{$m_1$};
...
\begin{frame}{Gravitational Field Strength Inside a Uniform Sphere}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[thick,dashed](1,0)--(1,2) node[pos=0,below]{$R$}--(0,2)
      node[left]{$\dfrac{GM}{R^2}$};
      \draw[very thick,red,domain=1:5] plot(\x,{2/\x});
...
\begin{frame}{Gravitational Field Lines}
  \begin{center}
    \begin{tikzpicture}[scale=1.5]
      ...
      \draw[fill=blue!20,thick] circle(.3) node{$m$};
    \end{tikzpicture}
  \end{center}
  \begin{itemize}
...
%      \draw[fill=blue!80] circle(.2) node[midway,below]{\tiny $M$};
%      \draw[dotted,thick] circle(1);
%      \fill (0,1) circle(.03);
...
%    \hspace{.3in}
%    \begin{tikzpicture}[scale=1.3]
%      \draw[fill=blue!20] circle(1) node[midway,below]{\tiny $M$};
%      \fill (0,1) circle(.03);
%      \draw[->,thick](0,0)--(0,.98) node[midway,right]{\tiny $r$};
%      \fill (2,0) circle(.03);
%      \draw[->,dash dot](0,1) to[out=0,in=135](2,0);      
...
%%      \fill[black] circle(.07) node[right]{\tiny\text{Sun}};
...
%  \textbf{radial velocity} $\vec v_r$ and \textbf{angular velocity}
%  $\vec v_\theta$.
%  \begin{columns}
%    \column{.3\textwidth}
