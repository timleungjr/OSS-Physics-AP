...
\begin{frame}
  ...
  \item Depends on:
    ...
    \item Normal force $N$: The force the two surfaces are pressed against
      each other
    \item Coefficient of friction ($\mu_s$ and $\mu_k$): Snmoothness of the
...
\begin{frame}{Drag}
  \textbf{Drag} (or \textbf{fluid resistance}) is the force opposing to the
  motion of an object moving in a fluid. Unlike kinetic friction, drag depends
  heavily on velocity. The direction of the drag $\mb{D}$ moving in a fluid is
  opposite to the velocity vector, and its magnitude is:
