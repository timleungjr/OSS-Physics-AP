...
\begin{frame}
  ...
  \vspace{-.15in}Assume that both velocity and momentum are continuous functions
  in time. We then apply the chain rule to find the infinitesimal change in
  momentum ($dp$) with respect to $\gamma$ and $v$:
  
  \eq{-.3in}{
    p=\gamma mv \quad\rightarrow\quad dp= \gamma dv +vd\gamma
  }

  \vspace{-.15in}Substituting that back into the integral, we have:
