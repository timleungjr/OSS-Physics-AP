...
\begin{frame}{Falling To the Center of the Earth}
  \begin{columns}
    ...
    \column{.75\textwidth}
    The gravitational force on this person has the same form as Hooke's law: a
    force that is proportional to displacement, but in the opposite direction:

    \eq{-.3in}{
      F_g(r)=-kr
    }
    
    \vspace{-.15in}The solution to this person's motion is a simple harmonic
    motion. He/she will oscillate through Earth with a period of:
...
\begin{frame}{What If?}
  \begin{itemize}
  \item What happens if $v_\mathrm{orbit}<v<v_\mathrm{esc}$? What kind of an orbit
    does it have?
  \item What happens if $v<v_\mathrm{orbit}$? It doesn't necessarily mean that
    the object will crash into the planet.
  \end{itemize}
\end{frame}
...
\begin{frame}{Kepler's 2nd Law: The Law of Equal Areas}
  ...
  \begin{columns}
    ...
    \end{tikzpicture}
    \column{.7\textwidth}
    The infinitesimal area $d\mb{A}$ swept out by an object as it moves in
    orbit by an infinitesimal amount $d\mb{r}$ is given by:
    ...
    \vspace{-.15in}The time derivative is called the \textbf{areal velocity}:
...
\begin{frame}{Kepler's Second Law: The Law of Equal Areas}
  ...
  \ldots as predicted by Kepler's second law. The rate a planet sweeps out the
  area in orbit is one-half of its angular momentum around the sun, divided by
  its mass.
\end{frame}
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=.5]
      ...
      \begin{scope}[rotate around={\angle:({sqrt(\a*\a-\b*\b)},0)}]
        ...
        node[pos=1,left]{\tiny $v_r$};
        \draw[->](7.65,0)--(7.65,.85) node[below]{\tiny $v_\theta$};
...
%      node[midway,left]{\tiny$r_0$};
    ...
    \begin{itemize}
    \item\textbf{Angular velocity} $\mb{v}_\theta$ in the angular direction.
      This presence of $\mb{v}_\theta$ means a centripetal acceleration toward
      $M$:
    ...
    \item\vspace{-.25in}\textbf{Radial velocity} $\mb{v}_r$. For circular
      orbits, $v_r=0$. If this velocity changes with time (necessary for
      elliptical orbits), then there will be an acceleration, also in the
      radial direction:
...
%      node[midway,left]{\tiny$r_0$};
    \end{tikzpicture}
    \column{.7\textwidth}
    As $m$ orbits around $M$, both components of acceleration are due entirely
    to gravitational force. Applying Newton's second law of motion gives the
...
%      node[midway,left]{\tiny$r_0$};
    ...
    This is where ``math magic'' happens. This differential equation is
    difficult to solve, and the equation for an ellipse depends on $\theta$ but
    not on $t$.
  \end{columns}
\end{frame}
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  First, we do a change of variable, and define

  \eq{-.2in}{
    u=\frac{1}{r}
  }
  
  \vspace{-.1in}Then, we use the fact that angular momentum $L$ is constant
  to relate derivatives in time $t$ to derivatives in angle $\theta$:
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  With this change, $d^2r/dt^2$ can be expressed in terms of $u$ and $\theta$.
  The first derivative $dr/dt$:
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  ...
  coefficients and a constant forcing function\footnote{
    This form of differential equation was first encountered in the harmonic
    motion problem.
  }:
  ...
  \vspace{-.1in}The solution to the differential equation is:
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  ...
  \vspace{-.1in}where $e$ is a constant:

  \eq{-.3in}{
    e=\frac{AL^2}{GMm^2}
  }
  
  \vspace{-.1in}The \emph{aphelion} and \emph{perihelion} (points of furthest
