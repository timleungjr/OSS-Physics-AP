...
%      node[midway,left]{\tiny$r_0$};
    ...
    This differential equation is difficult to solve, and the
    equation for an ellipse (that we've shown a few slides ago) depends on
    $\theta$ but not on $t$.
  \end{columns}
\end{frame}
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  To make things simpler, We define a new variable (this is \emph{not}
  obvious):
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  ...
  First, we find the first derivative $\dot{r}$ in terms of $\theta$:
...
\begin{frame}{Kepler's First Law: Law of Ellipses}
  The original differential equation, now written in terms of $u$ and
  ...
  \eq{-.2in}{
    u(\theta)=\frac{GMm^2}{L^2}+B\cos\theta
  }
\end{frame}
