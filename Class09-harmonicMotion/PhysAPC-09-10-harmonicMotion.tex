...
\begin{frame}{Horizontal Spring-Mass System}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.1]
      ...
      \draw[vectors,red] (5.5,1)--(5.5,2) node[above]{$\vec N$};
      \draw[vectors,red] (5.5,1)--(4.5,1) node[above]{$\vec F_s$};
    \end{tikzpicture}
  \end{center}
  $\vec F_g$ and $\vec N$ cancel out, so net force is due only to spring force
...
\begin{frame}{The Simple Pendulum}
  \begin{columns}
    ...
    shows that for small angles, $\sin\theta\approx\theta$
...
\begin{frame}{It's Never Perfect}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \uncover<2->{
        \draw[vectors,pink] (3.5,1)--(3.5,0) node[below]{$\vec F_g$};
...
\begin{frame}{Forced Harmonic Motion}
  ...
  {\color{magenta}additional external force term} on the right-hand side:

  \eq{-.1in}{
    m\diff[2]xt+b\diff xt+kx={\color{magenta}F\cos(\omega_Et)}
  }
\end{frame}
