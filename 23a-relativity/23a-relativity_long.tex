...
\begin{frame}
  ...
  \end{itemize}
  %\item Our ``equations of motion'' applies to all inertial frames of reference
  %\item For centuries, when experimental results didn't agree with Newtonian
  %  laws, the theory won at the end\ldots
  %\item Until the middle of 19th Century, when instruments were accurate enough
  %  to measure behaviours that were a bit different from prediction
...
\begin{frame}
  ...
  \vspace{-.1in} Now that you know $d\mb{x}$ and $dt$ depend on motion, we
...
\begin{frame}
  ...
  with respect to time. This has not changed
  ...
  \vspace{-.1in}We can substitute the relativistic expression for momentum into
  the expression for force, and then integrate
\end{frame}


\begin{frame}
  \frametitle{Work and Energy}
  The integral is not particularly difficult. For a 1D motion with no
  acceleration, we can rearrange the terms in the integral:

  \eq{-.2in}{
    W=\int Fdx=\int\frac{dp}{dt}dx=\int vdp
  }
  
  Assuming that both velocity and momentum are continuous in time. We need to
  take the derivative of the relativistic momentum term:
  
  \eq{-.25in}{
    p=\gamma mv \quad\rightarrow\quad dp= \gamma dv +vd\gamma
  }

  Substitiuting that into the integral, we have:
...
\begin{frame}
  ...
  \eq{-.25in}{
    W=\int mv(\gamma dv +vd\gamma)=\int m\left[\frac{c^2}{\gamma^2}+
      c^2\left(1-\frac{1}{\gamma^2}\right)\right]d\gamma
  }

  What we end up with is surprisingly simple:

  \eq{-.25in}{
    W=\int_{\gamma=1}^\gamma mc^2d\gamma
  }
